...
| `/database/reports`, `/pm/reports` | ✅ Done (Phase 4) | export helper ยก generic ที่ `lib/reports/` (tax re-export); `lib/master/reports.js` (3 data-quality) + `lib/pm/reports.js` (3 operational/management); หน้า + API ทั้งคู่ผ่าน `components/reports/ReportView.js` ตัวเดียวกัน |
...
### Acceptance
- [x] แต่ละ report อยู่ใต้โมดูลเจ้าของ + มี Excel/PDF export (ผ่าน `reportToXlsxBuffer` + `openReportPrintWindow` ที่ generic แล้ว)
- [x] Database report ไม่มี workflow metric — เป็น data-quality ล้วน (ครบ/ขาดข้อมูล, รออนุมัติ, ข้อมูลกำพร้า); tax usage count (registrations/orders) gate ด้วย `history:view` กัน staff/viewer เห็น

**สถานะ:** ✅ Done. ไฟล์:
- export helper generic: `lib/reports/exportExcel.js`, `lib/reports/reportPrint.js` (ย้ายจาก `lib/tax/` — tax เหลือ re-export shim)
- master: `lib/master/reports.js` (customer/product quality + usage-orphan), `api/master/reports/route.js`, หน้า `database/reports/page.js`
- pm: `lib/pm/reports.js` (project overview + overdue tasks + team summary; computed status/progress mirror หน้า board), `api/pm/reports/route.js`, หน้า `pm/reports/page.js`
- UI ใช้ร่วม `components/reports/ReportView.js` (segmented tabs + date/status/customer filter + Excel/Print + เลือกแถว) — tax page ยังเป็นตัวเดิม (มี ZIP ไฟล์แนบเฉพาะ)
- sidebar: เพิ่มเมนู "รายงานคุณภาพข้อมูล" (master, `products:view`) + "รายงาน" (pm, `pm:view`)

**Security/scope:** master gate `products:view||customers:view` (กว้าง — คุณภาพข้อมูลเป็นเรื่องของทุกคน), products team-scoped, customers central; pm gate `pm:view` + team-scope เหมือน `/api/pm/projects`.
