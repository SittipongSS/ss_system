...
| `/database/reports`, `/pm/reports` | ⏸ Deferred | **ยังไม่จำเป็น** (ผู้ใช้ตัดสิน 2026-06-26) — โค้ดเขียนเสร็จแล้วเก็บใน branch `claude/infallible-knuth-beb640` (commit `1465ad5`, **ไม่ merge**) เผื่อทำทีหลัง |
| Authorization / Transaction boundary | ✅ Done (Phase 5.1–5.2) | codify เป็น blueprint `BOUNDARY_MAP.md` (matrix สิทธิ์ × role + scope, transaction checklist + manual-rollback pattern) |
| Audit log | 🟡 Foundation (Phase 5.3) | ตาราง `audit_logs` (mig 0049) + helper เดียว `lib/audit.js`; wired customers/products/orders CRUD. เหลือหน้า `/audit` + API อ่าน |
| Unique customer `taxId + branchCode` | ✅ Done (Phase 5.4) | DB unique `(taxId, branchCode)` มีแล้ว (mig 0039, deployed); completeness gate ตอนยื่นทะเบียนใน `requirements.js` |
...
## Phase 4 — Report Boundary ⏸ Deferred (ยังไม่จำเป็น)

> **สถานะ (2026-06-26):** ผู้ใช้ตัดสินว่า report ของระบบฐานข้อมูล + ระบบจัดการโครงการ **ยังไม่จำเป็น**.
> โค้ดเขียนเสร็จแล้ว (3 master + 3 PM reports + export ร่วม) **เก็บไว้ใน branch `claude/infallible-knuth-beb640`
> (commit `1465ad5`) แต่ไม่ merge** — รื้อกลับมาทำได้เมื่อมีความต้องการจริง. `/tax/reports` ที่มีอยู่แล้วยังใช้งานปกติ.

**เป้าหมาย (เมื่อกลับมาทำ):** report เชิง workflow อยู่กับโมดูลเจ้าของ; Database report = คุณภาพข้อมูลกลาง
...
### 5.1 Authorization Boundary — ✅ Done
- codify เป็น blueprint [`BOUNDARY_MAP.md`](BOUNDARY_MAP.md): matrix **capability × role** (8 roles) + **data scope** (view/edit/delete) + คำตอบ "module ↔ master": ไม่มีสิทธิ์โมดูลแยก, master read มากับ per-resource cap อัตโนมัติ, write master ต้องมี `*:edit` ตรงๆ + ทำที่หน้า Database.

### 5.2 Transaction Boundary — ✅ Done
- เพิ่ม Transaction Checklist ใน `BOUNDARY_MAP.md` + อ้าง pattern manual-rollback (`orders/route.js` insert items fail → ลบ header). กฎ: cross-module write ต้องมีเจ้าภาพ transaction เดียว + rollback path ชัด.

### 5.3 Audit Log กลาง — 🟡 Foundation
- ตาราง `audit_logs` ([migration 0049](supabase/migrations/0049_audit_logs.sql), ⚠ รันมือ) + helper เดียว [`lib/audit.js`](src/lib/audit.js) `recordAudit()` (ไม่ throw, snapshot actor, before/after jsonb).
- wired: **customers / products / orders** create/update/delete (+ approval). 
- **เหลือ:** หน้า `/audit` (supervisor only, `audit:view`) + `/api/audit` อ่าน log; wire registration/project/user; action login/approve.

### 5.4 Unique Customer = `taxId + branchCode` — ✅ Done
- DB unique `(taxId, branchCode)` มีแล้วจริงใน [migration 0039](supabase/migrations/0039_customer_branch_shipping.sql) (deploy prod แล้ว — ไม่ต้อง migration ใหม่).
- **บังคับเฉพาะตอนยื่นทะเบียน** ผ่าน completeness gate ใน [`lib/tax/requirements.js`](src/lib/tax/requirements.js): ขาด taxId/branchCode → `missing[]` → submit ถูกบล็อก (เผื่อบุคคลธรรมดา/ต่างชาติที่ยังไม่มีตอนสร้าง customer).

**ขนาด:** S (5.1/5.2 เอกสาร) + S (5.4 gate, ไม่มี migration) + M (5.3 foundation: 1 migration + helper + wire 3 resource)

---

## Phase 6 — เก็บเอกสารเป็น Architecture Blueprint ✅ Done

1. ✅ blueprint เข้า repo แล้ว: [`BOUNDARY_MAP.md`](BOUNDARY_MAP.md) + ไฟล์นี้ ตามคอนเวนชัน `*_PLAN.md`.
   **หมายเหตุ:** ต้นฉบับภายนอก `MODULE_BOUNDARY_MAP.md` ของ codex ไม่ได้อยู่ในเครื่อง — `BOUNDARY_MAP.md`
   เขียนใหม่จากโค้ดจริงแทน (self-contained: มี Action Placement table แทน "map section 3/4" เดิม).
2. ✅ [`ATTACHMENT_REQUIREMENT_SPEC.md`](ATTACHMENT_REQUIREMENT_SPEC.md) แยกไว้แล้ว + doc-index ใน `BOUNDARY_MAP.md` ลิงก์ชุดเอกสารครบ.
3. ✅ Decision Checklist รวม Transaction (5.2) + Authorization (5.1) + audit แล้ว (New-Module Checklist ด้านล่าง ข้อ 9–10).
4. ทุกฟีเจอร์ใหม่ต้องผ่าน checklist ก่อนลง schema/UI (กติกาต่อเนื่อง).

### New-Module Checklist (ทุกโมดูลใหม่ต้องตอบครบก่อนเริ่ม)
> ฉบับเต็ม + คำอธิบายอยู่ใน [`BOUNDARY_MAP.md`](BOUNDARY_MAP.md#new-module-checklist-ทุกโมดูลใหม่ต้องตอบครบก่อนเริ่ม)
```
1. owner คือใคร
2. ใช้ master entity ไหน (อ้างด้วย id)
3. ต้องเก็บ snapshot อะไร (กันประวัติเพี้ยนเมื่อ master เปลี่ยน)
4. action สำคัญคืออะไร + ทำที่หน้าไหน (write ห้ามข้ามโมดูล — ดู Action Placement)
