...
สถานะ: **ใช้งานจริงแล้ว — Drive เป็นที่เก็บเดียว** · อัปเดต 2026-07-30

## สิ่งที่เปลี่ยนรอบ 2026-07-30 (อ่านตรงนี้ก่อนไล่อ่านแผนเดิมข้างล่าง)

**1. ตัดทาง Supabase Storage ของไฟล์แนบทิ้งทั้งสาย** — ยืนยันกับ prod ว่า attachments
128/128 แถวอยู่บน Drive ครบ ไม่มีแถวไหนเหลือ · `STORAGE_BACKEND` ถูกลบ (เคยเป็นเหตุให้
ปุ่มสร้างเอกสาร Google ของงานบริหารตอบ 400 ทั้งที่ Drive ใช้ได้) · bucket `sales-evidence`
(หลักฐาน Won) กับ `issued-quotation-pdf` เป็นคนละเรื่อง ยังอยู่ที่เดิม

**2. โครงโฟลเดอร์ใหม่** (ของเดิม: โฟลเดอร์ลูกค้ากองที่ root ปนกับโฟลเดอร์โมดูล)
```
<root>/
├─ ลูกค้า/<ชื่อ> (CUS-xxx)/
│   ├─ เอกสารบริษัท/     ← หนังสือรับรอง ภ.พ.20 แผนที่ สัญญา
│   ├─ สินค้า/<ชื่อ> (FG-xxx)/   ← artwork + ฉลากที่ยื่นทะเบียน
│   └─ ออเดอร์/          ← หลักฐานชำระ ใบเสร็จสรรพสามิต
├─ ขอราคา/{ผลิต|วัสดุ}/<ปี>/<เลขที่ใบ>/<รายการ> (id)/
├─ งานบริหาร/{งานติดตาม|การประชุม}/
├─ งานขาย/{งานส่วนบุคคล|ลีด|ดีล|ใบเสนอราคา|ใบสั่งขาย}/
├─ สหมิตร/<เลขที่ PO>/
└─ _รอจัดที่/
```
ชื่อโฟลเดอร์ทั้งหมดอยู่ที่ `FOLDER` ใน `lib/drive.js` ที่เดียว

**3. 🐞 บั๊กที่แก้ในรอบนี้**
- **ชื่อ entity ของเธรดไม่ตรงกับของไฟล์แนบ** (`excise_registration` vs `registration`)
  → รูปที่แนบในเธรดทะเบียนภาษี/ใบยื่น **ตกถัง `_รอจัดที่` ทุกไฟล์** แนบขึ้นแต่หาไม่เจอ ·
  ทะเบียนชื่อพ้องอยู่ที่ `lib/master/driveEntityMap.js` + เทสต์กันของใหม่หลุด
- **UI กลืนข้อความ error ของ server** (`AttachmentsPanel`, `UpdateThread`) → ผู้ใช้เห็นแค่
  "อัปโหลดไฟล์ไม่สำเร็จ" ไม่มีทางรู้ว่าเพราะชนิดไฟล์/ขนาด/Drive
- **ชนิดไฟล์ที่รับแคบเกินจริง** → เติม `.heic/.heif` (รูปจาก iPhone), `.ai/.psd/.eps`
  (Artwork ที่เป็นเอกสารบังคับ), `.doc/.xls/.ppt`, `.zip/.rar/.7z`
- **`driveFolderId` ที่ cache ไว้ไม่มีทางถอย** → คนย้าย/ลบโฟลเดอร์บน Drive ด้วยมือแล้ว
  ลูกค้ารายนั้นอัปโหลดพังถาวร · ตอนนี้ตรวจว่าโฟลเดอร์ยังอยู่ก่อนใช้ + สร้างใหม่ให้เอง
- **ลบไฟล์ = ลบถาวร** (`files.delete`) → เปลี่ยนเป็นทิ้งลง**ถังขยะ** = กู้ได้ 30 วัน
  โดยไม่ต้องมี Google Vault (ซึ่งต้องเป็น Workspace admin + แพ็กเกจ Business Plus ขึ้นไป)
- **`contentType` มาจาก client** → ตัดสินจากนามสกุลฝั่ง server + `nosniff` + บังคับ
  ดาวน์โหลดสำหรับชนิดที่ไม่ปลอดภัยจะเปิดในหน้า

**4. หน้าเครื่องมือ `ตั้งค่า → ที่เก็บไฟล์` (admin เท่านั้น)** — ตรวจการเชื่อมต่อทีละขั้น ·
ตรวจว่าไฟล์แนบทุกใบยังเปิดได้จริง · ดูแผน/สั่งจัดโครงโฟลเดอร์ (idempotent กดซ้ำได้)
> การย้ายบน Drive = เปลี่ยน parent เท่านั้น **id ไม่เปลี่ยน** ลิงก์เดิมทุกอันยังใช้ได้

---

## แผนเดิม (2026-06-24) — เก็บไว้เป็นบันทึกการตัดสินใจ
