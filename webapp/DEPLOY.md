...
### หลัง deploy — แท็บที่เปิดค้างรีเฟรชเอง (#1829 · 25/09/2026)

ทุก deploy ที่เลขคอมมิตเปลี่ยน = สัญญาณ "บังคับรีเฟรช" ของทุกแท็บที่เปิดค้างอยู่
(เจ้าของสั่ง "บังคับรีเฟรชทุกแอคเค้า" · เลือกแบบนี้แทนการเตะออกจากระบบทุกคน)

- ตอน build `next.config.mjs` ฝัง `VERCEL_GIT_COMMIT_SHA` ลงโค้ดฝั่งเบราว์เซอร์ (`NEXT_PUBLIC_BUILD_SHA`)
  แล้ว `VersionWatcher` ใน AppLayout ถาม `/api/version` ทุก 5 นาทีตอนแท็บเปิดอยู่ และทันทีที่กลับมาดูแท็บ
- เลขไม่ตรง ⇒ แถบ **"ระบบอัปเดตเป็นเวอร์ชันใหม่แล้ว"** + ปุ่มรีเฟรช · และ **เปลี่ยนหน้าครั้งถัดไป = รีโหลดเต็มหน้า**
  (ไม่รีโหลดกลางหน้าที่กำลังใช้ — ฟอร์มที่พิมพ์ค้างไม่หาย · ตัวกันงานหาย `useUnsavedChanges` ยังถามก่อนทิ้ง)
- **อยากยิงรีเฟรชทุกคนเดี๋ยวนี้** = deploy คอมมิตใหม่ขึ้นไป (ปุ่ม Run workflow ข้างบน) ·
  ⚠️ สั่ง deploy ซ้ำตอน `production` เท่ากับ `main` อยู่แล้ว **ไม่ยิงอะไร** — เลขคอมมิตไม่เปลี่ยน
- ไม่ทริกตอนเปลี่ยนบทบาทผู้ใช้ (บทบาทอ่านสดตอนเปิดหน้าใหม่อยู่แล้ว แต่แท็บที่ค้างจะยังไม่รู้จนกว่าจะเปลี่ยนหน้า
  หลัง deploy หรือกด F5 เอง) · กติกาเต็มอยู่ `src/lib/ui/versionWatch.js`
