...
>
> §13 "จุดตัดสินใจที่ต้องเคลียร์" ปิดครบทั้ง 3 ข้อแล้ว (ขีดฆ่าไว้ในเอกสาร) ·
> §2 "จุดตัดสินใจที่ล็อกแล้ว" ยังเป็นบันทึกว่าทำไม PM ถึงออกมาเป็นแบบนี้
>
> ⚠️ โครงสร้าง URL เปลี่ยนไปแล้วหลังเอกสารนี้ — `/pm` เหลือแค่ `/pm/tasks`
> ส่วนหน้าโครงการย้ายไป `/sa/projects` (ดู `SALES_PM_MERGE_PLAN.md`)
> โค้ดจริงอยู่ที่ `src/lib/pm/` (40+ ไฟล์ พร้อมเทสต์)
...
| ช่อง | ความหมาย | ใครเขียน | ใครอ่าน |
|---|---|---|---|
| `acOwner` / `acOwnerId` | **ผู้ประสานงานของงาน** | ฟอร์มโครงการ | PDR (ช่องผู้ประสานงาน) · แจ้งเตือน ([updateAccess](src/lib/master/updateAccess.js)) |
| `preparedBy` | **ผู้จัดทำ** ของหัวเอกสาร ISO | server ตั้งเป็นผู้สร้าง | เอกสารที่พิมพ์ |

🐞 เดิมช่องบนจอเขียนว่า "ผู้ประสานงานโครงการ (AC)" แต่ค่าไหลลง `preparedBy`
⇒ `acOwner` ว่างทั้ง 90 ใบบน prod · ช่องผู้ประสานงานบนใบ PDR ว่างตลอดกาล และ AC
ไม่เคยได้รับแจ้งเตือนของโครงการที่ตัวเองประสาน — ไม่ใช่เพราะไม่มีคนกรอก แต่กรอกคนละช่อง

AC ต้องเป็นบัญชี role `ac` ในทีมเดียวกับงาน (`resolveProjectAcOwner`) ·
**ไม่แตะ `team`/`ownerId`** — AC ประสานงาน ไม่ใช่เจ้าของงาน

### ⭐ โครงการมีครบสามฝ่ายตั้งแต่วันเกิด (มติผู้ใช้ 2026-08-14)

| ฝ่าย | คอลัมน์ | ตำแหน่งที่รับได้ | มีผลกับอะไร |
|---|---|---|---|
| ผู้ดูแล | `aeOwner` + `aeOwnerId` | `ae` · `senior_ae` | **ทีม/เจ้าของของแถว** เดินตามคนนี้ + แจ้งเตือน |
| ผู้ประสานงาน | `acOwner` + `acOwnerId` | `ac` (ทีมเดียวกับงาน) | ช่องผู้ประสานงานบน PDR + แจ้งเตือน |
| ผู้ตรวจสอบ | `aeSupervisor` + `aeSupervisorId` | `ae_supervisor` (ไม่มีด่านทีม) | ชื่อบนใบเสนอราคา + แจ้งเตือน |

**บังคับทั้งสามช่องตอนสร้าง** ทุกทาง (โครงการขาย · จากดีล · PO สหมิตร) — ตัวเลขจริง
ก่อนบังคับ: AC กรอกอยู่แล้ว 89/90 ใบ · ผู้ตรวจสอบ 92/93 ⇒ ที่ขาดคือคนลืม ไม่ใช่ไม่มีคน
· ช่องที่ถูกล็อกเป็นชื่อผู้สร้าง (AE/AC/หัวหน้า สร้างเอง) ถือว่ากรอกแล้ว

⚠️ **PATCH ไม่บังคับ** — ใบเก่าที่ช่องยังว่างต้องแก้ช่องอื่นได้ตามปกติ · แต่ **ล้างค่าที่มี
อยู่แล้วไม่ได้** ทั้งสามช่อง (เปลี่ยนเป็นคนอื่นได้ ถอดคนออกเฉย ๆ ไม่ได้)
