...
โลโก้ทางการ = **โลโก้ตัวเต็ม (ต้นไม้ + wordmark) จากไฟล์ SVG ต้นฉบับของฝ่ายแบรนด์**
→ **กฎ: ต้องปรากฏทุกหน้าจอและทุกเอกสารที่ระบบออก**

**ต้นฉบับเดียวคือ `src/lib/brandLogo.js`** (เก็บเส้นโลโก้เป็นสตริง + `brandLogoSvg()`);
`public/scent-sense-logo.svg` คือภาพเดียวกันเก็บไว้ให้งานนอกแอปหยิบไปใช้ ห้ามเพิ่มไฟล์โลโก้ที่อื่นอีก
ไฟล์ PNG เดิม (`brand-logo.png`, `scent-sense-logo.png`) ถูกถอดออกแล้ว

**สีมาจากบริบท ไม่ใช่จากไฟล์** — เส้นเป็น `currentColor` ทั้งหมด:

| ที่ | สี | มาจาก |
|---|---|---|
| Top bar | ขาว | `.topnav-brand-img` (แถบเป็นพื้นกรมท่าทั้งสองธีม) |
| login / หน้าแรก | กรมท่า (สว่าง) · ขาว (มืด) | `.brand-mark` |
| เอกสารพิมพ์ | กรมท่าตายตัว | `SYSTEM_DOCUMENT_LOGO_URL` = data URI ที่ฝังสีมาแล้ว |

> ทำไมเอกสารต้องฝังสี: `currentColor` ใน `<img src="...svg">` ไม่รับสีจากหน้า มันจะกลายเป็นสีดำ

| ที่ | การใช้ | ทำเมื่อ |
|---|---|---|
| Top bar (ชั้นระบบ) | `<BrandMark />` | ทำแล้ว |
| หน้า login / หน้าแรก | `<BrandMark className="brand-mark" />` | ทำแล้ว |
| เอกสารพิมพ์ทุกใบ | `<img src={SYSTEM_DOCUMENT_LOGO_URL}>` จาก `lib/documentBrand.js` — Gantt, QT, SO, ใบวางบิล/รายงาน Tax, ใบเตรียมส่งของ | ทำแล้ว |

- **หน้าจอใช้ `<BrandMark />` เท่านั้น เอกสารใช้ `SYSTEM_DOCUMENT_LOGO_URL` เท่านั้น** ห้ามอ้างไฟล์ภาพตรง ๆ
- โลโก้เป็นแนวนอน ~2.61:1 — **ห้ามยัดลงกรอบจัตุรัส** (กรอบ 56×56 เดิมของหน้า login/hub ถูกถอดแล้ว)
- โลโก้มี wordmark ในตัว — อย่าใส่ข้อความ "Scent & Sense" ซ้ำใต้ภาพ
