...
| R5 | ✅ **ความเคลื่อนไหว (activities) แนบรูป + พรีวิว + เก็บ Drive** — เสร็จ | **เลือก jsonb-on-activity แทนตาราง attachments กลาง** (ตารางกลางผูก permission กับ customer/product ไม่ใช่ scope ดีล + บล็อกรูป): `sales_deal_activities.attachments jsonb` (mig **0083** — ต้องรันมือ prod ก่อน deploy ไม่งั้นโพสต์ activity พังทั้งหมด); อัปผ่าน `/api/upload` (Drive/Supabase, resolve โฟลเดอร์ลูกค้าจาก deal.customerId); proxy ใหม่ `GET /api/sales-planning/activities/[id]/file?i=` stream Drive/redirect Supabase โดยเช็ค view-scope ดีล; broaden `ACCEPTED_UPLOAD_MIME` เพิ่มรูป (PNG/JPG/WEBP — ตรงกับข้อความ error เดิมที่โกหกว่ารับรูปแต่ constant บล็อก); UI: ปุ่มแนบ + พรีวิว thumbnail ก่อนโพสต์ + thumbnail ในฟีด + lightbox เต็มจอ; โพสต์รูปล้วน (ไม่มีข้อความ) ได้ |
...
| R7 | ✅ **หน้าภาพรวม: ปุ่มดูเต็มจอ + เปิดให้เห็นทุกทีม + ปุ่มทุกเดือน** (req 2026-07-09 หลัง merge #163) | (ก) ปุ่ม "เต็มจอ" ใช้ Fullscreen API บนกล่องเนื้อหา (`fsRef`) + ปุ่มออกในเต็มจอ; (ข) **dashboard route เปิดเป็นระดับทั้งฝ่าย** — เอา `applyDealScope`/`inSalesViewScope`/target-scope ออก คืนทุกทีมให้ทุก sales role (นโยบายผู้ใช้: overview โปร่งใส) → **จงใจกลับทิศ N4 เฉพาะ dashboard**; หน้า pipeline (deals) + targets grid ยัง scope ตามเดิม; (ค) toggle "ทุกเดือน" ใน `MonthPicker` (ปียังเปลี่ยนได้) → KPI+byForecast รวมทั้งปี (`yearAggregate`) |
...
| 7 (R5) | ✅ `0083_sales_activity_attachments.sql` — `ALTER TABLE sales_deal_activities ADD COLUMN attachments jsonb DEFAULT '[]'`. **ต้องรันมือบน prod ก่อน deploy** (โค้ด insert activity ใส่ `attachments` เสมอ — column ไม่มี = โพสต์ทุกอันพัง) + `NOTIFY pgrst, 'reload schema'` |
