...
const CAPS = {
  ...
  /* ⚠️ **ขึ้นทะเบียนตอนยังไม่เจ็บ** (2026-09-08 · mig 0350) — 30 แถววันนี้ แต่มันโต
     ...
     **ครั้งแรกที่มีจุดอ่านข้ามใบ** (ตารางงานผู้ปรุงกลิ่น) ⇒ ลงทะเบียนไว้เพื่อบังคับให้
     จุดอ่านใหม่ทุกจุดต้องห่อ fetchAll ตั้งแต่ต้น แทนที่จะไปไล่ตามตอนตัวเลขเพี้ยนแล้ว
     (แพตเทิร์นเดียวกับ `service_assets: 0`)
     ⚠️ **สี่จุดที่นับอยู่ตอนนี้ปลอดภัยคนละแบบกัน** — สามจุดกรองด้วย `requestId` ของ
     ใบเดียวซึ่ง `MAX_SCENT_BRIEFS = 40` การันตีเพดานให้แล้ว · ส่วน
     `master/scentFormulaAdmin.js` กรองด้วย `.in('id', briefIds)` ที่ยาวตามทะเบียนกลิ่น
