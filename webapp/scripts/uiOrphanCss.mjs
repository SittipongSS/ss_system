...
/* ⚠️ ว่างโดยเจตนา — เดิมเป็น ["src/components/documents/", "src/lib/"] พร้อมคอมเมนต์ว่า
   "เอกสารพิมพ์มี CSS ของตัวเองและคลาสถูกประกอบใน HTML string ของ builder"
   ล้างทิ้ง 2026-09-02 เพราะ **ทั้งสองตัวยกเว้นศูนย์ไฟล์**: ตัวกรองรับเฉพาะไฟล์ .css
   และวัดแล้วไม่มีไฟล์ .css อยู่ใต้สองพาธนั้นเลยสักไฟล์ (0 จาก 131) — เอกสารพิมพ์
   ประกอบสไตล์เป็นสตริงในโค้ด .js ไม่ได้มีชีตของตัวเอง คำอธิบายเดิมจึงบรรยายกลไก
   ที่ไม่มีอยู่จริง และทำให้คนอ่านเชื่อว่ามีการยกเว้นเกิดขึ้นทุกรอบ
   เก็บตัวแปรไว้ (ไม่ลบทิ้ง) เพราะจุดเรียกที่ล่างยังอ่านมันอยู่ และวันหน้าถ้าเอกสารพิมพ์
   มีชีตจริงจะได้เติมพาธที่มีอยู่จริง — พร้อมวัดก่อนว่ายกเว้นกี่ไฟล์ */
export const EXEMPT_CSS = [];
