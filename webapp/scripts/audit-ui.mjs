...
/* ── ระยะห่างที่เขียนเป็นเลขดิบใน `className` (2026-09-02) ────────────────────
   ...
   📉 32 → 29 (2026-09-06) — `mt-[18px]` สามจุดหายไปพร้อมกล่องครอบที่มีไว้เว้นระยะอย่างเดียว ตอนนี้ .ui-workspace ออก gap ให้เอง */
const RAW_TAILWIND_SPACING_CAP = 26;
...
/* ── ขนาด (width/height) ที่เขียนเป็นเลขดิบใน `className` (2026-09-02) ──────
   ...
   **ไม่มี --size-1..9 และไม่ควรมี** · กฎที่เขียนไว้ในรีโปบอกคนละเรื่องกับชั้นพิมพ์:
   ...
   ⚠️ **2 จาก 13 ไม่ใช่ utility จริง** — ใน settings/design-preview/page.js ตัวหนึ่งคือ *ข้อความ*
   ในหัว StatusNotice («อย่าเติม w-full / text-xs / h-[32px] ที่ปลายทาง» — ค้นด้วยข้อความ เลขบรรทัดเลื่อนทุกครั้งที่หน้าต้นแบบโต) และอีกตัวคือ
...
/* ── ผิวที่สาม/สี่ของสองสเกลที่ใหญ่ที่สุด: ระยะห่าง กับ ขนาด ใน `style={{…}}` ──
   ...
   📉 704 → 689 (2026-09-23) — กล่อง glass-panel สีแดงของสหมิตรห้าจอ (รอบ FC · วัสดุ · PO · กระทบยอด · ตรวจธง) ย้ายไปใช้ StatusNotice กลาง
      ตามมติ "ไทยนำ + ดิบเป็นบรรทัดเล็ก" (padding/gap/marginBottom ดิบ 3 จุดต่อกล่อง) */
const RAW_SPACING_JSX_CAP = 689;
...
/* ขนาดในผิว style object — คนละเรื่องกับระยะห่าง จึงแยกเพดาน
   ...
   📉 309 → 303 (2026-09-23) — ความกว้างหกคอลัมน์ของตารางรายการใบเสนอราคา (# · จำนวน · ราคา/หน่วย · ส่วนลดรายการ ·
   จำนวนเงิน · ปุ่มลบ) ย้ายจาก `style={{ width }}` บนหัวตารางไปเป็นคลาสของ QuotationLineItems.module.css ตอนยกเซลล์
   ไปเป็น QuoteLineCells (ตัวเดียวกับบรรทัดโซนของใบสั่งขายย้อนหลัง — มติเจ้าของ 23/09) */
const RAW_SIZE_JSX_CAP = 303;
...
const RAW_TAILWIND_TYPE_CAP = 148;
...
/* ── น้ำหนักตัวอักษรที่ซ่อนอยู่ในกิ่งของ ternary (style object · 2026-09-02) ──
   ...
   `fontWeight` 282 จุด เป็นโทเคน --fw-* 274 · **หลุด 8 จุด และทั้ง 8 เป็น ternary**
     ...
     src/app/sales-planning/deals/[id]/page.js:160     s.state === "current" ? 800 : 650
