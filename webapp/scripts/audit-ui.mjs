...
/* ── น้ำหนักตัวอักษรที่ซ่อนอยู่ในกิ่งของ ternary (style object · 2026-09-02) ──
   ...
   `fontWeight` 282 จุด เป็นโทเคน --fw-* 274 · **หลุด 8 จุด และทั้ง 8 เป็น ternary**
     src/app/pm/tasks/page.js:1047                     isToday ? 700 : 500
...
/* ══ ด่านการเข้าถึงตัวแรกของ CI: onClick ที่คีย์บอร์ดกดไม่ได้ (2026-09-02) ══════
   ...
   *ทางหนึ่ง* ที่ทำให้ผ่าน 2.1.1 + 4.1.2 + 2.4.3 พร้อมกัน **เมื่อยืนกรานจะเก็บ
   ...
   ปิดกล่องอีก 4 (dismissScrimExempt) = **39** ซึ่งคือค่าของเพดานข้างล่าง
   ...
   **สิ่งที่จงใจ *ไม่* ยกเว้น ทั้งที่มีข้ออ้าง — กันด่านกลวง**
   ...
   ── เหลือเป็นความผิด 39 จุด · แยกกลุ่มตาม *ทางแก้* ไม่ใช่ตามไฟล์ ────────────
   1) **แถวเปิดรายละเอียด `<tr onClick>` = 14** (พาไปหน้าอื่น 8 · เปิดลิ้นชัก 6)
   ...
   2) **แถวสวิตช์พับ/กรอง `<tr onClick>` = 3** (ProjectDocumentView:596 ·
      SalesKpiDashboard:156 · PoVsFcView:87) ⇒ ต่างจากข้อ 1 ตรงที่ *สถานะ* ต้อง
      ประกาศได้ ใช้ <button aria-expanded> / <button aria-pressed> ห้ามใช้ DetailRow
   3) **การ์ด/เซลล์กริดที่ทั้งใบกดได้ `<div onClick>` = 17** (พาไปหน้าอื่น 9 ⇒ <Link>
      · สั่งงานในหน้า 8 ⇒ <button>) · 3 ใน 17 อยู่ในคอมโพเนนต์ร่วม
      (customers/[id]:83 RelationRow · ApprovalQueue.js:47 · DataList.js:91)
   4) **ข้อความในบรรทัดที่กดได้ `<span onClick>` = 4** — ทั้งสี่เป็นการเปลี่ยนหน้า
   ...
   5) **เซลล์เจาะข้อมูล `<td onClick>` = 1** (YearHeatmap.js:52) ⇒ <button> ในเซลล์
   ...
   ทางยกเว้นทั้งสามแบบถูกล็อกจำนวนไว้ใน keyboardClickable.test.mjs แล้ว
   (กันคลิกทะลุ 22 · ฉากหลังปิดกล่อง 4 · ครบเกณฑ์อยู่แล้ว 2) ทางเดียวที่เลขนี้ลงได้
   คือแก้โค้ดจริงให้ element กดด้วยคีย์บอร์ดได้
   📉 58 → 39 (2026-09-02): กลุ่ม `<th>` 19 จุดปิดจบ ดูบล็อกข้างบน */
const A11Y_KEYBOARD_CAP = 39;
