...
### ตัดฟีเจอร์ออก (ตาม feedback 2026-07-02)
- ❌ **เส้น LD 60/90 + เรื่อง Lead time** — เอาออกจากกริดกระทบยอด (import material/holidays/ldBorder/legend note ลบหมด)
- ❌ **เมนู "วัสดุ / Lead time"** — เอาออกจาก sidebar (โค้ด/หน้า `/sahamit/material` ยังอยู่ เปิดกลับได้ภายหลัง)
- ❌ **ระบบธง "ตรวจการเปลี่ยน FC" ทั้งชุด** — เอาเมนูออก + ลบ KPI/คิวในหน้าภาพรวม + ลบปุ่ม "ตั้งธงให้ตรวจ" + รายการธง + ลิงก์ /review ใน drill-down (คงป้ายคาดการณ์ ✨ ไว้). Backend คงไว้เฉยๆ (route `/review`, `POST /api/sahamit/flags`, detectFlags ตอน import) — ไม่มี UI ชี้ถึงแล้ว, เปิดกลับได้

### ปรับให้ Smart ขึ้น (ตาม feedback + screenshot 2026-07-02)
- 🔧 **คาดการณ์ยิงเกินไป (รอบแรกขึ้นทุกช่อง)** → `predictShifts` ตอนนี้ **ต้องมีประวัติการเลื่อนจริง** (`shiftSamplesForSku` ไม่ว่าง) ถึงจะเดา; SKU ที่ไม่เคยเลื่อน/รอบแรก = เงียบ (เทสต์คุมแล้ว)
- 🔧 **ชดเชยควรอัตโนมัติ+แค่ยืนยัน** → CoveragePanel: การ์ดเสนอเป็นตัวเอก, ฟอร์มกรอกเองซ่อนใต้ปุ่ม "+ กรอกชดเชยเอง", เพิ่มคำอธิบายเมื่อขาดแต่ไม่มี PO เกินให้ดึง (การ์ดจะโผล่ก็ต่อเมื่อมี PO ส่วนเกินในเดือนอื่น — ในภาพ PO=0 ทุกเดือนจึงยังไม่มี)
- 🔧 **ล็อก FC=PO อัตโนมัติ** → ช่อง status `match` โชว์ 🔒 เอง (กริด opacity จาง + tooltip "FC=PO ตกลงแล้ว (ล็อกอัตโนมัติ)"; drill-down เป็นป้ายเขียวแทนปุ่มกดล็อก) — ล็อกมือยังใช้ได้กับช่องที่ยังไม่มี PO
