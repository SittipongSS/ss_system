...
>
> สถานะ: **เสร็จสมบูรณ์** · ตรวจกับโค้ดเมื่อ 2026-08-06
>
> §2 "จุดตัดสินใจที่ล็อกแล้ว" และ §3 "กฎทอง 3 ข้อ" **ยังบังคับใช้อยู่** — อ่านสองหัวข้อนี้
> ก่อนแตะอะไรใน `src/lib/master/` · ตรวจแล้วยังตรงทุกข้อ: ไม่มีตาราง `people`,
> ไม่มีตาราง `brands` แยก, cap `master:manage` มีใน `permissions.js`, route
> `/api/product-types` มีจริง
>
> §10 checklist ทำครบแล้วยกเว้นข้อ 10 — `src/components/CustomerPicker.js` **ไม่ได้สร้าง**
> (ฟอร์มสินค้าใช้วิธีอื่นแทน)
...
## 13. รหัสลูกค้า (AR) กำกับชื่อกิจการ (2026-08-11 · IS-26080003)

ผู้ใช้แจ้งผ่านระบบแจ้งปัญหาเอง:
> *"อยากให้งานระบบใหม่มีการแสดง รหัสลูกค้า (ตัวเลข 3 หลัก) กำกับชื่อตรงกิจการด้วย จะได้สัมพันธ์กับรหัสกลิ่น, รหัส MU"*

ทะเบียนกลิ่นตั้งชื่อรหัสกลิ่นล้อกับรหัสลูกค้าอยู่แล้ว (`PF119…` ↔ `AR-119`) แต่หน้าจอโชว์แต่ชื่อบริษัท
⇒ ต้องเปิดทะเบียนลูกค้าอีกแท็บเพื่อแปลงกลับทุกครั้ง

**กติกาที่ล็อกไว้** — `src/lib/master/customerAr.js`

- ⚠️ **รหัสอ่านสดจากทะเบียนลูกค้าเสมอ ไม่ประทับลงแถวลูก** — แถวกลิ่น/คำร้อง/ดีล ประทับ `customerName` ไว้เป็น
  หลักฐาน ณ เวลาที่ผูก (ชื่อเปลี่ยนภายหลังได้) ส่วนรหัสเป็น **ตัวชี้กลับทะเบียน** จึงต้องเป็นค่าปัจจุบัน
  ไม่งั้นตามกลับไม่เจอเมื่อรหัสถูกแก้
- ⚠️ ลูกค้าที่ยังไม่ได้ออกรหัสต้อง **ไม่เข้าแผนที่** — ไม่งั้นได้ค่าว่างไปวาดต่อท้ายชื่อ
- ⚠️ `customerArIndex` รับ non-array ได้ — `cachedFetchJson` คืน `null` ตอน API พลาด แล้ว `for…of null`
  โยน TypeError กลางการเรนเดอร์ (จอขาวทั้งหน้า)
- แผนที่ `customerId → arCode` สร้าง **ครั้งเดียวต่อการเรนเดอร์** ไม่ใช่ค้นหาใหม่ทุกแถว (O(n²) ในตาราง)
- ⭐ **ในตาราง รหัสอยู่บรรทัดใต้ชื่อ ตัวเล็กจาง** (มติผู้ใช้ 2026-08-11) — ชื่อบริษัทไทยยาวจนรหัสที่ต่อท้าย
  ถูกดันไปคนละบรรทัดกับชื่ออยู่ดี และตกไปคนละตำแหน่งในแต่ละแถว ⇒ กวาดตาหาเป็นคอลัมน์ไม่ได้
  · บรรทัดของตัวเองทำให้รหัสทุกแถวอยู่ระดับเดียวกัน · การ์ดบนจอแคบยังเป็นข้อความบรรทัดเดียว (`ชื่อ · AR-xxx`)
  เพราะการ์ดไม่มีคอลัมน์ให้กวาดอยู่แล้ว

**ที่ลงแล้ว** — ทะเบียนกลิ่น (`/database/scents`) และทะเบียนสูตร (`/database/formulas`): ตาราง ·
การ์ดจอแคบ · **ค้นด้วยรหัส AR ได้** · ทะเบียนสูตรลงทั้งตารางหลักและตาราง "รอจัดระเบียบ"
(แถวรอจัดระเบียบมาจากทะเบียนสินค้าซึ่งมี `customerId` ติดมาอยู่แล้ว)
· หัวใบคำร้อง (`/requests/[id]`) ลงไปก่อนหน้าใน ม-98

**ที่ยังไม่ลง** — คิวคำร้อง · ดีล · ใบเสนอราคา · ใบสั่งขาย · โครงการ · งาน PM · ภาษีสรรพสามิต
(ผู้ใช้ขอ "ทุกที่ที่มีชื่อลูกค้า" — ทยอยลงทีละโมดูลเพราะข้ามเจ้าของโมดูล)

---

## 14. alias `/api/master/*` ต้องส่งต่อ method ครบ (2026-08-11 · IS-26080008)

ผู้ใช้แจ้ง: *"ใส่วันที่ของเอกสารหนังสือรับรองไม่ได้ค่ะ"*

`AttachmentsPanel` ยิง `PATCH /api/master/attachments/[id]` เพื่อบันทึกวันที่ออกเอกสาร แต่ไฟล์ alias ตัวนั้น
re-export ไว้แค่ `DELETE` ⇒ Next ตอบ **405 Method Not Allowed** ⇒ ช่องวันที่เด้งกลับค่าเดิมทุกครั้งที่กรอก
· หนังสือรับรองจึงค้างที่ป้าย "ยังไม่ระบุวันที่" ตลอดกาล และด่านอายุเอกสาร 6 เดือนใช้งานไม่ได้เลย

- ⚠️ **405 ไม่มี log ฝั่ง handler ให้เห็น** เพราะ handler ไม่เคยถูกเรียก — ต่างจาก 404/500 ที่ตามรอยได้
- ⚠️ proxy ยุบชื่อ alias ลงชื่อจริงอยู่แล้ว (`normalizeMaster`) ⇒ **ด่านสิทธิ์ผ่าน** ปัญหาอยู่ที่ชั้น route ล้วน ๆ
  ซึ่งทำให้ดูเหมือน "ระบบพัง" ทั้งที่ handler ถูกทุกบรรทัด (อาการเดียวกับบั๊ก `/api/tax/*` ที่บันทึกไว้ใน proxy.js)
- ✅ เทสต์ `src/app/api/master/aliasCoverage.test.mjs` ไล่ทุกไฟล์ alias เทียบกับปลายทาง — ปลายทางเพิ่ม method
  ใหม่แล้วลืมมาต่อที่ alias จะแดงทันที
