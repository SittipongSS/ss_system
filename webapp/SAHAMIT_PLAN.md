...
## 7. มติการเชื่อมกับระบบขาย (ล็อกแล้ว 2026-07-19) — BUILD แล้ว 2026-07-19

> **สถานะ build:** settle-deal เปลี่ยนเป็น "ยืนยันดีล+ผูกโครงการ+ออก QT",
> create-project เหลือสร้างโครงการอย่างเดียว. ขั้น Won (แนบไฟล์ PO + เลือกเดือน)
> และ SO ใช้ route มาตรฐานเดิม ไม่มีโค้ดใหม่. core สร้าง QT แยกเป็น lib ใช้ร่วม:
> `src/lib/sales/createQuotationDraft.js`.
>
> **มติเพิ่ม 2026-07-19 — 1 PO = 1 ดีลรวม (เหตุผล: การเก็บเงินผูกกับ PO ทั้งใบ):**
> ยืนยันหนึ่งครั้ง → สร้างดีลรวม `SHM_PO <เลขที่>` (source `sahamit-po` — กติกา
> FC-อัพเดทเคลียร์ดีลไม่แตะ) + ออก **QT ใบเดียวหลายบรรทัด** ตาม PO (ราคา master)
> → ลูกค้าเซ็นใบเดียว งวดชำระชุดเดียว SO ใบเดียวยอดตรง PO. ดีล FC ต้นทางย้าย
> junction เข้าดีลรวมแล้วปิดด้วยธง `sahamitMergedIntoDealId` (กรองออกจากสถิติแพ้ได้
> เหมือนธง superseded); partial + เลือก "แบ่ง" → ย้ายเฉพาะส่วนที่ PO ครอบ ต้นทาง
> เปิดต่อรอ PO ถัดไป. เจ้าของดีลรวม = เจ้าของดีล FC ตัวแรกที่จับคู่ (fallback ผู้กด).
> FC-vs-PO รายสินค้ายังวัดได้จาก junction ที่ย้ายมาครบ; trade-off ที่ยอมรับ:
> จำนวนดีล Won นับต่อ PO ไม่ใช่ต่อสินค้า (ยอดเงินเท่าเดิม).

**ปัญหาที่พบจาก review:** เส้นทางปิด Won ของสหมิตร (settle-deal / create-project → `markWon` เขียน `stage='won'` ตรง) เป็นทางลัดคู่ขนานที่ (ก) ไม่ผ่าน guard ของสายขาย (QT อนุมัติ/หลักฐาน/สองคน) (ข) ชนกับ trigger 0110 ที่บังคับ Actual = ผลรวม approved SO เท่านั้น — สหมิตรไม่สร้าง SO ทำให้ **ดีล Won จากสหมิตรมี Actual = 0 และ wonMonth = null บนทุกแดชบอร์ด** (ค) ย้อน Won ไม่ได้เพราะ reversal 0116 ต้องยกเลิก approved SO

**มติ: สายสหมิตรเลิกปิด Won ทางลัด — วิ่งท่อขายเต็มรูปแบบ**

| ขั้น | กติกา |
|---|---|
| 1. PO เข้ามา | ผู้ใช้**ยืนยัน mapping** ว่าเชื่อมดีลไหน + โครงการอะไร ก่อนเสมอ |
| 2. ออก QT | ระบบสร้าง QT จากรายการใน PO — **ราคาล็อกจาก master** เหมือน QT ปกติ + เข้า**อนุมัติเจ้าของเซ็น**ตามปกติ |
| 3. ปิด Won | ผ่านทางมาตรฐาน (accept) — **แนบไฟล์ PO เป็นหลักฐาน** (private bucket) + **เลือกเดือนที่ยอดตก** (PO จริงอาจไม่ตรงเดือนคาดจาก FC) |
| 4. ออก SO | **เข้าคิวอนุมัติสองคนปกติ** → approved แล้ว Actual ถึงนับ (สอดคล้อง trigger 0110 โดยไม่ต้องแก้ trigger) |

มติประกอบ:
1. **create-project แยกหน้าที่** — สร้างโครงการ PM อย่างเดียว (สิทธิ์ `pm:edit` ถูกแล้ว) **เลิกปิด Won พ่วง**; Won มีทางเดียวผ่านท่อ QT→SO
   - ⚠️ **ต้องระบุผู้ดูแล (AE) ทุกครั้ง** (2026-08-14) — เดิมตรึง `aeOwner`/`ownerId` เป็น**คนกดปุ่ม** และทีมถอยไปค่าคงที่ `'KA'` ⇒ Admin/หัวหน้าฝ่ายขายกดแล้วโครงการตกเป็นของคนที่ไม่ใช่ AE (ชื่อผิดบนเอกสาร ISO ด้วย) · ตอนนี้โมดัลยืนยันมีช่องผู้ดูแล (ae/senior_ae ล็อกชื่อตัวเอง) และ route ตรวจด้วย `resolveProjectAeOwner` ตัวเดียวกับโครงการฝั่งขาย (mig 0253)
2. **PO ครอบคลุมดีลบางส่วน** — เลิก split ดีลลูกอัตโนมัติ ให้**ผู้ใช้เลือกตอน settle** ว่าจะ split หรือปิดทั้งดีล
3. **ทางย้อน:** settle ผิด = ยกเลิก SO ผ่าน reversal ปกติ (0116) — ไม่ต้องมีกลไกพิเศษ
4. **ของเก่า:** ยังไม่มีดีลสหมิตรที่ Won ค้างในระบบ → ไม่ต้อง backfill
5. ⚠ **ops:** ราคาสินค้าสหมิตรใน master ต้องตั้งให้ตรงราคาตกลงจริง — ระบบยึด master เสมอ ถ้าไม่ตรง ยอด SO จะไม่เท่ายอดบน PO

> ผลพลอยได้: ประเด็นสิทธิ์ (settle ไม่เช็ค `inSalesEditScope` รายดีล) หายไปเองเพราะทุกขั้นใช้ route มาตรฐานที่เช็คครบอยู่แล้ว · ฝั่ง FC→ดีล (sync-sales-planning / create-sales-deal) ไม่เปลี่ยน — review แล้วปลอดภัย (เคารพ forecastMonth, ข้ามดีลปิดแล้ว)
