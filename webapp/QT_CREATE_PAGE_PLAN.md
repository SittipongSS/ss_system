...
## 6) มติผู้ใช้ (2026-07-18) — การอนุมัติใบเสนอราคาในอนาคต

> สถานะ: **มติวางแผน — ยังไม่ implement** (บันทึกไว้กันหลงทิศ)

1. **การอนุมัติใบเสนอราคาจะกลับมา** แต่เป็น**คนละโมเดล**กับที่ถูกถอดออกใน mig 0100
   (`0100_quotation_no_approval.sql`) — โมเดลใหม่คือ **"การเซ็นรับรองโดยเจ้าของ"**
   ไม่ใช่ด่านอนุมัติตามเงื่อนไข/ยอด (แบบ 0070/0098 เดิม)
2. **ใครสร้าง / ใครอนุมัติ**: AC / AE / Senior AE สร้างใบได้ทุกคน แต่ **AE/Senior AE
   เจ้าของโครงการ/ลูกค้า ต้องอนุมัติก่อนใบจึงมีผล**
3. **เหตุผล**: ช่องเซ็นบนเอกสาร FM-SA-01 — **ผู้เสนอราคา = ผู้สร้างใบ**,
   **ผู้อนุมัติ = AE เจ้าของโครงการ/ลูกค้า** — โมเดลนี้สะท้อนช่องเซ็นจริงบนฟอร์ม
4. **โครงที่มีแล้ว** (ต่อยอดได้เลย): ฟิลด์บุคคล `aeOwner` / `preparedBy` / `aeSupervisor`
   ใน `quotations.metadata` ผ่าน `validateQuotationPeople`
   (`src/lib/sales/quotationPeople.js`)
5. **คำถามออกแบบที่ยังค้าง**:
   - เจ้าของสร้างใบเอง → เซ็นสองช่องเอง หรือดัน Senior ขึ้นเป็นผู้อนุมัติ?
   - แก้ใบหลังอนุมัติแล้ว ต้องขออนุมัติใหม่หรือไม่ (แนะนำ: **ใช่**)
6. **กำกับ**: **ห้าม resurrect โค้ด approval ยุค mig 0098/0100** — เขียนโมเดลใหม่
   ตามข้อ 1–4 เท่านั้น
