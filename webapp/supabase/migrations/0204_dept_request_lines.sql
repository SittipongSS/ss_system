...
--
--  ⚠️⚠️ **รันบน prod ไปแล้วในชื่อ `0202_dept_request_lines.sql` (2026-08-04)**
--  ไฟล์ถูกเลื่อนเลขเป็น 0204 ทีหลัง เพราะ 0202 ชนกับ migration ที่อยู่ลูกค้าที่
--  merge ไล่หลังกันมา (commit 9bc38d4e) — **ห้ามรันซ้ำบน prod**
--    · รันซ้ำจะ error ที่ `RENAME COLUMN "priceStatus"` (คอลัมน์ไม่มีแล้ว) ซึ่ง
--      ล้มแบบปลอดภัย ไม่ทำข้อมูลเสีย แต่จะดูเหมือนระบบพัง
--    · สภาพแวดล้อมใหม่ที่ยังไม่เคยรัน ให้รันตามปกติทั้งไฟล์
--
