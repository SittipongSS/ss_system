-- 0172 - ย้ายฟีดความเคลื่อนไหวของดีลมาเธรดกลาง (entity_updates, mig 0163)
--
-- ⚠️ ไฟล์นี้ย้ายเลขมาแล้ว **สองรอบ**: `0169_…` → `0170_…` → `0172_…`
--    รอบแรกชนกับ `0169_sales_order_reissue_after_cancel.sql`, รอบสองชนกับ
--    `0170_deal_stage_order_swap.sql` (การขยับรอบแรกไม่ได้ rebase ก่อน จึงย้ายไปทับ
--    เลขที่มีเจ้าของอยู่แล้วพอดี) — โหมดพังประจำ: สอง PR แยกกิ่งจากจุดเดียวกัน
--    แล้วต่างคนต่างจองเลขถัดไป · ขยับเลขตัวที่ merge ทีหลังตามกฎเดิม
--    **ไม่ต้องรันซ้ำถ้ารันในชื่อเดิมไปแล้ว** และถึงรันซ้ำก็ปลอดภัย: idempotent
--    ทั้งไฟล์ (`WHERE NOT EXISTS`) และไม่มี DDL เลย
--    ยืนยันบน prod 2026-07-28: `sales_deal_activities` = 0 แถว และ `entity_updates`
--    ที่ "entityType" = 'deal' = 0 แถว → ไฟล์นี้เป็น no-op บน prod ไม่ว่าจะรันหรือไม่
--
