-- ============================================================
--  Migration 0239: วันเริ่มจริงของขั้นตอนไทม์ไลน์ (มติผู้ใช้ 2026-08-12)
--
--  ✅ **รันบนฐานจริงไปแล้ว 2026-08-12** (ตอนนั้นไฟล์ยังชื่อ 0238) — ยืนยันด้วย constraint
--     `project_tasks_actual_order` ขึ้นครบ · ไม่ต้องรันซ้ำ และถ้ารันซ้ำก็ไม่พัง
--     (`ADD COLUMN IF NOT EXISTS` + `IF NOT EXISTS` ของ constraint + `CREATE OR REPLACE`)
--
--  🔁 เลขไฟล์เลื่อนสองรอบระหว่างรอ merge — **เนื้อ SQL ไม่เปลี่ยนสักตัวอักษร** ตามกติกาโปรเจกต์:
--     จอง 0236 → ชนกับ 0236_drop_chat_webhooks + 0237_master_code_atomic_insert (#1195)
--     → เลื่อนเป็น 0238 → ชนกับ 0238_quotation_doc_language (#1196) → เลื่อนเป็น 0239
--
...
--
--  ⚠️ เลข 0239 จองต่อจาก 0238 ตามกติกาโปรเจกต์ — ถ้าสายอื่น merge ก่อนแล้วกินเลขนี้ไป
