...
--  ⚠️ **ยังรันไม่ได้ ณ 2026-08-30 — พักไว้รอมติเรื่องข้อมูล** ทั้งสองคู่ลบทิ้งไม่ได้
--     เพราะแต่ละใบถือเนื้อในของตัวเอง (ที่อยู่/ผู้ติดต่อ/แบรนด์) แม้ใบส่วนเกินจะไม่มี
--     เอกสารอ้างถึงเลยก็ตาม · ทางที่เหลือมีสองทาง ซึ่งยังไม่ได้เลือก:
--       ก. ย้ายเนื้อในเข้าใบหลัก แล้วพักใบส่วนเกิน (isActive=false) — ถ้าเลือกทางนี้
--          index ต้องเป็น partial `where "isActive" is distinct from false` และด่าน
--          ฝั่งแอปต้องเช็คซ้ำตอน "เปิดใช้กลับ" ด้วย ไม่งั้นย้อนกลับมาชนกันได้
--       ข. เก็บทั้งสองใบไว้ใช้งานจริง — แปลว่าไม่มี unique ระดับ DB ได้เลย
--          ด่านเหลือแค่ฝั่งแอป (กันใบใหม่ได้ แต่ race สองคำขอพร้อมกันไม่มีอะไรกั้น)
--  ⚠️ ระหว่างนี้แถวยุคเก่ายัง **แก้ได้ตามปกติ** — ด่านฝั่งแอปทำงานเฉพาะตอนคีย์ขยับ
--  ⚠️ ก่อนรัน ต้องได้ 0 แถวจาก query ข้างล่าง
--
