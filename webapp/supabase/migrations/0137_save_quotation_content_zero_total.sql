-- 0137 - save_quotation_content: เลิกบล็อกใบยอด 0 บาทตอนสถานะ 'sent'.
--
-- มติผู้ใช้ 2026-07-18 (ดู lib/documentWorkflow.js): ใบยอด 0 บาท "ส่ง" ลูกค้าได้
-- (บางใบลดราคาจนเหลือ 0) — เฉพาะ accept/Won เท่านั้นที่ยังบังคับยอด > 0 เพราะการ
-- รับใบจะเขียนทับมูลค่าดีลด้วยยอดนี้. ฝั่ง API (send + คิวอนุมัติ) อนุญาต 0 บาทตาม
-- มติแล้ว แต่ RPC ตัวนี้ยังมี guard `quotation_total_zero` (คัดลอกต่อกันมาจาก 0098
-- ซึ่งเขียนก่อนมติ) → เปลี่ยนใบ 0 บาทที่อนุมัติแล้วเป็น sent หรือเซฟใบ sent 0 บาท
-- ซ้ำ พังเป็น 500 ดิบ ๆ ขัดกับกติกาที่โค้ดฝั่งแอปตกลงไว้.
--
-- คัดลอกทั้งฟังก์ชันจากนิยามล่าสุด (0124 — ไม่มี migration หลังจากนั้นแตะฟังก์ชันนี้)
-- แล้วถอดเฉพาะบรรทัด quotation_total_zero; guard อื่นตอนลงสถานะ sent
-- (quotation_lines_required / quotation_approval_required / quotation_approval_stale)
-- คงเดิมทุกบรรทัด. Idempotent — รันซ้ำได้.
