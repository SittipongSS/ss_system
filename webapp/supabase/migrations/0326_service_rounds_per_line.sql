...
--
--  🛠 **มติเปลี่ยนหลังไฟล์นี้ถูกรันแล้ว (2026-08-31 รอบสอง): กรอกที่ "ใบสั่งขาย"**
--     ผู้ใช้ขอย้ายทางเข้ามาที่ใบสั่งขายตรง ๆ ⇒ ช่องกรอกที่ใบเสนอราคาถูกถอดออก
--     · **SQL ในไฟล์นี้ไม่ถูกแก้** (รันไปแล้ว) — คอลัมน์ที่ quotation_lines จึงค้างไว้
--       โดยไม่มีใครเขียน และเส้นก๊อป QT→SO ก็ไม่มีค่าให้ก๊อป (ไม่เสียหาย)
--     · ทอดที่ยังสำคัญจริงเหลือทอดเดียว: SO → SO Rev. (revise_approved_sales_order_atomic)
--     · ด่าน/ตัวตรวจของทางเข้าใหม่อยู่ที่ src/lib/sales/serviceRoundsEntry.js
--     · ถ้าจะเก็บกวาด ให้ทำ migration ใหม่ถอนคอลัมน์ของ quotation_lines ทีหลัง
--
--  ── ต่อไปนี้คือบันทึกของมติเดิม ณ ตอนที่รันไฟล์นี้ ─────────────────────────
