...
| `A11Y_KEYBOARD_CAP` | 12 | **ด่าน accessibility ตัวแรกของ CI** (WCAG 2.1.1 Keyboard ระดับ A) — `onClick` ที่ทำงานจริงบนแท็กที่คีย์บอร์ดเข้าไม่ถึง · แยกตามแท็กเพราะ **ทางแก้คนละท่า**: `tr` 7 · `span` 4 (ทั้งหมดเปลี่ยนหน้า → `<Link>`) · `td` 1 (`YearHeatmap` → `<button>` ในเซลล์) · ยกเว้นอัตโนมัติแล้ว 28 จุด (กันคลิกทะลุ 21 · ฉากหลังปิดกล่อง 4 · ทางลัดเมาส์บนแถว 1 · ทางลัดเมาส์บนการ์ด 1 · ครบเกณฑ์อยู่แล้ว 1) · 🔒 **เลขรายแท็กถูกล็อกแยกอีกชั้นใน `keyboardClickable.test.mjs` (`A11Y_TAG_CAPS`)** — ยอดรวมอย่างเดียวปิดช่อง *สลับตัว* ไม่ได้ (แก้ `div` ไป 1 แล้วเขียน `<tr onClick>` ดิบใหม่ 1 ยอดเท่าเดิม) และกลุ่ม `tr` คือกลุ่มที่ย้อนกลับง่ายที่สุด · ⚠️ ห้ามลดเลขด้วย `role="button"` บน `tr`/`th`/`td` — ทับ role ของตารางทิ้ง (ตก 1.3.1) และห้ามลดด้วยการ **เติมทางยกเว้น** (จำนวนทั้งสี่แบบถูกล็อกไว้ใน `keyboardClickable.test.mjs`) · 📉 **58 → 39 (2026-09-02 รอบ `th`)**: กลุ่มหัวตารางเรียงลำดับ 19 จุดปิดจบด้วยการแก้ `SortTh` ตัวเดียว · 📉 **39 → 29 (2026-09-02 รอบแถวตาราง)**: กลุ่ม `tr` 17 → 7 ด้วย `DetailRow` + `ROW_MIRROR` — อีกรอบที่ทางแก้รวมอยู่ที่ primitive ตัวเดียว · ⚖️ **`DetailRow` เองไม่ได้ทำให้เลขลด** มันย้ายจากช่อง "ครบเกณฑ์อยู่แล้ว" (2 → 1) ไปช่อง "ทางลัดเมาส์บนแถว" (ดิบขึ้น 1 หักคืน 1) · ที่ลด 10 คือ *ผู้เรียกและแถวอื่น* ที่ได้ `<Link>`/`<button>` ในเซลล์จริงในคอมมิตเดียวกัน · 🪤 **7 ที่เหลือของกลุ่ม `tr` ไม่ใช่แถวที่คีย์บอร์ดกดไม่ได้** — ทุกตัวมีทางเข้าในเซลล์แล้ว แต่เป็น `<tr>` ดิบนอก `DetailRow` ที่ `ROW_MIRROR` ตรวจไม่ถึง ⇒ ไม่มีอะไรยืนยันว่าลิงก์นั้น ยังตรงปลายทางในวันข้างหน้า · ปิดเลขด้วยการ **ย้ายเข้า `DetailRow`** เท่านั้น ห้ามปิดด้วยการผ่อนด่านเป็น "มีอะไรโฟกัสได้ข้างในก็พอ" (แถวที่มีแค่ปุ่มลบก็เข้าเงื่อนไขนั้น) · 📉 **29 → 26 (2026-09-02 รอบการ์ด เฟสที่ 1: คอมโพเนนต์ร่วม)**: กลุ่ม `div` 17 → 14 — สามจุดที่ปิดเป็น *คอมโพเนนต์ที่ใช้ร่วม* ทั้งหมด (`excise/DataList` การ์ดจอแนวตั้ง ห่อทั้งใบด้วย `<Link>` โดยผู้เรียกไม่ต้องแก้สักบรรทัด · `ui/ApprovalQueue` ห่อทั้งแถวไม่ได้เพราะมีปุ่มท้ายแถว ทุกโหมด จึงย้ายทางเข้าไปเป็น `<Link>` ที่บล็อกข้อความ แล้วเปลี่ยนสัญญา `onOpen` → `rowHref` ที่ผู้เรียก 5 หน้า · `RelationRow` ของหน้ารายละเอียดลูกค้า) · ⚖️ **`ClickableCard` ไม่ได้ทำให้เลขขยับ** — เพิ่ม `<div onClick>` ดิบมา 1 แล้วถูกหักคืนด้วย `cardShortcutExempt` พอดี (รูปเดียวกับที่ `DetailRow` ทำในรอบแถว) · 🪤 `counts.stopper` รูด 22 → 21 พร้อมกัน เพราะตัวห่อ `stopPropagation` ของแถวคิวกลายเป็น โค้ดตายเมื่อแถวไม่มีตัวรับคลิกแล้ว · 📉 **26 → 12 (2026-09-02 รอบการ์ด เฟสที่ 2: หน้ารายหน้า)**: กลุ่ม `div` 14 → **0 ปิดจบ** — 14 จุดใน 7 ไฟล์ แยกท่าตามเนื้อในของการ์ดจริง ๆ: **ท่า A** (customers/[id] การ์ดสินค้า/ทะเบียนภาษี · products/[id] สามบล็อก · pm/tasks ชิปปฏิทิน) · **ท่า B** (customers/[id] การ์ดใบสั่งซื้อ · sahamit/reconcile ช่องกริดสองมุมมอง — ทั้งสามเปิดโมดัลในหน้าเดิม จึงไม่มี URL ให้ลิงก์) · **ท่า C** (ทะเบียนลูกค้า · ทะเบียนสินค้า · การ์ดคิวงาน pm/tasks — มีปุ่ม/`<select>`/เมนูอยู่ข้างใน) · และ 1 จุดที่ **ไม่ใช่ทั้งสามท่า** คือ mgmt/page.js ที่ทุกแถวชี้ปลายทางเดียวกัน ⇒ ลิงก์เดียวที่หัวกล่อง แถวกลับเป็นข้อความ (2.4.4) · ⚠️ **กลุ่มนี้ไม่มี primitive ตัวเดียวปิดให้ได้** ต่างจากรอบ `th`/`tr` — เนื้อในการ์ดแต่ละใบไม่เหมือนกัน · ⏭️ **ที่เหลือ 12 จุดก็เช่นกัน**: `tr` 7 ต้องย้ายเข้า `DetailRow` ทีละแถว · `span` 4 เปลี่ยนเป็น `<Link>` ได้ตรง ๆ · `td` 1 (`YearHeatmap`) ต้องมี `<button>` ในเซลล์ |
...
| `CARD_MIRROR` | **0** (hard-zero · ไม่มีเพดานให้ไต่) | ฝาแฝดฝั่งการ์ดของ `ROW_MIRROR` (2026-09-02) — ทุกที่เรียก `<ClickableCard href={X}>` ต้องมี `<Link href={X}>` **ข้อความนิพจน์เดียวกันเป๊ะ** อยู่ในการ์ด (ปกติที่หัวการ์ด) · เป็น **เงื่อนไข** ที่ทำให้ `<div onClick>` ของ `ClickableCard` ถูกหักออกจาก `A11Y_KEYBOARD_CAP` ได้ — ผ่อนเมื่อไหร่ ต้องบวกเพดานคีย์บอร์ดคืน 1 · 🪤 การ์ดที่มีแค่ปุ่ม "ลบ" หรือลิงก์ดินสอ `?edit=1` ยัง **ตก** (เทียบ *ปลายทาง* ไม่ใช่ "มีอะไรโฟกัสได้ก็พอ") · 🪤 `<ClickableCard …/>` แบบ self-closing (ส่งเนื้อผ่าน props) **ตกเสมอ** เพราะหาแท็กปิดไม่เจอ → body ว่าง ⇒ ท่าที่ซ่อนลิงก์ไว้ใน props ถูกปิดโดยอัตโนมัติ ไม่ต้องเขียนกฎเพิ่ม · ⚠️ **ตรวจไม่ได้ชุดเดียวกับ `ROW_MIRROR` ทั้ง 5 ข้อ** บวกอีก 1 ข้อเฉพาะการ์ด: `card(row)` ที่ผู้เรียกส่งเป็น render prop (ตัวสแกนอ่าน JSX ทีละไฟล์ จึงมองไม่เห็น) — รูนั้นปิดด้วย `src/components/excise/dataListCardCallers.test.mjs` ที่ **คำนวณรายชื่อผู้เรียกเอง** ไม่ใช่ทะเบียนที่พิมพ์มือ · 📌 ที่เรียกวันนี้ **4 จุดใน 3 ไฟล์** (ทะเบียนลูกค้า · ทะเบียนสินค้า · `pm/tasks` ซึ่งเรียกสองที่คือ `miniCard` กับ `taskRow`) — ทุกใบมีปุ่มของตัวเองอยู่ข้างในจึงห่อทั้งใบไม่ได้ · ⚠️ **ที่เรียกต้องไม่เขียน `<div onClick>` ของตัวเองขึ้นมาใหม่** — `cardShortcutExempt` ยกเว้นให้ `<div onClick>` **ในตัว primitive จุดเดียว** ไม่ได้ยกให้ไฟล์ผู้เรียก |
