...
- **ข้อ A · เรนเดอร์จริง** — อ่านเฉพาะ binding ที่มาจาก `import … from "@/components/ui/…"`
  ...
  **คอมเมนต์ · สตริง · text node ของ JSX** ทิ้งก่อนหาการใช้งาน — `<Binding` หรือ
  ...
  ⚠️ วัดที่ **ระดับโมดูล ไม่ใช่ระดับชื่อ export** — `Table` ใช้ผ่าน `TableShell`,
...
| `RAW_SIZE_JSX_CAP` | 310 | ขนาดในผิว `style={{…}}` · **ไม่ใช่หนี้รอแปลงเป็นโทเคน** — สเกลนี้ไม่มีบันไดตัวเลข มีแต่ชื่อตามบทบาท ⇒ เป็น **สายสะดุด**: ขนาดของ control ตัดสินที่ primitive แล้ว (`--ctl-h`) เขียนซ้ำที่ปลายทางคือทับของที่ตัดสินไปแล้ว · 📉 **321 → 310 (2026-09-15)** ช่องค้นหาของหน้ารายการเลิกตรึงความกว้างเอง (ย้ายเข้า ListPanel toolbar) |
