  ...
  return (
    <>
      ...
      {/* ── สถานที่ + พื้นที่ที่ต้องประเมิน (mig 0314) ──────────────────────
          ⭐ **อยู่โซน ② เพราะมันคือ "ขออะไร"** (มติผู้ใช้ 2026-09-02) — ใบนี้ขอให้ไป
             วัด *ที่นี่* *พื้นที่เหล่านี้* ⇒ ไซต์กับพื้นที่คือตัวตนของสิ่งที่ขอ ไม่ใช่บริบท
             ของงาน · แถวพื้นที่ยังเป็น "รายการที่ขอ" ของหัวข้อนี้ด้วย (`lineNoun: 'พื้นที่'`)
             ซึ่งทุกหัวข้ออื่นวางไว้โซนนี้อยู่แล้ว · เดิมอยู่โซน ① ติดกับดีล เพราะลิสต์
             สถานที่กรองด้วยลูกค้าของดีล — แต่นั่นเป็นเหตุผลเรื่อง *ลำดับการกรอก*
             ไม่ใช่เรื่อง *ความหมายของช่อง* และช่องมีข้อความบอกอยู่แล้วว่าให้เลือกดีลก่อน
          ⚠️ ย้ายที่นี่ต้องย้าย `tab` ของช่องบังคับใน `lib/requests/formTabs.js` ด้วย —
             เกจ "ยังขาด: สถานที่…" พาไปเปิดแท็บที่ระบุไว้ตรงนั้น (กับดักเดิมของยอดวางบิล) */}
      {needsSite && (
        <SurveySiteFields
