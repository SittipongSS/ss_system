 ...
 * @param accessKnown ไซต์ที่ส่งมามีช่วงเวลาที่ให้เข้าครบไหม — หน้าจัดคิว = true · หน้าใบ = false
 *                    (`surveySite` ไม่ได้ select ช่วงเวลามา · D2) ⇒ ด่าน ④ "หน้านี้ไม่เห็นช่วงเข้าไซต์" ไม่เดาว่าผ่าน
 *                    ⚠️ ข้อจำกัดที่รู้แล้ว (รีวิว UAT 24/09): ให้หน้าใบเห็นช่วงเวลา/ภาระไซต์ = เปลี่ยนการอ่าน API
 *                       (select ของ `surveySite`) — รอมติเจ้าของ · ผลของการกดยังบอกได้แน่นอนทั้งสองหน้า
 * @param siteLoad    ภาระของไซต์ `{ assets, packs }` (หน้าจัดคิว) — ไม่ส่ง = ไม่มีตัวเลขจุด (ไม่เดา)
