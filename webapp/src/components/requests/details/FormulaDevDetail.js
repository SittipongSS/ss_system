...
//     แทน · แถวงานของ NPD ระบบแตกจากแถวสินค้าใน PDR ตอนรับเรื่อง (ม-144) แล้วเดินก้าวรายแถว
//     ชุดเดียวกับ Standard ⇒ จอนี้วางแบบฟอร์ม (ถ้ามี) + กระดานรายแถว (เมื่อมีแถว)
...
export default function FormulaDevDetail({
  ...
  rowStep, onReload, bulkReady, today = null, due = null,
}) {
  ...
  const renderStep = rowStep
    ...
    : null;
  /* ⭐ **ใบ NPD มีแถวงานตั้งแต่รับเรื่อง** (ม-144) — ระบบแตกแถวสินค้าใน PDR เป็นแถวส่งสูตร ⇒ กระดาน
     สรุปกับปุ่มส่งงานต้องขึ้นเหมือน Standard · ก่อนรับเรื่องยังไม่มีแถว ⇒ ซ่อนการ์ดทั้งใบ
     🐞 ไม่ซ่อนตอน 0 แถว = การ์ด "สรุปทั้งใบ" ที่มีแต่หัว (FormulaDevBoard คืน null) อ่านเหมือนโหลดไม่เสร็จ
     🐞 รอบแรก (ม-141) ซ่อนตาม `requestUsesPdr` ⇒ แถวที่ระบบสร้างจะมองไม่เห็นและกดส่งสูตรไม่ได้เลย
     ⚠️ ถามจากแถวจริง + ทะเบียน ไม่ใช่เทียบชื่อรูปแบบในจอ */
  const usesPdr = requestUsesPdr(request);
  const showBoard = !usesPdr || board.length > 0;
  return (
    <>
