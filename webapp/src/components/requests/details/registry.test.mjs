...
test('🔴 เปลือกส่งก้อนของทุกหัวข้อไปให้ครบ — ห้ามเดาจากข้อมูลว่าใบนี้เป็นหัวข้อไหน', () => {
  ...
  /* ⚠️ `formulaTotals=` / `docTotals=` ถูกถอดจากรายการนี้ (มติผู้ใช้ 2026-08-25) —
     การ์ดขวาเป็นทรงเดียวกันทุกหัวข้อแล้ว และนับที่ `lib/requests/panelSummary.js`
     จาก `request.items` โดยตรง ⇒ สองก้อนนั้นไม่มีใครกิน · **กฎที่เทสต์นี้คุมยังเดิม**:
     เปลือกต้องส่งก้อนของหัวข้อไปให้ตรงตัว ห้ามเดาจากข้อมูลว่าใบนี้เป็นหัวข้อไหน */
  for (const prop of ['formulaBoard=', 'docBoard=', 'board=', 'briefSummary=']) {
    assert.ok(PAGE.includes(prop), `เปลือกต้องส่ง ${prop}`);
  }
  /* ⭐ "วันนี้" มาจากเปลือกตัวเดียว (มติผู้ใช้ 2026-08-25) — คอลัมน์ "ค้างมา" ของ
     สามตารางต้องนับจากวันเดียวกัน · ปล่อยให้ตารางอ่านนาฬิกาเองเมื่อไร จะได้เลข
     คนละวันในใบเดียวกันตอนเที่ยงคืน และ hydration mismatch ตอนเรนเดอร์ฝั่ง server */
  assert.ok(PAGE.includes('today={today}'), 'เปลือกต้องส่ง today ลงไป ไม่ให้ตารางอ่านนาฬิกาเอง');
  assert.ok(!/useState\(\(\) => businessDate\(\)\)/.test(PAGE),
    'ห้ามอ่านนาฬิกาตอนเรนเดอร์ — ต้องจับใน effect');
});
