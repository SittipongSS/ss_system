...
/* 📌 ADR 0016: ปุ่มลัด/การ์ด "ทำงานต่อ" ของหน้าแรกเดิมถูกลบทั้งชุด — กฎเป้าสัมผัส
   ย้ายไปอยู่กับแถวเมนูและชิปสารบัญของหน้าใหม่ ซึ่งเป็นสิ่งที่นิ้วต้องกดจริงบนหน้านั้น */
test("หน้าแรกบนจอแคบใช้เป้าขนาดจอสัมผัสที่มีชื่อ ไม่ใช่ตัวเลขลอย ๆ", () => {
  const sheet = stripComments(fs.readFileSync(path.join(process.cwd(), "src", "components", "home", "SystemMenuSheet.module.css"), "utf8"));
  assert.match(sheet, /--row:\s*var\(--ctl-h-touch\)/,
    "แถวเมนูบนจอแคบต้องสูงเท่าเป้าจอสัมผัส ไม่งั้นหน้าแรกกดยากกว่าปุ่มในฟอร์ม");
  assert.match(sheet, /\.chip \{[^}]*height:\s*var\(--ctl-h-touch\)/,
    "ชิปสารบัญเป็นเป้ากดบนมือถือโดยตรง");
  assert.doesNotMatch(sheet, /min-height:\s*40px/, "40px ลอย ๆ ห้ามกลับมา");
});
...
test("ค่าจอสัมผัสถูกใช้เฉพาะกับตัวควบคุมจริง", () => {
  ...
  const TOUCH_TARGETS_NOT_BTN = [
    ...
    ".topnav-skip", // ลิงก์ข้ามไปเนื้อหาบนหน้าแรก (ADR 0016) — เป้าเดียวที่คีย์บอร์ด/สวิตช์ใช้ข้ามแผงทั้งหน้า
    "input.premium-input.touch", // ช่องกรอกของจอหน้างาน (<Input touch>) — ช่างแตะช่องด้วยนิ้วบนมือถือ/แท็บเล็ต
  ];
