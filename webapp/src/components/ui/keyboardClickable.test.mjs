...
const SWALLOWED_ONCLICK = [
  /* สองจุดนี้โดนกลืนเพราะสตริง `accept="image/*"` ของ <input type="file"> ในไฟล์เดียวกัน
     เปิด "คอมเมนต์ปลอม" ให้ตัวล้างคอมเมนต์ (มันไม่รู้จักสตริง) แล้วตัวปิดคอมเมนต์ของ
     คอมเมนต์ JSX ถัดลงไปปิดให้ ⇒ สองปุ่มที่อยู่ระหว่างนั้นหายจากสายตาของด่าน
     ...
     ⚠️ เลขขยับ 428/441 → 434/447 ตอนเติมคอมเมนต์เหนือ effect (2026-09-09) · แล้วขยับ
        434/447 → 544/557 ตอนแผ่นปิดงานรู้จักนัดถอนเครื่องและสวิตช์แจ้งชำรุด (2026-09-11)
        — อ่านด้วยมือซ้ำแล้วทุกรอบ ยังเป็น <Button> ทั้งคู่ (ปุ่ม "ถ่ายรูป" กับ "เซ็นใหม่")
        · 544/557 → 550/563 ตอนแถบเวลาของแผ่นเลิกโชว์เวลาเสร็จเก่าของนัดที่ถูกเปิดกลับ (2026-09-24)
        · 550/563 → 591/606 ตอนลิงก์รูป/ลายเซ็นเลิกชี้ Drive ตรง (visitFiles · 2026-09-24)
          — อ่านด้วยมือแล้ว ยังเป็น <Button> ทั้งคู่ */
  "src/components/service/CloseVisitSheet.js:591",
  "src/components/service/CloseVisitSheet.js:606",
  /* JSDoc ที่เขียนว่า "href แทน onClick = รายการที่พาไปหน้าอื่น" — คอมเมนต์จริง ล้างถูกแล้ว
     ⚠️ เลขบรรทัดขยับจาก 25 เป็น 26 ตอนเติม import ของ `thaiText` (2026-09-07) —
     รายการนี้ผูกกับ **บรรทัด** จึงขยับตามทุกครั้งที่มีการเติมบรรทัดเหนือมัน */
  "src/components/ui/RowActionMenu.js:26",
