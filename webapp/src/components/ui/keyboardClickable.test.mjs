...
/* 🪤 เทสต์นี้คือตัวกัน "ด่านกลวง" — ด่านที่ยกเว้นทุกอย่างจนเหลือ 0 ก็ผ่านเพดานได้
   เหมือนกัน · ล็อกไว้ว่าตัวยกเว้นทั้งสามทางยังกว้างเท่าเดิม ไม่ได้ถูกขยายเงียบ ๆ
   เพื่อกดเลขลง (ถ้าตัวเลขพวกนี้ขยับ ต้องมีคนอธิบายว่าทำไมในคอมมิตเดียวกัน) */
test("ทางยกเว้นทั้งสามทางยังกว้างเท่าเดิม (กันด่านกลวง)", () => {
  const { counts } = scan();
  ...
  assert.equal(counts.native, 553, "แท็กที่คีย์บอร์ดกดได้เอง (<button> ล้วน)");
});
