...
test("แจ้งเตือนต้องไม่ต่ำกว่าโมดัล", () => {
  /* 🔴 ตอนนี้ *เท่ากัน* พอดี = ใครอยู่บนขึ้นกับลำดับใน DOM ล้วน ๆ ซึ่งเปราะ
     เทสต์นี้กันไม่ให้แย่ลงกว่านี้ (ห้ามต่ำกว่า) ส่วนการยกให้สูงกว่าจริง ๆ
     ต้องเปิดจอที่มีทั้งโมดัลและ toast พร้อมกันดูก่อน — เป็นงานแยก */
  assert.ok(TOKENS.get("--z-toast") >= TOKENS.get("--z-modal"),
    "แจ้งเตือนที่จมอยู่ใต้โมดัล = ผู้ใช้ไม่เห็นผลลัพธ์ของสิ่งที่เพิ่งกด");
});
