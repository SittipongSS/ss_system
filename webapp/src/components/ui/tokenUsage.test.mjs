...
/* หนี้ที่ยกเว้นไว้ — **ว่างแล้วตั้งแต่ 2026-09-23**
   เดิมถือ `--doc-accent-watermark` กับ `--doc-accent-soft`: ธีมเอกสารพิมพ์ใน src/lib/ ผลิตสตริงโทเคนแบบ
   generate ทั้งชุด (--doc-accent + -soft + -watermark) แม้สองตัวหลังไม่มีกฎไหนอ่าน · ปิดหนี้แล้วด้วยการถอด
   `soft`/`watermark` ออกจาก DOCUMENT_ACCENT_THEMES · accentStyle · ค่าตั้งต้นบน .document (documentShell.js)
   ⇒ เอกสารทุกชนิดประกาศแค่ --doc-accent ตัวเดียวที่ชื่อเอกสารอ่านจริง
   ⚠️ ห้ามเติมชื่อใหม่เข้าลิสต์นี้เพื่อให้เทสต์ผ่าน — โทเคนตายให้ลบ หรือต่อสายในรอบเดียวกัน
   (ประวัติเดิมของสองชื่อนี้เก็บไว้ข้างล่างให้คนที่ grep เจอเข้าใจว่าเคยเกิดอะไร)
   ⭐ `--doc-accent-soft` ออกจากลิสต์เมื่อ 2026-09-17 — ใบสเปคสินค้า FM-SA-04 ใช้เป็น
      พื้นหัวตารางจริง (mig 0364) ⇒ ไม่ใช่โทเคนที่ประกาศแล้วไม่มีใครอ่านอีกต่อไป
   ↩️ **กลับเข้าลิสต์ 2026-09-22** — ผู้อ่านคนเดียวคือ FM-SA-04 และมติผู้ใช้รอบสี่ ("ปรับการใช้ accent color
      ดูใบเสนอราคาเป็นตัวอย่าง") ถอด accent ออกจากตาราง/กรอบภาพของใบนั้นทั้งหมด (accent เหลือที่ชื่อเอกสาร
      แบบใบเสนอราคา) ⇒ กลับไปเป็นหนี้ชุดเดิมของธีม (ชื่อเดิม ไม่ใช่ชื่อใหม่) · เทสต์ที่สองข้างล่างยืนยันว่าตายจริง
      ⚠️ ทางแก้จริงคือถอด `soft` ออกจากธีม accent ของเปลือก (DOCUMENT_ACCENT_THEMES/accentStyle) — แตะ HTML
      ของเอกสารทุกชนิด จึงแยกเป็นงานของสายเอกสาร ไม่ทำในรอบสีของ FM-SA-04
   ✅ **ทำแล้ว 2026-09-23** (งานสายเอกสาร) ⇒ ลิสต์ว่าง */
