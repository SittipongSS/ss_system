...
test("ห้ามห่อ TableScroll ด้วยการ์ดที่มีกรอบอยู่แล้ว", () => {
  ...
  /* 🎯 **hard zero** ตั้งแต่ 2026-09-07 — ไล่เก็บครบทั้ง 16 จุดแล้ว ไม่มีเพดานให้ไต่
     และ **ไม่มีลิสต์ยกเว้น** (ทะเบียนยกเว้นคือทะเบียนที่หมดอายุเงียบ)

     ทางออกมีสองทางเท่านั้น:
       ตารางไม่ได้พึ่งสไตล์ของคลาสนอก  ⇒ ถอด div ทิ้ง แล้วใช้ `surface="auto"`
                                          ให้ TableScroll วาดการ์ดเอง
       ตารางพึ่งอยู่ (ไม่ใช่ .premium-table) ⇒ **ย้ายคลาสไปไว้บน
                                          <TableScroll className=…>** ไม่ใช่ห่ออีกชั้น */
  assert.deepEqual(offenders, [],
    "การ์ดที่มีกรอบห่อ TableScroll = กรอบมนซ้อนกัน (บางที่ได้สกอร์ลแนวนอนสองชั้นแถมมาด้วย)\n"
