...
const RECOMMENDED = [480, 560, 640, 680, 768, 900, 1000, 1200];
...
/* ค่านอกชุดแนะนำที่ยังเหลือ — 2026-07-30 ยุบไป 8 ค่าที่ช่วงเปลี่ยนไม่ชนความกว้าง
   อุปกรณ์จริง ห้าตัวนี้ยุบไม่ได้เพราะช่วงที่จะเปลี่ยน **คาบเกี่ยวอุปกรณ์ที่ใช้จริง**
   ต้องมีคนลากหน้าต่างดูก่อนถึงจะตัดสินได้ */
const OFF_SCALE = {
  800: "ชนความกว้าง 800 พอดี",
  820: "ชน 800/810/820 — iPad Air แนวตั้ง",
  1050: "ชน 1024",
  1100: "ชน 1024 และ 1080",
  1120: "ห่างจาก 1200 ถึง 80px = ขยับเลย์เอาต์เยอะ",
};

test("ค่านอกชุดแนะนำมีเท่าที่บันทึกไว้ ไม่มีตัวที่หกงอกมาเงียบ ๆ", () => {
  /* breakpoints() คืน Map (ค่า → ตำแหน่งที่ใช้) ต้องเอา key ไม่ใช่ entry */
  const extra = [...breakpoints().keys()].filter((v) => !RECOMMENDED.includes(v)).sort((a, b) => a - b);
  assert.deepEqual(extra, Object.keys(OFF_SCALE).map(Number).sort((a, b) => a - b),
    "มีค่านอกชุดแนะนำเปลี่ยนไป — ถ้าเพิ่มค่าใหม่ให้หยิบจากชุดแนะนำ ถ้ายุบได้ให้ลบออกจาก OFF_SCALE");
});
