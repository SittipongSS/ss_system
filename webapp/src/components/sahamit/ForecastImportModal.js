...
export default function ForecastImportModal({ open, onClose, onCreated, products = [] }) {
  ...
  // Changing the range manually clears any upload-pinned months.
  const onStart = (v) => { setStartMonth(v); setMonthsOverride(null); };
  const onEnd = (v) => { setEndMonth(v); setMonthsOverride(null); };
  ...
  return (
    <Modal open={open} onClose={onClose} title="นำเข้ารอบ FC ใหม่" size="lg">
      <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
        {/* Round meta + month range (start → last month of the round) */}
