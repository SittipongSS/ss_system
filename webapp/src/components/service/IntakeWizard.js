...
export default function IntakeWizard({
  ...
  useEffect(() => {
    ...
    setError("");
    setAllocs([]);
    /* 🪤 ผูกกับ **รหัสใบ** ไม่ใช่ตัวอ็อบเจกต์ — หน้าคิวโหลดใหม่แล้วส่งแถวก้อนใหม่ของใบเดิมเข้ามาได้
       ถ้าผูกกับตัวอ็อบเจกต์ วิซาร์ดจะเด้งกลับขั้น 1 แล้วลบสิ่งที่ TS จัดสรรค้างไว้ทิ้ง */
  }, [open, order?.orderId]);
  ...
  /* ⚠️ ตัดแถวจัดสรรที่กลุ่มของมันหายไปแล้วทิ้ง — คิวโหลดใหม่ระหว่างเปิดวิซาร์ดได้ (คนอื่นผูกจุดนั้นไปก่อน)
     แต่สิ่งที่ TS กรอกค้างไว้ยังอยู่ใน state ⇒ ถ้านับต่อ ปุ่มบันทึกจะเปิดทั้งที่ไม่มีอะไรจะผูก */
