...
import { loadCommercialPresetKindByVersion, updateCommercialPresetDraft, CommercialPresetError } from '@/lib/admin/commercialPresets';
...
export async function PATCH(request, context) {
  ...
  try {
    ...
    // ชนิดคลังมาจาก DB เสมอ (identity แก้ไม่ได้) — client ส่ง kind มาก็ไม่มีผล
    const kind = await loadCommercialPresetKindByVersion(getSupabaseAdmin(), id);
    const normalized = normalizeCommercialPresetInput(body, { kind });
    ...
    await recordAudit({ user, action: 'update', entityType: 'commercial_preset_version', entityId: id, before: result.before, after: result.after, summary: `บันทึกชุดเงื่อนไขการค้า“${result.after.title}” Version ${result.after.versionNumber}`, request });
