...
import { enforceMasterPrices, normalizeManualLines, quotePriceField } from '@/lib/sales/quoteLines';
...
export const POST = withUser(async ({ user, supabase, req, ctx }) => {
  ...
  // ฐานราคาสืบทอดจากใบเดิม (metadata.priceBasis — สายสหมิตร = ราคาโรงงาน)
  body.lines = await enforceMasterPrices(
    supabase,
    normalizeManualLines('lines' in body ? body.lines || [] : quote.lines || []),
    quote.lines || [],
    quotePriceField(quote.metadata),
  );
