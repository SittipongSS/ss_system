  ...
  try {
    ...
    for (const row of rows) {
      const scent = await createScent(supabase, {
        ...
        note: row.note,
        /* ⭐ **ผู้ปรุงกลิ่นมาจากใบ** (มติผู้ใช้ 2026-09-02: "perfumer คือคนปรุงกลิ่น
           ตอนพัฒนากลิ่นจะมีผู้เซ็น perfumer นั่นแหละอันเดียวกัน") — ชื่อที่ฝ่ายเลือกไว้
           ตอนกดรับเรื่องถูกใช้ต่อทันที ⇒ ทะเบียนไม่ต้องมากรอกซ้ำ และสองที่ตรงกันเสมอ
           ⚠️ ไม่มี `perfumerId` เพราะช่องผู้เซ็นเก็บเป็น **ชื่อบนกระดาษ** อย่างเดียว
           (คนเซ็นที่ไม่มีบัญชีมีจริง) · ทะเบียนยังแก้ทีหลังให้ผูก id ได้
           ⚠️ ใบที่ไม่ได้เลือกผู้เซ็นไว้ = ว่าง ไม่ใช่เดาจากคนที่กดส่ง — คนกดส่งอาจเป็น
           ผู้ประสานงาน ไม่ใช่คนปรุง (ของจริงในทะเบียน 115 กลิ่นเป็นแบบนั้นทั้งหมด) */
        perfumerName: before.pdrSignPerfumer || null,
      }, user, { accepted: true });
