...
export async function PATCH(request, { params }) {
  ...
  if (body.role !== undefined) {
    ...
    /* ⚠️ ทะเบียนสดเป็นตัวตัดสิน ไม่ใช่ค่าคงที่ (มติ 2026-09-07) — เส้นนี้คือทางฉุกเฉิน
       ...
       🐞 **"ไม่ได้ส่งทีมมา" ต้องยอมรับทีมที่ปิดไปแล้วด้วย** (ตรวจย้อน 2026-09-07)
       ฟอร์มแก้ผู้ใช้ส่ง `role` เสมอแต่ไม่ส่งทีม ⇒ ค่าเดิมถูกยกมาเทียบ · ถ้าเทียบกับ
       เฉพาะทีมที่ยัง active คนที่ทีมเดิมถูกปิดไปแล้วจะ **แก้อะไรไม่ได้เลยตลอดกาล**
       (เปลี่ยนชื่อ · เปลี่ยนตำแหน่ง · รีเซ็ตรหัส ตีกลับ 400 หมด) ทั้งที่คนแก้ไม่ได้
       เป็นคนเลือกทีมนั้น · ส่งทีมมาเองเมื่อไร ค่อยบังคับว่าต้องเป็นทีมที่เปิดอยู่ */
    const teamGiven = body.team !== undefined || body.teams !== undefined;
    const salesCodes = await loadSalesTeamCodes(supabase, { includeInactive: !teamGiven });
