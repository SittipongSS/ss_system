...
// PUT  → บันทึกผลทั้งชุดในคำขอเดียว (ช่างติ๊กทีละตัวบนจอ แล้วกดบันทึกครั้งเดียว)
//
...
export const PUT = withUser(async ({ user, supabase, req, ctx }) => {
  ...
  try {
    ...
    /* เขียนทับทั้งชุด: ลบของเดิมแล้วใส่ใหม่
       ...
       และถ้า insert ล้ม ช่างเห็น error แล้วกดบันทึกใหม่ได้ทันทีจากฟอร์มที่ยังคาอยู่ */
