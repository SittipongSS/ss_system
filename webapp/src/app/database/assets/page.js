...
export default function ServiceAssetsPage() {
  ...
  /* ⚠️ **สามสภาพ ไม่ใช่สอง** (mig 0344) — เครื่องที่ยังไม่ได้ติดตั้ง **ไม่มีไซต์เลย**
     ...
     `/database/sites/null` และชื่อลูกค้าเป็นขีดสองบรรทัดซ้อนกัน */
  const locationCell = (asset) => {
    ...
    return (
      <>
        <Link href={`/database/sites/${asset.siteId}`} className={styles.siteLink}>{naText(asset.siteName)}</Link>
    ...
    >
      ...
      <Tabs
        ...
        onChange={(next) => {
          ...
          router.replace(next === "models" ? "/database/assets?tab=models" : "/database/assets", { scroll: false });
        }}
        ...
        {loadError ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {pageRows.map((asset) => (
              ...
              <ClickableCard key={asset.id} href={`/database/assets/${asset.id}`} className={`${styles.card} clickable-row p-4 flex-col gap-2`}>
                <div className="flex items-start justify-between gap-2">
                  <Link href={`/database/assets/${asset.id}`} className="min-w-0 linklike linklike-block">
                ...
                {pageRows.map((asset) => (
                  <tr key={asset.id}>
                    <td>
                      ...
                      <Link href={`/database/assets/${asset.id}`} className={`${styles.assetLink} mono`}>
