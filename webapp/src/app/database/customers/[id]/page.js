...
import { brandLabelOf, brandLabel, normalizeBrands } from "@/lib/master/brands";
import { fmtPhone, fmtNationalId, productName } from "@/lib/format";
                ...
                <div key={p.id} onClick={() => (window.location.href = `/database/products/${p.id}`)} className="glass-panel clickable-row cursor-pointer p-4 flex flex-col gap-2">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <div className="font-semibold text-[var(--text)] text-sm truncate">{productName(p)}</div>
                      <div className="text-[11px] text-[var(--text-3)] font-mono mt-0.5">{p.fgCode} · {brandLabel(p.brandName, p.brandNameEn)}</div>
                    </div>
                  ...
                  {products.map((p) => {
                    ...
                    return (
                      <tr key={p.id} onClick={() => (window.location.href = `/database/products/${p.id}`)} className="clickable-row">
                        ...
                        <td>
                          <div className="font-semibold text-[var(--text)]">{productName(p)}</div>
                          <div className="text-[10px] text-[var(--text-3)] font-mono mt-0.5">Brand: {brandLabel(p.brandName, p.brandNameEn)}</div>
                        </td>
