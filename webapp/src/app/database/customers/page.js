...
import { brandTh, brandEn, brandLabelOf } from "@/lib/master/brands";
                ...
                {c.brands?.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {c.brands.slice(0, 4).map((b, i) => (
                      <span key={i} title={brandTh(b) || undefined} className="bg-[var(--panel-2)] px-2 py-0.5 rounded text-[11px] text-[var(--text-2)]">{brandLabelOf(b)}</span>
                    ))}
                ...
                {pageRows.map((c) => (
                  <tr key={c.id} onClick={() => open(c)} className="clickable-row" style={c.isActive === false ? { opacity: 0.55 } : undefined}>
                    ...
                    <td className="text-[var(--text-2)]">
                      <div className="flex flex-wrap gap-1.5">
                        {c.brands?.map((b, i) => (
                          <span key={i} className="bg-[var(--panel-2)] px-2 py-0.5 rounded text-[11px] text-[var(--text-2)]">{brandLabelOf(b)}</span>
                        ))}
                      </div>
                    </td>
