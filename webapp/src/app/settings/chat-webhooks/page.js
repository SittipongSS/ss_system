...
// ข้อมูลปฏิบัติการ แก้ตรงบนตาราง chat_webhooks (Decision 0012 ฉบับแก้ไขครั้งที่ 2:
...
export default function ChatWebhooksPage() {
  ...
  const [pendingSave, setPendingSave] = useState(null); // row ที่รอยืนยันบันทึก
  ...
  return (
    <Workspace hideHeader back={{ href: "/settings", label: "กลับหน้าตั้งค่า" }}>
          ...
          {rows.map((row) => (
            <section key={row.key} className="glass-panel" style={{ padding: "16px 18px" }}>
              ...
              <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                ...
                <button type="button" className="btn btn-primary" disabled={busyKey === row.key || !row.dirty} onClick={() => setPendingSave(row)}>
                  <Save size={15} /> บันทึก
                </button>
                <button
                  type="button"
      ...
      <ConfirmDialog
        ...
        description={pendingSave ? `บันทึก webhook ของ space "${pendingSave.label}"?` : ""}
        detail={pendingSave
          ? `${pendingSave.enabled ? "เปิดใช้" : "ปิดใช้"} · ${pendingSave.url ? "ใช้ URL จากหน้านี้" : pendingSave.envFallback ? "ไม่มี URL — ใช้ค่าจาก env (Vercel)" : "ไม่มี URL — แจ้งเตือน space นี้จะถูกปิด"} — มีผลกับการแจ้งเตือนทันที`
          : ""}
        confirmLabel="บันทึก"
        busy={!!pendingSave && busyKey === pendingSave.key}
        onConfirm={() => save(pendingSave)}
        onClose={() => { if (!busyKey) setPendingSave(null); }}
      />
