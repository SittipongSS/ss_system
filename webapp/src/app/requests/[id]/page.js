...
import {
  canEditPdrRefManual, normalizePdrRefNo, pdrRefManualError,
...
export default function RequestDetailPage() {
  ...
  /* ⭐ ฟอร์มอยู่ใน `CommitDueDialog` (ตัวเดียวกับหน้าจัดคิว · มติเจ้าของ 23/09) — หน้านี้ถือแค่เปิด/ปิด
     ⚠️ ค่าตั้งต้นของฟอร์มเกิดใหม่ทุกครั้งที่เปิด (`commitDueDefaults`) ไม่ค้างจากรอบก่อน */
  ...
  /* ⭐ ธงกำลังโหลด/พังส่งเข้าโมดัลลงคิว — ไม่งั้นทะเบียนช้าหรือพัง = โมดัลบอกว่า "ยังไม่มีบัญชีฝ่าย TS" (รีวิว 24/09) */
  const { users: directory, loading: directoryLoading, error: directoryError } = usePeopleDirectoryState();
  ...
  /* คนที่ "ถูกมอบหมายให้เข้าไซต์" ได้ — ใช้ตัวกรองเดียวกับหน้าจัดคิวเจ้าหน้าที่และหน้าไซต์
  ...
  /* ⚠️ ตัวเลือกคน (`CrewLoadPicker`) เรียงตามลำดับที่ส่งให้ ไม่เรียงเอง ⇒ เรียงชื่อที่นี่
     · ชื่อว่างถอยไปอีเมล/รหัส (ของเดิมของดรอปดาวน์) — แถวที่ไม่มีชื่อเลยคือแถวที่เลือกไม่ได้ */
  const technicians = useMemo(() => activePeople.filter(canBeServiceAssignee)
    .map((u) => ({ ...u, name: u.name || u.email || u.id }))
    .sort((a, b) => String(a.name).localeCompare(String(b.name), "th")), [activePeople]);
  ...
  const formulaLockedFor = (item) => (formulaPreviewFor(item).lockLineage
    ? ["customerId", "scentId", "categoryCode", "derivedFromFormulaId"]
    : ["customerId", "scentId", "categoryCode"]);
  // ประโยคบอกผลก่อนกด (เก็บสูตรเดิม · ผูกของเดิม · ติดร่าง) — ต้องรู้ก่อนกด ไม่ใช่ไปเจอในทะเบียน
  const formulaReworkNote = (item) => formulaPreviewFor(item).note;
  /* ดึงทะเบียนสูตรใหม่หลังส่งสูตร — ทะเบียนบนจอโหลดครั้งเดียวตอนเปิดหน้า ⇒ รอบแก้ที่เกิดทีหลังในหน้าเดียวกันหา
     สูตรต้นทางไม่เจอ (ไม่เติมชื่อ · ช่องต้นทางที่ล็อกขึ้นว่าง · ประโยคไม่มีรหัส) (รีวิว ม-147) */
  const refreshFormulas = useCallback(() => {
    apiFetch("/api/master/formulas", { cache: "no-store" })
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => { if (Array.isArray(d)) setRegistry((prev) => ({ ...prev, formulas: d })); })
      .catch(() => {});
  }, []);
  ...
  const call = useCallback(async (path, init, okMsg) => {
    ...
    try {
      ...
      /* ⚠️ **งานที่สำเร็จครึ่งเดียวต้องไม่ขึ้นว่า "สำเร็จ"** — บางก้าวเขียนสองที่
         (ใบ + นัดของเจ้าหน้าที่) แล้วไม่มีทรานแซกชันครอบ · server ส่ง `_warning` มาบอก
         ⇒ ทักด้วยน้ำเสียงเตือน ไม่ใช่เขียว */
      // ค้างนานกว่าทักทั่วไป — ข้อความพวกนี้บอกทางแก้ที่ต้องอ่านให้จบ (`RESPONSE_WARNING_TOAST`)
      if (d?._warning) setToast({ kind: "error", msg: d._warning, duration: RESPONSE_WARNING_TOAST.duration });
  ...
  /* ⭐ **วันที่ถืออยู่เป็นของรอบก่อน** (มติผู้ใช้ 2026-08-25) — คำบนปุ่ม/โมดัลเปลี่ยนจาก
     "แจ้งกำหนดส่ง" เป็น "แจ้งวันส่งรอบแก้" · ตัดสินใน `commitDueLabels` (ถาม `dueIsStale`
  ...
  /* ⭐ **ใบมีวันแล้วแต่นัดไม่เกิด** — ครึ่งหลังของการลงคิวล้ม (หรือนัดถูกลบ) ⇒ ต้องมี
     ...
     แต่เจ้าหน้าที่ไม่มีงานบนตาราง (ด่านฝั่ง server เปิดทางไว้แล้วด้วยธง `requeue`) */
  /* 🐞 **"ไม่มีนัด" ต้องหมายถึงไม่มีนัดที่ยัง *มีชีวิต*** ไม่ใช่ "ไม่มีแถวนัดเลย" —
     ของเดิมเช็ค `!req.surveyVisit` ⇒ ใบที่เจ้าหน้าที่ไปแล้วเข้าไม่ได้ (`unable`) หรือนัดถูกยกเลิก
     ...
     🐞 **ช่างไปถึงไซต์แล้ว (`done`/`partial`) ไม่ใช่ "ไม่มีนัด"** — ของเดิมถามแค่ "ไม่มีนัดที่ยังมีชีวิต"
        ⇒ ใบที่เข้าพื้นที่เสร็จแล้วขึ้นปุ่ม "ลงคิวใหม่" พร้อมคำใบ้ "นัดยังไม่ขึ้นตาราง" ซึ่งผิดทั้งสองท่อน
        (งานเดินหน้าไปรอผลแล้ว — surveyStepBack.js) · ใบที่ผู้ขอปิดแล้วก็เคยได้ปุ่มนี้
        ⇒ ถาม `surveyQueueStep` **ตัวเดียวกับการ์ดบนหน้าจัดคิว** (มติเจ้าของ 23/09) — สองจอตอบตรงกันเสมอ */
  const needsRequeue = surveyQueueStep(req) === "requeue";
  const dueDialogLabels = commitDueLabels(req, { requeue: needsRequeue });
  ...
  const submitHop = async () => {
    ...
    if (ok) {
      setHopDraft(null);
      if (hop === "ready" && item.lineKind === "product_dev") refreshFormulas();
    }
  };
  ...
  const submitBulkReady = async () => {
    setSaving(true);
    const failed = [];
    // คำเตือนของแถวที่ส่งสำเร็จ (ผูกสูตรเดิม = ฟอร์มไม่ได้ใช้) — ต้องถึงตาคน ไม่ใช่ขึ้นเขียวว่า "สูตรเข้าทะเบียนแล้ว" (รีวิว ม-147)
    const warnings = [];
    let sent = 0;
    try {
    ...
    await load();
    if (sent) refreshFormulas();
    if (failed.length) {
      setBulkReady({ ...bulkReady, rows: failed });
      setToast({
        kind: "error",
        msg: [`ส่งแล้ว ${sent} รายการ · ติด ${failed.length} รายการ — แก้แล้วกดส่งซ้ำได้`, ...warnings].join(" · "),
        ...(warnings.length ? { duration: RESPONSE_WARNING_TOAST.duration } : {}),
      });
    } else if (warnings.length) {
      setBulkReady(null);
      setToast({
        kind: "error", msg: `ส่งงาน ${sent} รายการแล้ว · ${warnings.join(" · ")}`, duration: RESPONSE_WARNING_TOAST.duration,
      });
    } else {
      setBulkReady(null);
  ...
  const confirmCopy = () => {
    ...
    /* ⭐ **รับเรื่องต้องมีโมดัลบอกผลลัพธ์** — กติกาเดียวกับทุกการอนุมัติในระบบ ·
       ...
       ⭐ ข้อความย้ายไป `lib/requests/acknowledgeConfirm.js` (มติเจ้าของ 23/09) — การ์ดคำร้องบน
          หน้าจัดคิวกดรับเรื่องได้ด้วย ⇒ สองปุ่มต้องบอกผลชุดเดียวกัน (ตัวอักษรเดิม · เทสต์ล็อกไว้) */
    if (confirm.kind === "acknowledge") return acknowledgeConfirmCopy(req);
    ...
    if (confirm.kind === "answer") {
      /* ⭐ **ผู้ขอปิดฝั่งตัวเองไปก่อนแล้ว = ปุ่มนี้คือการปิดเรื่องจริง ๆ** (มติผู้ใช้
         2026-08-28) — ตราครบสองฝั่งเมื่อไรใบจบถาวร ⇒ โมดัลต้องพูดแบบเดียวกับฝั่งผู้ขอ
         ที่กดทีหลัง (สาขา `deptPending` ของ `close` ข้างล่าง) ไม่ใช่ยืนยันว่า "ยังไม่จบ"
         🐞 ของเดิมบอกว่า "ใบยังไม่จบจนกว่าผู้ขอจะกดปิดเรื่องด้วย" ทั้งที่ผู้ขอกดไปแล้ว
         ⇒ ฝ่ายกดโดยคิดว่ายังถอนได้ แต่ใบปิดถาวรทันที */
      const requesterDone = closure.requesterDone;
      return {
        title: requesterDone ? "ปิดเรื่อง" : "ทำเครื่องหมายว่าตอบแล้ว",
        description: req.docNo || "",
  ...
  const runConfirm = async () => {
    ...
    const labels = {
      ...
      // ⭐ ข้อความหลังกดต้องตรงกับสิ่งที่เพิ่งเกิด — ตราครบสองฝั่ง = ใบจบ ไม่ใช่ "ตอบแล้ว"
      answer: closure.requesterDone ? "ปิดเรื่องแล้ว · ครบสองฝั่ง" : "บันทึกว่าตอบแล้ว",
      close: closure.deptDone ? "ปิดเรื่องแล้ว · ครบสองฝั่ง" : "ปิดเรื่องแล้ว",
  ...
  const primaryAction = editing
    ...
    : owner && req.status === "pending"
      ...
      : owner && (requestAwaitingDue(req) || needsRequeue)
        ? {
          id: "commit-due",
          /* ป้าย/คำใบ้มาจาก `commitDueLabels` ตัวเดียวกับปุ่มบนการ์ดหน้าจัดคิว (ลงคิวใหม่ · รอบแก้ · หัวข้อ) */
          label: dueDialogLabels.action,
          hint: dueDialogLabels.hint,
          kind: "approve",
          icon: CalendarClock,
          /* ⭐ ค่าตั้งต้น (วันที่ผู้ขอต้องการ · ของเดิมบนใบตอนกู้ · วันส่งผล) เกิดในโมดัลกลาง
             (`commitDueDefaults`) — หน้านี้แค่เปิด */
      ...
      : canMarkAnswered
        ? {
          id: "answer",
          /* ⭐ **ปุ่มพูดตามผลของการกด** (มติผู้ใช้ 2026-08-28) — ผู้ขอกด "ปิดเรื่อง"
             ไปก่อนแล้ว การกดของฝ่ายคือตราใบสุดท้าย ⇒ ใบจบถาวรทันที · เขียนว่า
             "ตอบแล้ว" ต่อไปคือปุ่มที่บอกน้อยกว่าสิ่งที่มันทำ
             ⚠️ **ปุ่มเดียวเหมือนเดิม ไม่ใช่ปุ่มใหม่** — action ที่ยิงยังเป็น `answer`
             (ตราฝั่งฝ่าย = `answeredAt`) เปลี่ยนแค่คำที่คนอ่าน ⇒ กติกาสองตราไม่ขยับ */
          label: closure.requesterDone ? "ปิดเรื่อง" : "ตอบแล้ว",
          hint: closure.requesterDone
            ? `${requestSideText(req, "requester", "ปิดฝั่งตัวเองแล้ว")} — กดแล้วใบจบถาวร`
            : `${requestSideText(req, "requester", "ยังต้องกดปิดเรื่องด้วย")}`,
          kind: "approve",
          icon: CheckCheck,
  ...
  return (
    <Workspace hideHeader back={back}>
      ...
      {/* ⭐ **แจ้งกำหนดส่ง — ก้าวที่สองของฝ่ายผู้รับ** (มติผู้ใช้ 2026-08-19)
          ...
          ⭐ **โมดัลกลางตัวเดียวกับหน้าจัดคิว** (มติเจ้าของ 23/09 — TS ลงคิวคำร้องได้จากหน้าจัดคิวด้วย)
             ก้อนที่ส่งประกอบจาก `commitDuePayload` ตัวเดียว · ตัวเลือกเจ้าหน้าที่เห็นภาระของวันนั้น
             (หน้านี้ไม่ส่งภาระ ⇒ โมดัลโหลดเองด้วยสูตรเดียวกับหน้าจัดคิว) */}
      <CommitDueDialog
        open={dueDialogOpen}
        request={req}
        ...
        }, dueDialogLabels.okMsg).then((ok) => { if (ok) setDueDialogOpen(false); })}
      />
          ...
          <>
            ...
            {bulkReady.rows.map((row, i) => (
              <div key={row.item.id} className={styles.bulkRow}>
                ...
                {formulaReworkNote(row.item) && <p className={styles.fieldHint}>{formulaReworkNote(row.item)}</p>}
                <FormulaForm
                  ...
                  categories={productTypes}
                  locked={formulaLockedFor(row.item)}
      ...
      >
        {hopDraft && (
          <>
            ...
            {hopDraft.hop === "ready" && hopDraft.item.lineKind === "product_dev" && (
              <>
                {/* ⚠️ ประโยค "สูตรเข้าทะเบียนทันที" จริงเฉพาะแผนสร้าง/รอบแก้ — แผนผูกของเดิม/ติดร่าง ประโยคบอกผลข้างล่างพูดแทน
                    (สองประโยคขัดกันบนจอเดียว = คนเชื่อตัวใหญ่ · รีวิว ม-147) */}
                ...
                {formulaReworkNote(hopDraft.item) && (
                  <p className={styles.fieldHint}>{formulaReworkNote(hopDraft.item)}</p>
                )}
                <FormulaForm
                  mode="create" canSetCode codeRequired
                  ...
                  formulas={registry.formulas}
                  categories={productTypes}
                  locked={formulaLockedFor(hopDraft.item)}
