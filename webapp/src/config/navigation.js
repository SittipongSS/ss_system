...
/* ── เปลือกของหน้านี้หน้าตาเป็นอย่างไร — ตอบที่เดียว ────────────────────────
 *
 * ⭐ `/home` (ADR 0016) กางเมนูของทุกระบบอยู่ในหน้าแล้ว ⇒ ทุกชิ้นบนหัวที่ทำหน้าที่
 *    "พาไปเมนู/ระบบอื่น" กลายเป็นสำเนาที่สองบนจอเดียว จึงไม่วาดเลย: แถวระบบ ·
 *    ตัวสลับระบบ · แฮมเบอร์เกอร์+ลิ้นชัก · แถวเมนูของระบบ · แถบล่างมือถือ
 *    เหลือแถวบน 52px (โลโก้ · กระดิ่ง · บัญชี) เหมือนทุกหน้า แล้วต่อด้วยแถบต้อนรับในหน้า
 * ⚠️ `/account` ยัง **มี** แถวระบบและตัวสลับระบบตามเดิม — สองหน้านี้คนละกติกากัน
 *    (บัญชีของฉันเป็นหน้าเดี่ยวที่ต้องมีทางกลับเข้าระบบ · หน้าแรก *คือ* ทางเข้าระบบ)
 */
export function shellFlagsFor(pathname) {
  const bareShell = isBareShellPathname(pathname);
  const homeHub = pathname === '/home';
  return {
    bareShell,
    homeHub,
    // ไม่มีเมนูของระบบให้แบก: แฮมเบอร์เกอร์ · ลิ้นชัก · แถวเมนู · แถบล่างมือถือ
    hideSystemMenu: bareShell || homeHub,
    /* ตัวสลับระบบ (≤1200) และแถวระบบ (>1200) — ซ่อนเฉพาะหน้าแรก
       ⭐ **ตัวเลขบนหัวหายไปพร้อมกัน** (ADR 0016 ข้อ 4) เพราะทุกชิ้นที่ถือตัวเลขอยู่ใน
          สองก้อนนี้กับแถวเมนู/แถบล่างที่ `hideSystemMenu` ตัดไปแล้ว · ป้ายกระดิ่งคนละเรื่อง
       ⚠️ และไม่มีชิ้นไหนไฮไลต์ว่ากำลังอยู่ระบบไหน เพราะหน้าแรกไม่ได้อยู่ในระบบใดเลย */
    hideSystemSwitcher: homeHub,
  };
}
...
export function systemForPathname(pathname, user) {
  ...
  // ⭐ หน้าแรกก็ไม่ใช่ของระบบไหน — มันกางเมนูของ **ทุกระบบ** พร้อมกัน (ADR 0016)
  // 🐞 ถ้าไม่ดักตรงนี้จะตกไป `return 'tax'` ท้ายฟังก์ชัน ⇒ หน้าแรกสวมเมนูภาษีสรรพสามิต
  // และ AppLayout จะเขียน `ss:last-system='tax'` ทับระบบที่คนนั้นเพิ่งออกมา
  if (pathname === '/home' || pathname === '/notifications' || pathname === '/account'
