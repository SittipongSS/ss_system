...
// ── ใบตัวอย่างของ PDR ────────────────────────────────────────────────────────
// ⚠️ **สองเลขบนหัวใบ ไม่ได้มาจากรูปแบบเลขที่ของหน้าตั้งค่า** — "เลขที่เอกสาร" คือเลข ISO ของฝ่าย RD
// (`pdrRefNo` · DDMMYY-XXX ออกตอนรับเรื่อง · lib/requests/pdrRefNo.js) ส่วน "เลขที่คำร้อง" คือเลขของใบในระบบ
// ที่ออกตอนกดส่ง ⇒ ตัวอย่างใส่ทั้งสองเลขตามรูปจริง ไม่ใช่ numberingPatternExample เหมือนอีกสามใบ
// (🐞 เดิมคอมเมนต์นี้บอกว่า PDR ไม่ออกเลขเอง ซึ่งล้าสมัยตั้งแต่ mig 0271 และพรีวิวขึ้น "เลขที่เอกสาร -")
...
const pdrPreviewRequest = () => ({
  ...
  pdrContext: {
    // ⚠️ รูปเดียวกับที่ `pdrContext` ของ server จัดให้ (DD/MM/YYYY ผ่าน fmtDate) — ไม่ใช่ ISO ของฐาน
