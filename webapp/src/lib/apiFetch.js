...
//
// ⚠️ **ลองใหม่กับ POST/PATCH/DELETE มีสิทธิ์เกิดของซ้ำ** — `Failed to fetch` แปลว่า
//    "ไม่ได้ response" ไม่ได้แปลว่า "เซิร์ฟเวอร์ไม่ได้ทำ" · ถ้าเซิร์ฟเวอร์บันทึกเสร็จแล้ว
//    response หายกลางทาง การลองใหม่จะบันทึกซ้ำอีกใบ · ที่นี่ยอมรับความเสี่ยงนั้นเป็น
//    ค่าตั้งต้นเพราะของที่ซ้ำได้ส่วนใหญ่คือแถวที่ลบทิ้งได้ (งาน/โน้ต) และอาการเน็ตสะดุด
//    พบบ่อยกว่ามาก — **แต่ endpoint ที่ซ้ำแล้วเสียหาย** (ออกเลขเอกสาร, ตัดเงิน,
//    ยิงอนุมัติ, ส่งเมล) **ต้องส่ง `retry: false` เอง** ตอนย้ายมาใช้ตัวนี้
//
 ...
 *   retry — `true`/`false` บังคับเอง · ไม่ส่ง = ลองใหม่ (ดูคำเตือนหัวไฟล์)
 */
export async function apiFetch(url, options = {}) {
  const { retry, retryDelayMs = RETRY_DELAY_MS, ...init } = options;
  const method = (init.method || "GET").toUpperCase();
  const safe = SAFE_METHODS.has(method);
  const mayRetry = (retry ?? true) && isReplayableBody(init.body);
