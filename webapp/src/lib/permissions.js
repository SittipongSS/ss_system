...
//   TS = ฝ่ายเทคนิคบริการ (Technic Service) — เจ้าหน้าที่ที่เข้าไซต์ลูกค้าดูแลระบบกระจายกลิ่น
...
const DEPARTMENT_ROLES = {
  ...
  /* ⭐ **ทุกฝ่ายมี role ของตัวเอง** (มติผู้ใช้ 2026-08-28: *"จะไม่มีตำแหน่ง staff แล้วทุกฝ่าย"*)
     ...
     `canViewService`) · พลาดที่ไหนที่หนึ่ง = คลัง/QC เห็นต้นทุน หรือเจ้าหน้าที่แก้ตารางผลิตได้
  ...
  /* ⭐ **ห้าตำแหน่งจริงของฝ่าย** (มติผู้ใช้ 2026-08-30) — เดิมมี role เดียว (`ts`)
     ทำให้หัวหน้ากับเจ้าหน้าที่หน้างานถือสิทธิ์ชุดเดียวกันเป๊ะ · ผู้ใช้สั่งว่า "ต้องมี role
     ตามที่ส่ง เพื่อกำหนดสิทธิ" ⇒ ตำแหน่งเป็นตัวกำหนดสิทธิ์ ส่วน *ทีมเจ้าหน้าที่บริการ* จัดนอกระบบ role
     ⚠️ **ลำดับมีความหมาย** — ตัวแรกคือค่าที่ฟอร์มเลือกให้อัตโนมัติเมื่อสลับฝ่ายเป็น TS
        (users/page.js) ⇒ วางเจ้าหน้าที่หน้างานไว้ก่อน เพราะเป็นตำแหน่งที่รับเข้าบ่อยที่สุด */
...
/* ⭐ **คนที่ "แจกงาน" ในฝ่าย RD ได้** (มติผู้ใช้ 2026-09-08 — ตอบคำถามที่ค้างมาตั้งแต่
   2026-09-01 ว่า *"จ่ายงานหรือหยิบเอง"*: **จ่ายงาน หัวหน้าเป็นคนแจก**)
   หัวหน้าฝ่าย + ผู้ประสานงาน (คนที่ถือคิวของฝ่ายอยู่แล้ว) + `rd` เดิมที่ยังไม่ถูก
   ย้ายตำแหน่ง · **`rd_perfumer` และ `rd_chemist` ไม่อยู่ในลิสต์นี้** — เห็นตารางครบ
   แต่แจกงานให้ตัวเองหรือคนอื่นไม่ได้

   ⚠️ **นี่คือด่านของ "การแจกกลิ่น" เท่านั้น ไม่ใช่การรื้อสิทธิ์รายตำแหน่ง** — ทั้งห้า
   role ยังถือ cap ชุดเดียวกันหมด (`ROLE_CAPS[role] || ROLE_CAPS.rd`) ตามมติ
   2026-09-01 ที่ยังไม่ถูกกลับ · แตะ `ROLE_CAPS` เมื่อไรคือการรื้อสิทธิ์ทั้งฝ่าย
   ซึ่งกระทบทะเบียนกลิ่น/สูตรที่ gate ด้วย `isRdRole()` ไม่ใช่ด้วย cap

   ⚠️ **ไม่ทำเป็น cap ใหม่โดยตั้งใจ** — `adminHoldsEveryCap` สร้าง regex จากลิสต์
   prefix ที่เขียนมือ ⇒ cap ที่ขึ้นต้นด้วย prefix ใหม่จะ **มองไม่เห็น** แล้วแอดมิน
   ขาด cap นั้นได้เงียบ ๆ โดยเทสต์ยังเขียว · และ `SALES_HEAD_CAPS` รับ cap ใหม่ของ
   แอดมินไปอัตโนมัติ ⇒ หัวหน้าฝ่ายขายจะได้สิทธิ์แจกงานในฝ่าย RD ไปด้วยโดยไม่มีใครสั่ง
   ⚠️ แอดมินผ่านทาง `isSuperuser` ที่ผู้เรียก ไม่ใช่ทางลิสต์นี้ */
export const RD_LEAD_ROLES = ['rd', 'rd_coordinator', 'rd_supervisor'];

/** แจกกลิ่นให้ผู้ปรุงได้ไหม — หัวหน้า/ผู้ประสานงานของฝ่าย RD (หรือ superuser) */
export const isRdLead = (role) => RD_LEAD_ROLES.includes(normalizeRole(role));
...
export const ROLE_LABELS = {
  ...
  ts: 'เจ้าหน้าที่บริการ (Operation)',
  ts_planner: 'วางแผนงานบริการ (Planner)',
  ts_senior: 'เจ้าหน้าที่บริการอาวุโส (Senior)',
...
const SALES_OPS = [
  ...
  // ฝ่ายขายต้องตอบลูกค้าได้ว่า "ผลิตวันไหน / เจ้าหน้าที่เข้าเมื่อไหร่" จึงอ่านได้ทุกคน
...
const SUPERUSER_CAPS = [
  ...
  // ตารางเข้า service — เจ้าหน้าที่ฝ่าย TS + ทีมขาย SV · `service:work` = ปิดงานของตัวเอง
  // (เจ้าหน้าที่หน้างานถือตัวนี้แทน service:edit — ดู ROLE_CAPS.ts)
...
const ROLE_CAPS = {
  ...
  /* ── ฝ่ายปฏิบัติการ: หนึ่งฝ่าย หนึ่ง role (มติผู้ใช้ 2026-08-28) ──────────────
     ...
     ให้หลุด · QC ไม่มี production:edit ให้แก้ตาราง · เจ้าหน้าที่ไม่มี production:* เลย
  ...
  /* ── ฝ่ายเทคนิคบริการ (TS) — ห้าตำแหน่ง สามระดับสิทธิ์ (มติผู้ใช้ 2026-08-30) ──
     **ไม่อยู่ในสายโรงงาน** จึงไม่เห็นตารางผลิตทุกตำแหน่ง (มติ 2026-07-31)
     ...
     🔴 **เจ้าหน้าที่หน้างานไม่ถือ `service:edit`** — cap นั้นเปิดทั้งโมดูลพร้อมกัน (จัดคิวให้คนอื่น ·
        แก้ทะเบียนไซต์/โซน/เครื่อง · ผูกใบสั่งขายเข้าโซน · ลบนัด) ซึ่งเกินกว่า "ปิดงานของ
        ตัวเอง" ที่ผู้ใช้สั่งไว้มาก ⇒ เจ้าหน้าที่ได้ `service:work` แทน ซึ่งเปิดเฉพาะ *งานที่ตัวเอง
  ...
  ts: [
    ...
    /* ⭐ **ฝ่าย TS เปิดใบสั่งขายได้ (มติผู้ใช้ 2026-08-30)** — เดิมไม่มี role ฝ่าย TS
       ตัวไหนถือ `salesplan:view` เลยสักตัว ⇒ เปิดทะเบียน SO และหน้าใบไม่ได้ทั้งหมด
       (เมนูไม่ขึ้น · เปิด URL ตรงก็โดนเด้ง · API 403)
       ⇒ มติ "SO เป็นศูนย์กลางที่ SA·FN·TS มองใบเดียวกัน" เป็นจริงไม่ได้เลย
       ⚠️ **อ่านอย่างเดียว** — ไม่มี `salesplan:edit` · TS ไม่ใช่คนแก้งานขาย
       ⚠️ เมนูที่ได้คือเอกสารร่วมสามชนิด (ใบเสนอราคา · ใบสั่งขาย · สัญญา) ทรงเดียวกับ FN
          — ดีล/โครงการ/ภาพรวมไม่ขึ้น เพราะ `worksInSalesPipeline` ตัดฝ่ายที่มีบ้าน
          ของตัวเองอยู่แล้ว
       ⚠️ **TS เห็นราคาขายและยอดของลูกค้าทุกใบ** ไม่ใช่แค่ใบบริการ — ผู้ใช้รับทราบ
          ตอนเลือก (เสนอทางเลือก "เห็นเฉพาะส่วนที่เกี่ยว" ไปแล้ว แต่เลือกแบบนี้) */
    'salesplan:view',
  ],
  /* Planner — คนจัดตารางเจ้าหน้าที่และเป็นคนรับเรื่อง/ลงคิวใบคำร้องประเมินพื้นที่
     (มติ 2026-08-30: "ลงคิว/มอบหมายเจ้าหน้าที่ได้ = Planner + หัวหน้าเท่านั้น") */
  ts_planner: [
    'pm:view', 'products:view', 'customers:view',
    'service:view', 'service:work', 'service:edit',
    // เปิดใบสั่งขายได้เหมือน role `ts` — เหตุผลเต็มอยู่ที่นั่น (มติผู้ใช้ 2026-08-30)
    // ⚠️ หัวหน้าฝ่าย (`SERVICE_HEAD_CAPS`) สืบ cap ชุดนี้ต่อ ⇒ เติมที่นี่ที่เดียวครบทั้งฝ่าย
    'salesplan:view',
...
/* หัวหน้าฝ่าย TS = สิทธิ์ของ Planner + **จัดทีมเจ้าหน้าที่บริการของฝ่ายตัวเอง**
...
/* ── ตารางผลิต (Production Schedule) ─────────────────────────────────
   ฝ่ายที่ "วางคิวผลิตจริง" — ฝ่ายผลิต (PD) เท่านั้น (มติผู้ใช้ 2026-09-16)

   ⚠️ **PC เห็นโมดูลได้แต่ไม่ได้อยู่ในลิสต์นี้** (มติผู้ใช้ 2026-09-17 "PC เห็นได้") —
   เขาอ่านได้อย่างเดียว รูปเดียวกับ WH/QC เดิม · ไม่ใช่ทางตันแบบกลับด้าน เพราะทางตัน
   คือ "แก้ได้แต่เปิดหน้าไม่ได้" ไม่ใช่ "เปิดหน้าได้แต่แก้ไม่ได้"
   ⇒ วันไหนจะให้ PC วางคิวจริงอีกครั้ง เติม 'PC' ที่ลิสต์นี้ */
export const PRODUCTION_PLANNER_DEPARTMENTS = ['PD'];

/* ฝ่ายที่เปิดโมดูลวางแผนผลิตได้ — **PD (วางคิว) + PC (อ่าน)**
   มติผู้ใช้ 2026-09-16 แคบเหลือ PD · 2026-09-17 เติม PC กลับเข้ามาเฉพาะฝั่งอ่าน

   ⭐ เดิมกว้างถึง WH/QC และฝ่ายขายทุก role (cap `production:view` อยู่ใน
   SALES_OPS + OBSERVER_CAPS) ตามเหตุผล "ต้องตอบลูกค้าได้ว่าผลิตวันไหน"

   ⚠️ **ไม่ได้ถอด cap ออกจาก role ไหนเลย** — ตัวกั้นจริงคือ `canViewProduction`
   ข้างล่างที่แคบด้วยฝ่ายซ้ำอีกชั้น (รูปเดียวกับ `canQuoteCosting`) · วันไหนจะเปิด
   คืนให้คลัง/QC/ฝ่ายขาย เติมรหัสฝ่ายที่ลิสต์นี้ที่เดียวพอ

   ⚠️ **ไม่รวม TS** — ธุรกิจบริการเป็นคนละทีมปฏิบัติงาน ไม่ได้อยู่ในสายโรงงาน */
export const PRODUCTION_VIEWER_DEPARTMENTS = ['PC', 'PD'];

/* อ่านตารางผลิต — cap อย่างเดียวไม่พอ ต้องอยู่ฝ่าย PC/PD (หรือเป็น admin)

   ⚠️ **admin เท่านั้นที่ข้ามฝ่ายได้ ไม่ใช่ `isSuperuser`** — `isSuperuser` นับ
   หัวหน้าฝ่ายขายด้วย ซึ่งมติ 2026-09-16 ไม่ได้ให้เข้าโมดูลนี้ (รูปเดียวกับ
   `canManageTeams`) · ด่านนี้คือตัวเดียวที่การ์ดระบบ · แถบเมนู · และ API
   (`requireProduction`) ใช้ร่วมกัน ⇒ แคบที่นี่ที่เดียวแคบครบทุกทาง */
...
// แก้ไลน์/กำลังผลิต/คิวผลิต — ฝ่ายที่วางคิวจริงคือ PD (มติ 2026-09-16)
// ⚠️ **ด่านฝ่ายยังจำเป็น** แม้ cap จะแคบแล้ว (wh/qc ไม่มี production:edit) เพราะ
// `pc` ยังถือ production:edit อยู่ที่ชั้น role และวันหน้าถ้ามี role อื่นได้ cap นี้
//
...
export function canEditProduction(user) {
  ...
  // ⚠️ `admin` ไม่ใช่ `isSuperuser` — หัวหน้าฝ่ายขายไม่อยู่ในโมดูลนี้แล้ว (มติ 2026-09-16)
  if (user?.role === 'admin') return true;
  return PRODUCTION_PLANNER_DEPARTMENTS.includes(departmentOf(user));
}

// ── ตารางเข้า service (Technic Service) ─────────────────────────────
// ฝ่ายบริการที่เข้าไซต์ + ทีมขายธุรกิจบริการที่เป็นเจ้าของสัญญา
/* ── จัดทีม (มติผู้ใช้ 2026-08-28 · docs/team-management-plan.md) ──────────
   ...
   ⚠️ **ขอบเขตคือฝ่ายของตัวเอง** — หัวหน้าฝ่ายขายจัดทีมเจ้าหน้าที่บริการไม่ได้ และกลับกัน
...
export function canManageTeams(user, department = null) {
  ...
  /* ⚠️ **admin เท่านั้นที่ข้ามฝ่ายได้ ไม่ใช่ isSuperuser** — `isSuperuser` นับ
     ...
     ⇒ หัวหน้าฝ่ายขายจัดได้เฉพาะทีมของฝ่ายขาย ไม่ใช่ทีมเจ้าหน้าที่บริการ */
  if (user?.role === 'admin') return true;
...
// อ่านธุรกิจบริการ: ฝ่ายขายทุกตำแหน่งอ่านได้ (ต้องตอบลูกค้าได้ว่าเจ้าหน้าที่เข้าเมื่อไหร่)
...
export function canViewService(user) {
  ...
  return isServiceInsider(user);
}

/* ── โมดูลธุรกิจบริการเป็นของฝ่าย TS เท่านั้น (มติผู้ใช้ 2026-08-30) ─────────
   🔴 **"ระบบธุรกิจบริการ เข้าใช้ได้เฉพาะ TS"** — ของเดิมเปิดถึง **ทีมขาย SV** ด้วย
      (มติ 2026-07-30 ตอนที่ยังไม่มีบัญชีฝ่าย TS สักคน ทีม SV จึงเป็นคนดูแลงานบริการไปก่อน)
      · วันนี้ฝ่าย TS มีคนจริงครบทั้งห้าตำแหน่งแล้ว เหตุผลนั้นจึงหมดอายุ
...
// มติผู้ใช้ 2026-07-30: เห็น/แก้งานของ **ทั้งฝ่าย** ไม่ใช่แค่นัดที่ตัวเองถูกมอบหมาย —
// เจ้าหน้าที่สลับกันไปแทนกันเป็นเรื่องปกติ ถ้าล็อกรายคนงานจะค้างทันทีที่คนลา
export function canEditService(user) {
  if (!canUser(user, 'service:edit')) return false;
  return isServiceInsider(user);
}

/* ── เจ้าหน้าที่หน้างานทำงานของ *ตัวเอง* ได้ (มติผู้ใช้ 2026-08-30) ───────────────
   ...
   🔴 **เทียบรายใบเสมอ ไม่ใช่เทียบแค่ cap** — ถ้าเช็คแค่ cap เจ้าหน้าที่จะแก้ใบของเพื่อนได้
...
/* ── สร้างไซต์บริการ (มติผู้ใช้ 2026-08-29) ────────────────────────────────
   ...
   ⚠️ **แคบกว่า `canEditService` โดยตั้งใจ — และห้ามขยายตัวนั้นแทน**
      ...
      ใบสั่งขายเข้าโซน · เธรดของนัด ⇒ ขยายตัวนั้นเท่ากับให้คนขายทุกคนแก้คิวเจ้าหน้าที่ได้
...
/* ── อ่านใบส่งงาน (`/service/visits/[id]`) — ฝ่ายบริการ + ฝ่ายขาย (มติผู้ใช้ 2026-09-24) ─────────
   *"ใบส่งงานเปิดให้ฝ่ายขายดูได้ด้วย"* — ฝ่ายขายต้องตอบลูกค้าได้ว่าเข้าไปทำอะไร เจอปัญหาอะไร
   โดยไม่ต้องขอภาพหน้าจอจาก TS (ทะเบียนไซต์เปิดให้อ่านแล้ว 24/09 แต่ลิงก์ใบส่งงานยังตัน)
   ⚠️ **อ่านอย่างเดียว ใบเดียว** — ไม่ใช่ทางเข้าโมดูลบริการ: คิว · ปฏิทิน · ปิดงาน ยังเป็นของ TS
      (ด่านเขียนของนัดยังเป็น `canEditService` / `canWorkOwnVisit` เหมือนเดิมทุกจุด)
   ⚠️ ถาม **กลุ่ม** `isSalesRole` ไม่เทียบชื่อตำแหน่ง (ratchet ของผังตำแหน่งฝ่ายขาย) */
export function canViewVisitReport(user) {
  return canViewService(user) || isSalesRole(user?.role);
}

/* อ่านทะเบียนไซต์/พื้นที่ **เพื่อผูกกับใบคำร้อง** — คนละคำถามกับ "เข้าโมดูลได้ไหม"
   🔴 ฝ่ายขายไม่ได้เข้าโมดูลธุรกิจบริการแล้ว (มติ 2026-08-30) แต่ฟอร์มใบประเมินพื้นที่
      ต้องกางรายการไซต์ของลูกค้าให้เขาเลือก และต้องอ่านพื้นที่เดิมของไซต์นั้นมาให้ติ๊ก
      ⇒ ถ้าปิดทางอ่านนี้ไปด้วย ฟอร์มจะว่างเปล่าโดยไม่มีข้อความบอกว่าทำไม
   ⚠️ **อ่านอย่างเดียว** — การ *แก้* ทะเบียนยังเป็นของฝ่าย TS (`canEditService`) */
export function canPickServiceSite(user) {
  return canViewService(user) || canCreateServiceSite(user);
}

/* ── อ่านทะเบียนไซต์/โซน/เครื่อง = ข้อมูลหลักของทุกฝ่าย (มติผู้ใช้ 2026-09-17) ─────
   ⭐ **ทะเบียนย้ายบ้านไป `/database` แล้ว** — กฎสามชั้นข้อ 3: ข้อมูลหลักที่ทุกฝ่ายใช้ร่วม
      อยู่ฐานข้อมูล ไม่ว่าใครผลิต (ทรงเดียวกับทะเบียนกลิ่น/สูตรที่ RD ผลิตแต่ทุกคนเปิดอ่านได้)
      ⇒ ฝ่ายขายต้องตอบลูกค้าได้ว่า "ตอนนี้มีเครื่องกี่เครื่อง ตั้งอยู่ที่ไหนบ้าง" โดยไม่ต้องถาม TS
   🔴 **กว้างเท่าทะเบียนสินค้า/ลูกค้าโดยตั้งใจ** — `products:view` อยู่ใน DEFAULT_CAPS
      (แทบทุกคนถือ) ซึ่งคือสิ่งที่ผู้ใช้สั่ง · ทะเบียนนี้ไม่มีราคาทุนเหมือนทะเบียนวัสดุ
      จึงไม่มีอะไรรั่วจากการเปิดกว้าง
   ⚠️ **อ่านอย่างเดียว** — แก้ทะเบียนยังเป็น `canEditService` (TS + แอดมิน) เหมือนเดิมทุกจุด
      และ **ห้ามเอาตัวนี้ไปใช้กับคิวงาน/นัด/รอบบริการ** ซึ่งเป็นงานของฝ่าย ไม่ใช่ข้อมูลหลัก
      (`requireService` ตัวเดิมยังคุมเส้นพวกนั้นอยู่) */
export function canViewServiceRegistry(user) {
  /* 🪤 ต้องกว้างกว่า `canPickServiceSite` เสมอ — เส้นที่ฟอร์มใบประเมินพื้นที่ใช้ถูกเปลี่ยนมาใช้
     ด่านนี้แทน · ถ้าตัวนี้แคบกว่าเมื่อไร ฟอร์มนั้นจะกางรายการไซต์ไม่ได้อีก (อาการเดิมที่
     `forRequestForm` เกิดมาเพื่อแก้) */
  if (canPickServiceSite(user)) return true;
  return canUser(user, 'products:view') || canUser(user, 'customers:view');
}
...
export function canImportServiceData(user) {
  /* ⚠️ ตั้งแต่โมดูลเป็นของฝ่าย TS เท่านั้น (มติ 2026-08-30) `canEditService` เหลือ
     **แอดมิน + ฝ่าย TS** อยู่แล้ว ⇒ ด่านนี้เหลือหน้าที่เดียวคือ "นำเข้าเป็นก้อน
     แคบกว่าการแก้รายใบ" — เขียนทีเดียวหลายร้อยแถวและย้อนกลับไม่ได้
     🔴 เจ้าหน้าที่ฝ่าย TS ทุกตำแหน่ง **รวมหัวหน้า** ก็นำเข้าไม่ได้ — เหลือแอดมินคนเดียว */
  if (!canEditService(user)) return false;
  return user?.role === 'admin';
}

/* คนที่ "ถูกมอบหมายให้เข้าไซต์" ได้ — **ฝ่ายบริการ TS เท่านั้น** (มติ 2026-08-30)

   🐞 บั๊กจริงบน prod (2026-07-31): ของเดิมกรองเฉพาะฝ่าย TS แต่ **ยังไม่มีบัญชีฝ่าย TS
   สักคน** → dropdown "ผู้รับผิดชอบ" ว่างเปล่า → ทุกนัดถูกบันทึกด้วย assigneeId = null
   → เมนูงานของฉันว่างตลอดกาล ⇒ ตอนนั้นจึงเปิดทีมขาย SV เป็นทางสำรอง
   ⭐ วันนี้ฝ่ายมีคนจริงครบห้าตำแหน่งแล้ว ทางสำรองนั้นถูกปิด — คนขายไม่ควรถูกมอบหมาย
   ให้ขับรถเข้าไซต์ และเขาไม่เห็นโมดูลนี้อีกแล้วด้วย */
export function canBeServiceAssignee(user) {
  /* ⚠️ **ฝ่าย TS เท่านั้น** (มติ 2026-08-30) — เดิมมีสาขาทีมขาย SV เป็นทางสำรอง
     เพราะตอนนั้นยังไม่มีบัญชีฝ่าย TS สักคน ดรอปดาวน์เลือกผู้รับผิดชอบจึงว่างเปล่า ·
     ตอนนี้ฝ่ายมีคนจริงแล้ว ทางสำรองนั้นกลายเป็นช่องที่ทำให้คนขายถูกมอบหมายให้เข้าไซต์ */
...
export function worksInSalesPipeline(user) {
  /* ⭐ **ฝ่ายที่มีบ้านของตัวเอง ไม่ทำงานในเปลือกงานขาย** (มติผู้ใช้ 2026-08-29)
     — เดิมเช็ค `!== 'FN'` ตรง ๆ ตอนฝ่ายบัญชีได้โมดูลของตัวเอง (2026-08-22) · พอฝ่าย
     R&D ขอแบบเดียวกัน การไล่เติมรหัสฝ่ายทีละตัวคือที่ที่ฝ่ายถัดไปจะตกหล่น
     ⇒ ถามจาก `homeSystemForUser` ซึ่งเป็นตัวเดียวกับที่ตัดสินว่าเปลือกไหนเป็นบ้านเขา
     ⚠️ **ตัดที่เมนู ไม่ใช่ที่ cap** — `salesplan:view` ยังอยู่ครบ ⇒ กดลิงก์จากใบคำร้อง
     ไปดีล/โครงการ/ใบสั่งขาย ยังเข้าได้เหมือนเดิม (กฎข้อ 7)
     ⚠️ admin คืน null ⇒ ยังเห็นทุกเมนูตามเดิม */
  return homeSystemForUser(user) === null;
}
...
export function homeSystemForUser(user) {
  ...
  /* ⭐ ฝ่ายบริการ TS (มติผู้ใช้ 2026-08-31) — เพิ่มทีหลังเพราะ **ตกหล่นมาตั้งแต่แรก**:
     โมดูล `/service` เป็นบ้านของฝ่ายนี้ตั้งแต่ 2026-08-28 แต่ไม่เคยถูกลงทะเบียนตรงนี้
     ⇒ ไม่มีใครเห็นเพราะ TS ยังไม่มี `salesplan:view` เมนูงานขายเลยไม่ขึ้นอยู่ดี ·
     พอเปิดสิทธิ์อ่านใบสั่งขายให้ TS (มติ 2026-08-30) ช่องโหว่นี้โผล่ทันที:
     ฝ่ายที่มีบ้านของตัวเองได้เมนู ภาพรวม/ดีล/โครงการ ของงานขายมาทั้งชุด
     ⚠️ นี่คืออาการที่คอมเมนต์ของ `worksInSalesPipeline` เตือนไว้เองว่าจะเกิด
        ("การไล่เติมรหัสฝ่ายทีละตัวคือที่ที่ฝ่ายถัดไปจะตกหล่น") */
  if (dept === SERVICE_DEPARTMENT && canViewService(user)) return 'service';
  return null;
}
