...
// แก้ไซต์/เครื่อง/รอบ/นัด — เจ้าหน้าที่ฝ่าย TS หรือคนขายทีม SV
// มติผู้ใช้ 2026-07-30: เจ้าหน้าที่เห็น/แก้งานของ **ทีม SV ทั้งหมด** ไม่ใช่แค่นัดที่ตัวเอง
// ถูกมอบหมาย — เจ้าหน้าที่สลับกันไปแทนกันเป็นเรื่องปกติ ถ้าล็อกรายคนงานจะค้างทันทีที่คนลา
...
export function canImportServiceData(user) {
  ...
  if (!isSuperuser(user?.role)) return false;           // เจ้าหน้าที่/คนขายรายคนไม่ได้
  return departmentOf(user) === SERVICE_DEPARTMENT || hasTeam(user, SERVICE_SALES_TEAM);
}

// คนที่ "ถูกมอบหมายให้เข้าไซต์" ได้ — ฝ่ายบริการ TS **หรือ** ทีมขาย SV
//
// 🐞 บั๊กจริงบน prod (2026-07-31): ของเดิมกรองเฉพาะฝ่าย TS แต่ **ยังไม่มีบัญชี
// ฝ่าย TS สักคน** (23 ผู้ใช้ ไม่มี TS เลย) → dropdown "เจ้าหน้าที่ผู้รับผิดชอบ" ว่างเปล่า
