...
// ฝ่ายขายเปิดใบ → กางบรรทัดต้นทุนจากแม่แบบของประเภทสินค้า (mig 0140) → ผูกวัสดุ
// จาก **ทะเบียนวัสดุ** ด้วย id แล้วดึงราคามาตรึงเป็น snapshot (mig 0157–0159;
// ราคาที่ยังไม่มี/เกินอายุ เปิด "เคสขอราคาวัสดุ" ให้ RD/PC ตอบ) → ผู้บริหาร
// อนุมัติราคาผลิต **รายสินค้า** → ป้อนกลับเป็นต้นทุน FG
//
...
// ราคาวัสดุมาจากทะเบียน (เซลดึงเอง) จึงรับ status 'draft' ได้. ส่ง libraryBlocker
// (จาก libraryPricingBlocker) มาเพื่อบล็อกบรรทัดที่ยังไม่ผูกวัสดุ/ราคาเกินอายุ —
// ถ้าไม่ส่งมาจะข้ามการเช็คทะเบียน (หน้าจอเช็คคร่าว ๆ ก่อนยิง, ด่านจริงอยู่ที่ API)
//
// 0159: 'pricing' = มีเคสขอราคาวัสดุค้างอยู่ (ไม่ใช่ธง legacy อีกแล้ว) → ส่งไม่ได้
export function submitToExecError(request, libraryBlocker = null) {
  if (!request) return 'ไม่พบใบขอราคา';
  if (request.status === 'pricing') {
    return 'ยังมีเคสขอราคาวัสดุค้างอยู่ — รอฝ่าย RD/PC ตอบก่อนส่งผู้บริหาร';
  }
  if (!['draft', 'assembling', 'returned'].includes(request.status)) {
  ...
  // บรรทัดที่ยังไม่ผูกวัสดุ / ราคาเกินอายุ / กรัมว่าง (ส่งผลลัพธ์สำเร็จรูปมา)
...
//
// ⚠️ **ออกเลขพร้อม UPDATE ในทรานแซกชันเดียว** (mig 0240) — ห้ามกลับไปจองเลขแล้วค่อย
