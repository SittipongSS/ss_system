...
/* ══ กดบันทึกจากแผ่นปิดงาน — ส่ง `stamp` หรือไม่ ═══════════════════════
 *
 * 🐞 **ปุ่ม "แก้ผลการเข้า" กดแล้วได้ 409 ทุกครั้ง 100%** — เซตของใบที่ปุ่มขึ้น
 *   (`isClosedVisit`) กับเซตของใบที่ server ปฏิเสธ **เป็นเซตเดียวกันเป๊ะ**
 *   (route: `if (body.stamp && isClosedVisit(before)) return conflict(...)`)
 *   ⇒ ไม่มีเคสไหนสำเร็จเลย · ช่างที่ลืมแนบรูป/ลืมให้ลูกค้าเซ็น กลับมาเติมไม่ได้สักใบ
 *   และ **ไม่มีจอไหนในทั้งเว็บเติมได้** (ลายเซ็นเขียนได้จากแผ่นนี้ที่เดียว · ฟอร์มแก้นัด
 *   ที่ข้อความ error ชี้ไป ไม่มีช่องรูป/ลายเซ็นเลย และช่างก็เปิดหน้านั้นไม่ได้)
 *
 * 🔴 **ยิ่งกว่านั้น: มันเขียนไปครึ่งทางแล้วค่อยพัง** — แผ่นปิดงานยิง `PUT .../assets`
 *   ก่อนเสมอ และเส้นนั้นไม่มีด่านใบปิด ⇒ ผลรายเครื่องถูกลบ+เขียนทับจริง แล้ว PATCH
 *   ค่อย 409 ⇒ ผู้ใช้เห็น "ไม่สำเร็จ" แต่ของเปลี่ยนไปแล้ว และสถานะบนใบค้างค่าเก่า
 *
 * ⭐ **`stamp` แปลว่า "ประทับเวลาจบให้ด้วย" ไม่ใช่ "บันทึก"** — การกลับมาแก้ผลของใบที่
 *   ปิดไปแล้วไม่ใช่การจบงาน ⇒ ไม่ต้องประทับ · ด่านฝั่ง server ถูกแล้วและต้องอยู่ต่อ
 *   (มันกันเวลาจริงไม่ให้ถูกเขียนทับ) — ที่ผิดคือจอส่ง `stamp` มาทุกครั้ง
 */
export function closeVisitPayload(visit, form = {}) {
  return isClosedVisit(visit) ? { ...form } : { ...form, stamp: 'end' };
}
