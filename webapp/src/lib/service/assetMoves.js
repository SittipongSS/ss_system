...
export const MOVE_LABELS = {
  /* 🪤 **กิ่งที่ตายแล้ว แต่ถอดออกไม่ได้** — ไม่มีโค้ดไหนสร้างคำสั่งนี้อีกตั้งแต่ลบ
     เส้น "รับเครื่องเข้าคลัง" (2026-09-06) และ `assetMoveOwnerError` ก็ซ่อนปุ่มอยู่แล้ว
     ⇒ ปล่อยไว้เพราะ **CHECK ของ `service_asset_moves.kind` (mig 0335) ยังลิสต์มันอยู่**
       และเทสต์บังคับให้สองลิสต์ตรงกัน · ถอดออกต้องมี migration แก้ CHECK คู่กัน
     (ตรวจ 2026-09-06: ยังไม่มีแถวประวัติไหนใช้ kind นี้เลย — ถอดได้จริงถ้าจะทำ) */
  receive: 'รับเข้าคลัง',
...
/* ── ช่องที่ฟอร์มแก้ข้อมูลเครื่อง **แตะไม่ได้** — เป็นผลของคำสั่ง ไม่ใช่ค่าที่พิมพ์ ──
   🔴 **สองเหตุผล ไม่ใช่แค่ความสวยงามของสถาปัตยกรรม**
     · เลือก "ว่าง" (`in_stock`) จากฟอร์มของเครื่องที่อยู่ในไซต์ = ผิด CHECK
       `service_assets_place_by_status` (mig 0344: ว่าง ⇒ ต้องไม่มีไซต์)
       ⇒ **500 ดิบทุกครั้ง ไม่มีทางสำเร็จ** — ปุ่มที่กดแล้วไม่มีวันสำเร็จ
     · อีกสามค่าเป็นทางลัดปลดระวาง/ส่งซ่อม **ที่ไม่มีเหตุผลและไม่มีประวัติ** แข่งกับ
...
/** คืนข้อความไทยเมื่อ body พยายามเปลี่ยนช่องที่เป็นของคำสั่ง · null = ผ่าน */
export function assetFormLockError(before = {}, body = {}) {
  for (const [field, label] of MOVE_ONLY_FIELDS) {
