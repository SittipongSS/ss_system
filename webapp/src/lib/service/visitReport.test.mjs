...
/* 🔄 **ทุกชนิดนับรายตัวแล้ว** (มติผู้ใช้ 2026-09-03) — เดิมชนิด "แถวรวมทั้งชุด"
   (reed/สบู่/แอลกอฮอล์) ต่อท้าย spec ด้วย "N จุด" · แนวคิดนั้นถูกถอดทิ้งทั้งอัน
   ⇒ ยามกลับด้าน: **ห้าม**มีคำว่า "จุด" ในบรรทัดของชนิดไหนอีก */
test('ไม่มีชนิดไหนโชว์จำนวนจุดอีกแล้ว — ทุกชนิดนับรายตัว', () => {
  for (const line of build().lines) {
    assert.ok(!/\d+ จุด/.test(line.spec), `${line.assetId} ยังโชว์จำนวนจุดอยู่`);
  }
});
