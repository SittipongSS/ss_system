...
export function isAssetOnSite(asset) {
  /* ⚠️ **ต้องมีไซต์จริงด้วย** (mig 0344) — ก่อนหน้านี้ `siteId` เป็น NOT NULL
     ⇒ "ไม่ใช่ removed/in_stock" แปลว่า "อยู่หน้างาน" โดยอัตโนมัติ
     พอเครื่องไม่มีไซต์ได้ ข้อสมมตินั้นพัง: เครื่องสถานะ **ซ่อม** ที่ยังไม่เคยติดตั้ง
     (ส่งซ่อมจากของที่ว่างอยู่) จะถูกนับเป็นของหน้างานทั้งห้าจุดที่ถามฟังก์ชันนี้ —
     🐞 เจอตอน UAT 2026-09-06: การ์ดสรุปขึ้น "ที่ไซต์ลูกค้า 1" ทั้งที่ไม่มีเครื่อง
        ตัวไหนอยู่ไซต์เลย · และ `indexAssetCounts` จะได้คีย์ `null|...` เป็นขยะ */
  return asset?.status !== 'removed' && asset?.status !== 'in_stock' && !!asset?.siteId;
}
...
/* ── ที่อยู่ของไซต์แบบมีโครงสร้าง (มติผู้ใช้ 2026-09-24 · mig 0383) ─────────────────
...
/** ข้อความก้อนเดียวยุคก่อนแยกช่อง (ไซต์ ~160 แห่งที่คีย์ก่อน mig 0383) — ฟอร์มโชว์ข้อความเดิมครบ
...
/* ── ช่องที่ก๊อปได้จาก `customers.addresses[]` มาตั้งต้นไซต์ (mig 0313 · 0383) ────
...
/* ค่าที่ควรถูกก๊อปลงไซต์เมื่อกดไทล์ / "ดึงใหม่" — **ทะเบียนว่าง = ไม่แตะ**
   ...
   ⭐ **ที่อยู่ก๊อปทั้งชุดฟิลด์ย่อย** (mig 0383) — แถวทะเบียนที่แยกช่องแล้วได้ฟอร์มแยกช่องที่เลือก
...
/* ── ช่องที่บังคับ **เฉพาะตอนสร้างไซต์ใหม่** (มติผู้ใช้ 2026-09-21) ──────────
 *
 * ⭐ **ทำไมไม่อยู่ใน `normalizeSiteInput`** — ไซต์ยุคก่อน mig 0315 บนของจริงไม่มี
 *   ข้อมูลพวกนี้ครบ · ใส่ลงตัวตรวจรูปร่างเมื่อไร **แก้ช่องอื่นของไซต์เก่าไม่ได้เลย**
 *   ⇒ ด่านนี้เป็นของ "ใบใหม่" อย่างเดียว (แพตเทิร์นเดียวกับจังหวัดที่มีอยู่ก่อนแล้ว)
 *
 * 🔴 **ทำไมบังคับ** — ไซต์คือที่ที่ช่างต้องขับรถไปให้ถึง · ไซต์ที่ไม่มีที่อยู่/หมุด/
 *   เบอร์คนหน้างาน แปลว่าวันนัดจริงช่างต้องโทรถามฝ่ายขายทีละใบ และช่วงเวลาที่
 *   อาคารให้เข้าเป็นตัวที่ตัดสินว่านัดนั้นเข้าได้จริงไหม (`evaluateVisitGate`)
 *
 * ⚠️ เรียกจาก **สองที่เท่านั้น**: โมดัลเพิ่มไซต์ (บอกก่อนกด) และ `POST /api/service/sites`
 *   (ยามจริง) — ข้อความเดียวกันคำต่อคำ
 */
export function siteCreateMissing(value = {}) {
  ...
  /* ⚠️ เวลาเข้าได้บังคับ **เป็นคู่** — มีแค่ข้างเดียวคือช่วงที่ไม่มีปลาย ซึ่งด่าน
     เข้าไซต์เอาไปตัดสินอะไรไม่ได้ · รูปแบบเวลาถูกตรวจไปแล้วที่ `normalizeSiteInput` */
  if (!has('accessFrom') || !has('accessTo')) {
    return 'ไซต์ใหม่ต้องระบุช่วงเวลาที่เข้าได้ทั้งเวลาเริ่มและเวลาสิ้นสุด';
  }
  return null;
}
...
export function normalizeSiteInput(body = {}) {
  ...
  for (const [field, label, max] of [
    ...
    ['address', 'ที่อยู่', 500],
    // ฟิลด์ย่อยของที่อยู่ (mig 0383) — ชื่อมาจากทะเบียนกรมการปกครองที่ฟอร์มเลือก
...
export function normalizeAssetInput(body = {}) {
  ...
  for (const [field, label2] of [
    ...
    /* วันรับเข้า (mig 0332) — **คนละช่องกับวันติดตั้ง** ชีตเก่ามีทั้งสองและต่างกันเป็นปี
       🔄 เดิมข้อความว่า "วันที่รับเข้าคลัง" — ตั้งแต่ mig 0344 คลังไม่ใช่ที่จอดของเครื่อง
          ที่ยังไม่ติดตั้งแล้ว ⇒ มันคือวันที่ **บริษัทได้เครื่องมา** เฉย ๆ */
