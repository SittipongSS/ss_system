...
export function buildDraft(row, map, { rowNumber = 0 } = {}) {
  ...
  if (!isBlankCell(kindCell)) {
    ...
    //   กระจายกลิ่นแล้วถูกนับเป็นเครื่อง จะบวมเข้าไปในภาระเจ้าหน้าที่ทุกวันหลังจากนั้น
  ...
  if (qty !== null && qty > 0) {
    if (perUnit && qty > MAX_ASSETS_PER_ROW) {
      ...
      //   ⇒ สร้างแถวเปล่าไว้รอเจ้าหน้าที่เติมรายละเอียดตอนเข้าไซต์ครั้งแรก
      if (serial && qty > 1) {
  ...
  return {
    ...
    /* ชั้นของโซนมาจากคอลัมน์ 'ชั้น' แถวเดียวกัน (เดิมใช้กับ *เครื่อง* อย่างเดียว) —
       ตั้งแต่ mig 0315 ชั้นเป็นช่องบังคับของโซน (เคยเป็นท่อนของรหัสโซนจนถึง mig 0384)
       ⚠️ **เซลล์เดียวไปสองปลายทางคนละกติกา**: `service_zones.floor` ถูก normalize
          เป็นค่ามาตรฐาน (`04` · `GF`) · ส่วน
       ...
       ⇒ ใบส่งงานจึงอาจเห็นสองรูปคู่กัน ซึ่งถูกต้อง: ชั้นของ *พื้นที่* กับที่ *เจ้าหน้าที่จดไว้* */
