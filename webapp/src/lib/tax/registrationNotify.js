// ── แจ้งเตือนข้ามเลน SA ↔ LG ของทะเบียนสรรพสามิต ──────────────────────────
//
...
// ทั้งสองกิ่ง ⇒ ตั้งแต่นั้นมา **ไม่มีใครรู้อะไรเลย**: ฝ่ายกฎหมายไม่รู้ว่ามีทะเบียนรอ
...
// ต่อจริง: คิวตรวจเป็นของ **ตำแหน่ง** (legal) ส่วนผลการตรวจส่งกลับ **เจ้าของใบ**
...
/* ตำแหน่งที่ "ตรวจอนุมัติทะเบียน" ได้ — ตรงกับด่าน `legal:approve` ของ route
   admin เป็น **ตัวสำรอง** ใช้เมื่อไม่มี legal ในระบบเลย: ไม่มีใครรับแจ้งเตือนคือ
...
export function registrationNotice({ action, registration, directory, actorId, reason } = {}) {
  ...
  if (action === 'submit') {
    ...
    return {
      ...
      body: [customer, 'เอกสารครบแล้ว รอฝ่ายกฎหมายตรวจอนุมัติ'].filter(Boolean).join(' — '),
    };
  }
