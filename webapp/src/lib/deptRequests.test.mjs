...
/* ── ลงคิวซ้ำเมื่อ "ใบมีวันแล้วแต่ไม่มีนัด" (เฟส 2 · รีเช็ค 2026-08-30) ──────
   🐞 ไม่มีธงนี้ = ใบค้างสถานะ "ลงคิวแล้วแต่เจ้าหน้าที่ไม่มีงาน" แล้วกู้ได้ทางเดียวคือ
      เลื่อนไปวันปลอมแล้วเลื่อนกลับ ⇒ เธรดมีแถว "เลื่อนวัน" ที่ไม่มีใครเลื่อนจริง */
test('🔴 requeue: ใบที่มีวันแล้วต้องกดลงคิวซ้ำด้วยวันเดิมได้', () => {
  const acked = {
    status: 'acknowledged', acknowledgedAt: '2026-08-30T02:00:00Z',
    committedDueDate: '2026-09-08', items: [],
  };
  // ปกติ: มีวันแล้วต้องไปทางเลื่อนวัน
  assert.match(
    commitDueRequestError(acked, { committedDueDate: '2026-09-08' }),
    /แจ้งกำหนดส่งไปแล้ว/,
  );
  // กู้: ผู้เรียกพิสูจน์แล้วว่านัดไม่มีจริง ⇒ วันเดิมผ่าน
  assert.equal(
    commitDueRequestError(acked, { committedDueDate: '2026-09-08', requeue: true }),
    null,
  );
  // ธงไม่ข้ามด่านอื่น — ยังไม่รับเรื่องก็ยังลงคิวไม่ได้
  assert.match(
    commitDueRequestError({ status: 'pending', items: [] }, { committedDueDate: '2026-09-08', requeue: true }),
    /ยังไม่ได้รับเรื่อง/,
  );
});

/* ── แก้ใบ: เวลาที่ต้องการต้องถึง DB ────────────────────────────────────
   🐞 ช่อง "ช่วงเวลาที่ต้องการ" กางอยู่บนฟอร์มแก้ แต่ `requestEditPatch` ไม่มีคีย์นี้
      ⇒ ตัว route ต้องเป็นคนรับ · เทสต์นี้ปักไว้ว่าเส้นทางนั้นยังอยู่ (ตัวจริงคือ
      `normalizeSurveyTime` ที่ route เรียกเมื่อหัวข้อนั้นต้องการสถานที่) */
