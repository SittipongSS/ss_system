...
test('ฝ่ายโรงงาน/rd manage their OWN tasks without pm:edit (งานของฉันใช้ได้จริง)', async () => {
  const db = fakeSupabase();
  ...
  assert.equal(await canManagePersonalTask(db, task, { id: 'worker', role: 'pc', department: 'PC' }), true);
