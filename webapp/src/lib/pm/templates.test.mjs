...
test('NPD ใหม่เริ่มที่ Mock-up ไม่มีขั้นกลิ่น และยังมีขั้นสรรพสามิต', () => {
  ...
  assert.equal(excise.categoryOnly, '01-002');
...
test('buildProjectTasks gen ได้ทั้ง 3 ประเภท (smoke — ไม่มี dangling predecessor)', () => {
  ...
  for (const type of ['SCENT', 'NPD', 'RE-ORDER']) {
    const rows = buildProjectTasks({ type, productMainCategory: '01-002', startDate: '2026-07-13', aeOwner: 'AE' }, 'PRJ-test');
    assert.ok(rows.length > 0, `${type}: มี task`);
    const ids = new Set(rows.map((r) => r.id));
    for (const r of rows) {
      for (const p of r.predecessors || []) assert.ok(ids.has(p), `${type}: dangling predecessor ${p}`);
    }
    assert.equal(rows.length, templateFor(type).filter((t) => !t.categoryExclude || t.categoryExclude !== '01-002').length, `${type}: จำนวนตรง template (หมวด 01-002)`);
  }
});
