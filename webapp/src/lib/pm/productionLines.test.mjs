...
test('⭐ PC **อ่าน**ตารางผลิตได้ แต่วางคิวไม่ได้ (มติผู้ใช้ 2026-09-17 "PC เห็นได้")', () => {
  /* 🐞 จุดที่พลาดง่าย: เปิดฝั่งอ่านแล้วเผลอเปิดฝั่งแก้ตามไปด้วย — PC ถือ `production:edit`
     ที่ชั้น role อยู่แล้ว ตัวที่กั้นคือ PRODUCTION_PLANNER_DEPARTMENTS เท่านั้น
     ⚠️ ทางตันคือ "แก้ได้แต่เปิดหน้าไม่ได้" ไม่ใช่ทางกลับด้าน ⇒ อ่านได้อย่างเดียวไม่เป็นไร */
  assert.equal(canViewProduction(pc), true);
