...
// หมวดสินค้าจำลอง — สรรพสามิตตัดสินจากธง isExcise (mig 0131) ไม่ใช่รหัส 01-002 ตายตัว
const TYPES = [
  { mainCategoryCode: '01', typeCode: '002', isExcise: true },
  { mainCategoryCode: '02', typeCode: '001', isExcise: false },
  { mainCategoryCode: '03', typeCode: '005', isExcise: true }, // หมวดใหม่ที่เพิ่งติ๊ก
];
const related = (over = {}) => ({ productTypes: TYPES, ...over });
...
test('dealHasExciseFg: จริงเฉพาะเมื่อหมวดของ FG ติ๊ก isExcise', () => {
  assert.equal(dealHasExciseFg(deal({ metadata: { fgCodes: ['FG-A-01-002-1'] } }), [], TYPES), true);
  assert.equal(dealHasExciseFg(deal({ metadata: { fgCodes: ['FG-A-02-001-1'] } }), [], TYPES), false);
  // หมวดใหม่ที่ติ๊ก isExcise ก็เข้าข่าย — ไม่ผูกกับรหัส 01-002
  assert.equal(dealHasExciseFg(deal({ metadata: { fgCodes: ['FG-A-03-005-1'] } }), [], TYPES), true);
  // มาจาก project_products ก็นับ
  assert.equal(dealHasExciseFg(deal(), [{ product: { fgCode: '01-002' } }], TYPES), true);
  assert.equal(dealHasExciseFg(deal(), [], TYPES), false);
  // ไม่ส่งรายการหมวด → ไม่รู้จักหมวด → false (จงใจไม่มี fallback รหัสตายตัว)
  assert.equal(dealHasExciseFg(deal({ metadata: { fgCodes: ['FG-A-01-002-1'] } })), false);
});
...
test('Won ทำผ่านใบเสนอราคาเท่านั้น; Lost ยังทำได้จากดีลที่เปิด', () => {
  assert.equal(dealLifecycle(deal({ stage: 'lead' })).canGo, false);
...
test('nextAction ที่ stage สำคัญ', () => {
  assert.equal(dealLifecycle(deal({ stage: 'deposit_pending' })).nextAction.kind, 'quote');
...
test('route สรรพสามิต: มีเฉพาะดีลที่มี FG หมวดติ๊ก isExcise; locked ถ้ายังไม่มีโครงการ', () => {
  const noExcise = dealLifecycle(deal({ stage: 'won', metadata: { fgCodes: ['FG-A-02-001-1'] } }), related());
  assert.equal(exciseRoute(noExcise), undefined);

  const excNoProj = dealLifecycle(deal({ stage: 'won', metadata: { fgCodes: ['FG-A-01-002-1'] } }), related());
  assert.equal(exciseRoute(excNoProj).status, 'locked');

  // หมวดใหม่ที่ติ๊ก isExcise ได้การ์ดสรรพสามิตเหมือนกัน
  const newCat = dealLifecycle(deal({ stage: 'won', metadata: { fgCodes: ['FG-A-03-005-1'] } }), related());
  assert.equal(newCat.routes.some((r) => r.kind === 'excise' || r.kind.startsWith('excise:')), true);
});
...
test('สรรพสามิตราย FG: ทะเบียนยังไม่อนุมัติ → เปิดทะเบียนไปทำต่อ (ไม่สร้างซ้ำ)', () => {
  for (const st of ['draft', 'pending_legal', 'rejected']) {
    const lc = dealLifecycle(
      deal({ stage: 'in_project', projectId: 'P1', metadata: { fgCodes: ['FG-A-01-002-1'] } }),
      related({ exciseRegistrations: [{ id: 'R1', fgCode: 'FG-A-01-002-1', status: st }] }),
    );
    const r = exciseRoute(lc);
...
test('สรรพสามิตราย FG: อนุมัติแล้ว → ไปยื่นชำระ (ไม่สร้างทะเบียนซ้ำ)', () => {
  const lc = dealLifecycle(
    deal({ stage: 'in_project', projectId: 'P1', metadata: { fgCodes: ['FG-A-01-002-1'] } }),
    related({ exciseRegistrations: [{ id: 'R1', fgCode: 'FG-A-01-002-1', status: 'approved' }] }),
  );
  const r = exciseRoute(lc);
