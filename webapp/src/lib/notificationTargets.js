...
const HREF = {
  ...
  /* นัดเข้าบริการ → **ใบส่งงานของนัดนั้น** (/service/visits/[id])
     ⭐ ผู้รับหลักคือเจ้าหน้าที่เจ้าของนัด (`recipients` = assigneeId ใน updateAccess) ซึ่งเป็น
        role `ts` · ตรวจแล้วเปิดได้: proxy ปล่อย `/service` (OPEN_PAGES) · GET นัดผ่าน
        `requireVisit` → `canViewService` (ts ถือ service:view + ฝ่าย TS) · หน้า "งานวันนี้"
        ของเจ้าหน้าที่ก็ลิงก์มาหน้านี้อยู่แล้ว และปุ่มย้อนกลับพากลับ "งานวันนี้" ให้เอง
     🐞 เดิมพาไป `/service/schedule` เปล่า ⇒ ลงที่สัปดาห์ปัจจุบัน ต้องไล่หานัดเอง และเป็นหน้า
        จัดคิวที่ไม่อยู่ในเมนูของเจ้าหน้าที่
     ⚠️ หน้าใบส่งงานไม่มีเธรด — ข้อความของแจ้งเตือนอ่านได้ในกล่องเอง ส่วนเธรดเต็มยังอยู่ที่โมดัลนัด */
  service_visit: (id) => `/service/visits/${id}`,
  ...
  /* รอบบริการใกล้หมด (mig 0327) — ไม่มีหน้ารายละเอียดรายไซต์ฝั่งขาย และ "เรื่อง"
     ยังไม่เกิดจนกว่าจะมีคนกดรับ ⇒ พาไปที่ **ทะเบียน** ซึ่งเป็นที่ที่เปิดเรื่องได้จริง */
  service_renewal: () => '/sa/contracts?tab=renewals',
  /* เอกสารใบสเปคสินค้า FM-SA-04 (mig 0370) — แจ้งเตือนยื่น/อนุมัติ/ตีกลับพาไปที่หน้าเอกสาร
     ซึ่งเป็นที่ที่กดอนุมัติ/ตีกลับได้จริง (ไม่ใช่หน้าสเปคของสินค้า ที่ไม่มีด่านอนุมัติแล้ว) */
  product_spec_document: (id) => `/sales-planning/spec-documents/${id}`,
};
