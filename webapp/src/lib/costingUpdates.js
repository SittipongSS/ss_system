...
export function askActionUpdate(action, ask, {
  ...
  /* สรุปสั้น ๆ ที่ผู้เรียกอยากพ่วงท้ายบรรทัด "ตอบเรื่องแล้ว" — ใช้กับหัวข้อที่ผู้ขอ
     รอ *ตัวเลข* ไม่ใช่รอแค่คำว่าตอบแล้ว (ใบประเมินพื้นที่: กี่พื้นที่ กี่แพ็คเกจ) */
  summary = null,
  ...
  if (action === 'answer') {
    /* ⭐ ปุ่มนี้เป็น **ตราปิดฝั่งฝ่าย** (ปิดสองฝั่ง · 2026-08-20) — เธรดต้องบอกด้วยว่า
       ...
       ⭐ **ใบประเมินพื้นที่พ่วงตัวเลขมาด้วย** — ผู้ขอรอ "ตร.ม. กี่แพ็คเกจ" เพื่อเอาไป
         ตั้งราคา ⇒ อ่านจากกระดิ่งได้เลยโดยไม่ต้องเปิดใบ คือสิ่งที่เขารออยู่จริง ๆ
         ⚠️ ผู้เรียกเป็นคนส่ง `summary` มา (route ที่รู้ตัวเลข) — ไฟล์นี้ไม่คำนวณเอง
            เพราะตัวเลขอยู่ในตารางลูกที่ตรรกะเธรดไม่ควรรู้จัก */
    return {
      kind: 'answer',
      body: `${dept} ตอบเรื่องนี้แล้ว`
        + (clip(summary, 200) ? ` — ${clip(summary, 200)}` : '')
        + (ask.closedAt ? ' — ปิดครบสองฝั่ง' : ' — รอผู้ขอปิดเรื่อง'),
      meta: { dept },
    };
  }
  /* ฝ่ายปิดใบโดยไม่ได้ผล (§5E ③) — ต้องบอก **เหตุผล** เสมอ เพราะผู้ขอเป็นคนรอผล
     แล้วจู่ ๆ ใบจบโดยไม่มีผล ⇒ ไม่มีเหตุผล = ใบที่ตอบลูกค้าไม่ได้ */
  if (action === 'close-unassessed') {
    return {
      kind: 'close_unassessed',
      body: `${dept} ปิดใบโดยไม่ได้ผล — ${clip(reason) || 'ไม่ระบุเหตุผล'}`,
      meta: { dept },
    };
  }
  ...
  if (action === 'update') {
    return {
      kind: 'update',
      body: variantChange
        ? `ผู้ขอสลับรูปแบบงานเป็น ${variantChange} — ตรวจรายละเอียดอีกครั้งก่อนรับเรื่อง`
        : 'ผู้ขอแก้ข้อมูลคำร้อง — ตรวจรายละเอียดอีกครั้งก่อนรับเรื่อง',
      meta: {},
    };
  }
