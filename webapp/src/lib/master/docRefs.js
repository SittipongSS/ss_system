...
export const DOC_REF_TYPES = {
  ...
  ST: { label: 'ไซต์บริการ', table: 'service_sites', column: 'code', body: '\\d{4}-\\d{2}-[A-Z]{3}-\\d{4}', example: 'ST-0032-01-BKK-1005', path: (id) => `/database/sites/${id}` },
  ...
  /* ใบสเปคสินค้า FM-SA-04 (mig 0370 · มติ 21/09/2569) — เลขที่ `FM-SA-04-DDMMYY-XXX` ได้ตอน AC
     ออกเอกสารจากบรรทัด SO และถูกอ้างในอีเมล/แชตโดยไม่มีหัวเอกสารติดไปด้วย (เหตุผลที่เลขมีรหัส
     แบบฟอร์มอยู่ในตัว — lib/sales/productSpecDocNo.js)
     ⚠️ คำนำหน้าของทะเบียนนี้คือสองตัวอักษรแรก ⇒ `FM` ทั้งตระกูลชี้ตารางนี้ตารางเดียว · `body`
        ล็อก `SA-04-` ไว้ ⇒ รหัสแบบฟอร์มลอย ๆ (`FM-SA-04` · `FM-RD-01`) ไม่กลายเป็นลิงก์
     🪤 วันที่แบบฟอร์มอื่นออกเลข `FM-…` ของตัวเอง (เช่น FM-SA-07) ต้องรื้อให้คำนำหน้าหนึ่งตัว
        ชี้ได้หลายตาราง — เติม `|` ใน body เฉย ๆ จะพาเลขของใบนั้นมาค้นในตารางนี้แล้วเจอ "ไม่พบเอกสาร"
     ⚠️ ไม่รับ Rev ต่อท้าย — เลขในฐานไม่เปลี่ยนตาม Rev · เลขบนกระดาษ/จอ `DDMMYY-XXX-RR` (formatSpecDocNo)
        ไม่มีคำนำหน้าจึงไม่ลิงก์ (รูปเดียวกับเลขอ้างอิง PDR) ⇒ จอโชว์เลขเต็มคู่กันเสมอ (การ์ดหน้า SO · eyebrow หน้าเอกสาร) */
  FM: { label: 'รายละเอียดผลิตภัณฑ์ (FM-SA-04)', table: 'product_spec_documents', column: 'docNo', body: 'SA-04-\\d{6}-\\d{3,}', example: 'FM-SA-04-220969-001', path: (id) => `/sales-planning/spec-documents/${id}` },
