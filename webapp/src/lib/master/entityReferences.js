  ...
  product: {
    ...
    tables: [
      ...
      /* ── ใบสเปคสินค้า FM-SA-04 — โมเดลเอกสารชุดใหม่ (mig 0370 · มติ 21/09/2569) ──
         สเปคเป็นข้อมูลของสินค้า (1 แถวต่อสินค้า ไม่มีเลข ไม่มี Rev) ส่วนเลขที่ Rev และด่าน
         อนุมัติเป็นของ **เอกสาร** ที่ AC ออกจากบรรทัด SO · ตาราง Rev/การออกของโมเดลเดิม
         (product_spec_revisions · product_spec_issues) ถูกถอดใน 0370 และไม่เคยถือ productId
         ⚠️ ลูกของสองตารางนี้ (`product_spec_items` · `product_spec_document_revisions`) ไม่ถือ
            productId — ชี้พ่อด้วย specId/documentId ⇒ ไม่ต้องอยู่ในทะเบียน

         สเปค — **บล็อกการลบ ไม่ใช่กวาดทิ้ง** · FK `productId` ยังเป็น CASCADE จาก 0364
         ⇒ ถ้าไม่มีบรรทัดนี้ ลบสินค้าแล้วเนื้อสเปค checklist และใบรับรองหายไปพร้อมกันเงียบ ๆ
         ⚠️ สเปคที่ยังไม่เคยออกเอกสารก็บล็อกด้วย เพราะทะเบียนนี้นับรายตารางไม่มีเงื่อนไข —
            ทางออกคือลบสเปคที่หน้าสเปคของสินค้าก่อน (ลบได้เฉพาะตอนยังไม่มีเอกสารสักใบ) */
      { table: 'product_specs', label: 'สเปคสินค้า (FM-SA-04)', sample: null },
      /* เอกสารที่ออกจากบรรทัด SO — เลขที่ออกไปนอกบริษัทแล้ว บางใบลูกค้าเซ็นกลับมา ⇒ เป็น
         หลักฐานของสิ่งที่ตกลงกับลูกค้า ไม่ใช่สมุดของตัวสินค้าเอง (ต่างจาก `product_price_history`)
         ฐานกันไว้แล้วชั้นหนึ่ง (FK RESTRICT + trigger ห้ามลบแถว) แต่ด่านที่นี่คือที่ที่บอกผู้ใช้ว่า
