...
export const CUSTOMER_DOC_TYPES = {
  company: [
    ...
    // ⭐ **ไม่บังคับ** (มติผู้ใช้ 2026-08-22) — บัตรประชาชนกรรมการเป็นเอกสารของ
    // *ตัวบุคคล* ที่ลูกค้าหลายรายไม่ยอมส่ง (และเราไม่มีเหตุต้องถือไว้ทุกราย) ·
    // ตัวตนของนิติบุคคลยืนยันได้จากหนังสือรับรอง + ภ.พ.20 ซึ่งยังบังคับอยู่
    // ⇒ ยังแนบได้เหมือนเดิม แค่ไม่ปิดกั้นการอนุมัติลูกค้า
    // ⚠️ ยังอยู่ใน PERSONAL_DOC_TYPES เหมือนเดิม — "ไม่บังคับ" กับ "เป็นข้อมูล
    // ส่วนบุคคล" คนละเรื่องกัน ใบที่แนบมาแล้วต้องคุมการเห็นเท่าเดิม
    { key: "director_id_card", label: "สำเนาบัตรประชาชนกรรมการผู้มีอำนาจลงนาม", required: false },
...
// `canViewRecord` คืน true ทันทีเมื่อ resource เป็น `customers`/`products`
// (แคตตาล็อกกลาง เห็นได้ทุกทีม ตามมติ 2026-07-20) และ **ไฟล์แนบพลอยได้สิทธิ์
// เดียวกับตัวระเบียนไปด้วย** โดยไม่มีใครตัดสินใจเรื่องนี้แยก ⇒ สำเนาบัตรประชาชน
// กรรมการ (เอกสาร `required` ของลูกค้านิติบุคคล) · ทะเบียนบ้าน · Bookbank
// เปิดได้หมดทั้งบริษัท รวมบัญชีผู้สังเกตการณ์อ่านอย่างเดียว
//
// ⭐ **แยกตามประเภทเอกสาร ไม่ใช่จำกัดทั้งก้อน** (มติผู้ใช้ 2026-08-16) — เอกสารธุรกิจ
// ถูกใช้ข้ามฝ่ายจริง: หน้าทะเบียนสรรพสามิตดึงเอกสารลูกค้ามาแสดงเพื่อยื่นภาษี และ
// `ra:approve` **แก้ลูกค้าไม่ได้** ตามกติกาเดิม (permissions.js) ⇒ ถ้าจำกัดทั้งก้อน
// ตามทีมผู้ดูแล งานสรรพสามิตพังทันที · ที่ต้องกันคือ "เอกสารของบุคคล" ไม่ใช่ "ของบริษัท"
//
// ⚠️ เพิ่ม docType ใหม่ที่เป็นของบุคคล **ต้องมาเติมที่นี่** ไม่งั้นมันจะเปิดกว้างเงียบ ๆ
// เหมือนที่เกิดมาแล้ว — เทสต์ `attachmentPersonalDocs.test.mjs` คุมไว้ว่าทุกคีย์ใน
// `CUSTOMER_DOC_TYPES` ต้องถูกจัดกลุ่มไว้ฝั่งใดฝั่งหนึ่งอย่างชัดเจน
export const PERSONAL_DOC_TYPES = new Set([
  // ลูกค้านิติบุคคล — เอกสารของ "ตัวบุคคล" ที่แนบมาประกอบ
  'director_id_card',    // สำเนาบัตรประชาชนกรรมการ
  'director_house_reg',  // สำเนาทะเบียนบ้านกรรมการ
  'power_of_attorney',   // หนังสือมอบอำนาจ — มีเลขบัตร/ลายเซ็นของบุคคลอยู่ในนั้น
  // ลูกค้าบุคคลธรรมดา — ทั้งใบเป็นของบุคคลอยู่แล้ว
  'id_card',
  'house_reg',
  'name_change',
  // ใช้ร่วมทั้งสองประเภท
  'bank_book',           // เลขบัญชีธนาคาร = ข้อมูลการเงินส่วนบุคคล
]);

/** เอกสารใบนี้เป็นข้อมูลส่วนบุคคลหรือไม่ — ใช้ตัดสินทั้งการเห็นและการลง audit */
export const isPersonalDoc = (entityType, docType) => entityType === 'customer'
  && PERSONAL_DOC_TYPES.has(String(docType || ''));
...
export const ATTACHMENT_TYPES = {
  ...
  // ⭐ โครงการมีชนิดเดียว: **เอกสารร่วม** (Google Doc/Sheet) — ตั้งใจไม่เปิดให้อัป
  // ไฟล์นิ่ง เพราะไฟล์หลักฐาน (PO · มัดจำ) ต้องผูกดีลรายใบ ไม่ใช่ผูกโครงการ
  // โครงการเป็นที่รวมของหลายดีล แนบตรงนี้จะไม่รู้ว่าเป็นหลักฐานของดีลไหน
  // ⇒ ที่นี่มีได้แค่ของที่เป็นของ "ทั้งโครงการ" จริง ๆ: ร่างสเปก โน้ตประชุม ตารางเทียบ
  project: [
    { key: 'other', label: 'เอกสารร่วม' },
  ],
  // ⭐ สัญญา (mig 0278) — ไฟล์ที่ลูกค้าเซ็นแล้วสแกนกลับมา
  // การลงนามเกิดบนกระดาษ (มติผู้ใช้ 2026-08-20) ⇒ ไฟล์นี้คือ **หลักฐานเดียว**
  // ที่ตอบว่า "ฉบับเซ็นอยู่ไหน" · ระบบไม่ให้ปิดสถานะ "ลงนามแล้ว" ถ้ายังไม่มีไฟล์
  // ⭐ `external_doc` (mig 0322 · มติผู้ใช้ 2026-08-30) — เอกสารที่ **ใช้แทน** สัญญา
  //    (PO ลูกค้า · อีเมลยืนยัน · สัญญากระดาษฉบับเดิม) · แยกคีย์จาก `signed_contract`
  ...
  /* ⭐ ใบสั่งขาย — เอกสารที่ลูกค้าส่งมา **หลังออกใบ** (แท็บ "เอกสาร" · มติเจ้าของ 25/09/2569)
     ⚠️ ไม่ใช่ที่ของไฟล์ยืนยันคำสั่งซื้อ — ชุดนั้นอยู่ `sales_orders.confirmAttachments` (ด่านยื่นอนุมัติ ·
       ตรึงตอนอนุมัติ) · ที่นี่คือของที่มาเพิ่มทีหลังเท่านั้น
     ⚠️ ตัวแรกคือชนิดที่กล่องแนบแบบ inline ใช้ (ลากมาวาง/แนบไฟล์) — เปลี่ยนลำดับ = ไฟล์ใหม่เปลี่ยนป้าย */
  sales_order: [
    { key: 'customer_doc', label: 'เอกสารจากลูกค้า' },
    { key: 'other', label: 'เอกสารอื่นๆ' },
  ],
  ...
  //   tax_receipt  = ใบเสร็จกรมสรรพสามิต     — RA แนบหลังยื่นชำระจริง
  order: [
  ...
  // (ฝั่ง RA แนบ) จึงไม่ required. แผนที่บริษัท (address_map) เป็นเอกสารระดับ
...
//   • .heic/.heif — รูปจาก iPhone ตามค่าตั้งต้นของเครื่อง (เจ้าหน้าที่ถ่ายหน้างานส่งมาไม่ได้เลย)
