...
// แอดมินแก้เลข AR ของใบที่ **เผลอสร้างด้วยสวิตช์ระบบใหม่** เป็นเรื่องที่เกิดซ้ำ ๆ
// (ของจริงคือลูกค้าเก่าที่ต้องได้เลข 3 หลัก) · ถ้าไม่คืนเลข ทุกครั้งที่พลาด = เลข 4 หลัก
// ตายไปหนึ่งตัว ชุดเลขจะมีรูถาวรโดยไม่มีใครได้อะไรจากมันเลย
//
// ⚠️ **เงื่อนไขเดียวกับตอนลบร่างทิ้งเป๊ะ ๆ (trigger `customer_code_reclaim` ของ mig 0248)** —
// คืนเฉพาะใบที่ (ก) รหัสเดิมเป็นรูปที่ระบบออกให้ · (ข) **ไม่เคยผ่านอนุมัติ** (`firstApprovedAt`
// ว่าง) · ของที่เคยอนุมัติแล้วรหัสไปอยู่บนใบเสนอราคา/ใบสั่งขาย/ทะเบียนสรรพสามิต ⇒ ออกซ้ำ
// เมื่อไร เอกสารสองใบคนละเรื่องจะอ้างรหัสเดียวกัน · **ห้ามตัดสินจาก `approvalStatus`**
// เพราะการแก้ไขดีดใบที่เคยอนุมัติกลับเป็น pending (เหตุผลเต็มอยู่ที่ isReusableCode)
//
// คืนเลขเป็นแค่ "หย่อนกลับเข้ากอง" ไม่ได้ถอยเคาน์เตอร์ — ตัวออกรหัสหยิบจากกองก่อนเสมอ
// และเช็คซ้ำก่อนใช้ว่าเลขนั้นว่างจริง จึงไม่มีทางออกทับใบที่ถือเลขนั้นอยู่
export function reclaimableArNumber(record, { previousCode = record?.arCode } = {}) {
  if (!isAutoArCode(previousCode)) return null;   // รหัสกรอกเองไม่เคยกินเลขรัน ไม่มีอะไรให้คืน
  if (!isReusableCode(record)) return null;       // เคยอนุมัติ = เลขตายถาวร
  const no = Number(String(previousCode).trim().slice(AR_PREFIX.length));
