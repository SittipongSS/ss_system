// ── ชนิดรายการในเธรดอัปเดต (mig 0163) — โค้ดล้วน ไม่ต้อง migration ────────
// ตาราง entity_updates ไม่มี CHECK บน kind โดยเจตนา: ชุด kind เป็นของแต่ละ entity
// (งานมี due/late, ดีลมี call/meeting) การล็อกลง DB จะทำให้เพิ่ม entity ใหม่ต้อง
// ออก migration ทุกครั้ง — แพตเทิร์นเดียวกับ attachmentTypes / materialTypes
//
// ⚠️ ป้าย/สีของงานยกมาจาก UPDATE_META เดิมในหน้า pm/tasks แบบตรง ๆ — ผู้ใช้ต้อง
// ไม่รู้สึกว่าอะไรเปลี่ยนหลังย้ายมาใช้ของกลาง

// ── ธงต่อ kind ───────────────────────────────────────────────────────────
// `authorable: true` = คนเลือกชนิดนี้เองได้ตอนโพสต์ · ไม่ติดธง = ระบบเขียนให้
// เท่านั้น (ปล่อยให้ client ส่ง kind='status' มาเอง = ปลอมไทม์ไลน์ได้)
// `due: true`        = ชนิดนี้กรอก "กำหนดวัน" ได้ เก็บใน meta.dueDate
// `narrative: true`  = แถวนี้พก **ข้อความที่คนพิมพ์เอง** (เหตุผลที่ตีกลับ · ทำไมเลื่อนวัน
//                      · ลูกค้าว่าอะไร) ⇒ อยู่ในบทสนทนา · ไม่ติดธง = เปลี่ยนสถานะล้วน
//                      ซึ่งเป็น log ของระบบ
//
// ⭐ **ทำไมต้องแยกสองอย่างนี้** (ผลตรวจ 2026-08-17 · นับจริงทั้งระบบ): เธรดของคำร้อง
// 132 แถว เป็นข้อความคน 33 (25%) เหตุการณ์ระบบ 99 (75%) · **16 ใบจาก 32 ไม่มีข้อความ
// คนสักแถว** ⇒ การ์ดที่พาดหัวว่า "พูดคุยในคำร้องนี้" เป็น log ล้วน 100% บนครึ่งหนึ่ง
// ของใบทั้งระบบ
//
// ⚠️ เส้นแบ่งคือ **"มีคนพิมพ์อะไรลงไปไหม" ไม่ใช่ "ระบบเขียนไหม"** — ม-49 ตั้งขึ้นเพราะ
// *เหตุผล* หายไปกับสาย ซึ่งจริงเฉพาะแถวที่พกข้อความของคน · แถว "แก้ข้อมูลคำร้อง" ·
// "มอบหมายผู้รับผิดชอบ" · "รับเรื่อง" ไม่มีเหตุผลของใครอยู่ในนั้นเลย
// ⚠️ **แยกตอนวาด ไม่ใช่ตอนเขียน** — ทุกชนิดยังลง `entity_updates` เหมือนเดิม
// เพราะแจ้งเตือนรายคนเกาะอยู่กับแถวเธรด (`appendUpdate → notifyThreadUpdate`)
// หยุดเขียนแถวเมื่อไร แจ้งเตือน "RD รับเรื่องแล้ว" ตายทันที
//
// ⚠️ ส่วนใหญ่มี authorable ตัวเดียวคือ comment — ชุดหลายชนิดมีเฉพาะฟีดดีลที่
// แยก โทร/ประชุม/อีเมล มาแต่เดิม (mig 0063) และผู้ใช้ยืนยันให้คงไว้ 2026-07-27
export const UPDATE_KINDS = {
  personal_task: {
    comment: { label: 'อัปเดต', color: 'var(--accent)', authorable: true },
    status: { label: 'เปลี่ยนสถานะ', color: 'var(--blue)' },
    due: { label: 'เลื่อนกำหนด', color: 'var(--amber)' },
    late: { label: 'สาเหตุที่เสร็จช้า', color: 'var(--red)' },
    // "รอคนอื่น" (mig 0266) — ม่วงชุดเดียวกับป้ายสถานะ ไม่ใช่แดง เพราะงานไม่ได้พัง
    // แค่ไม่ได้อยู่ในมือเรา
    blocked: { label: 'รออะไรอยู่', color: 'var(--purple)' },
    /* งานเปลี่ยนมือ (มติผู้ใช้ 2026-08-20) — สีกลาง ไม่ใช่น้ำเงิน เพราะเป็นการจัดคน
       ไม่ใช่ความคืบหน้าของงาน · ป้ายเดียวกับ `assign` ของคำร้อง
       ⚠️ **quiet โดยเจตนา** — การเด้งเป็นหน้าที่ของ `lib/pm/taskAssignNotify.js`
       ซึ่งเขียนคนละข้อความให้ "คนที่งานเข้ามือ" กับ "คนที่งานหลุดมือ" · ปล่อยให้
       เธรดเด้งด้วยจะได้สองใบต่อหนึ่งการกระทำ (บั๊กเดิม #1205) */
    assign: { label: 'มอบหมายงาน', color: 'var(--text-3)', quiet: true },
  },
  // เคสขอราคาวัสดุ (mig 0158) — เธรดสองฝ่าย: เซลถาม ↔ RD/PC ตอบ
  // ป้าย 'ข้อความ' ไม่ใช่ 'อัปเดต' เพราะที่นี่คนคุยกันจริง ไม่ใช่รายงานความคืบหน้า
  dept_request: {
    comment: { label: 'ข้อความ', color: 'var(--accent)', authorable: true, narrative: true },
    submit: { label: 'ส่งเคส', color: 'var(--blue)' },
    acknowledge: { label: 'รับเรื่อง', color: 'var(--blue)' },
    // ตีกลับ = ผู้รับเรื่องส่งคืนผู้ยื่น (mig 0209) — สีเหลือง ไม่ใช่แดง เพราะเป็น
    // "ต้องแก้แล้วส่งใหม่" ไม่ใช่ "จบแล้ว" (แดงสงวนให้ยกเลิก/ปฏิเสธถาวร)
    bounce: { label: 'ตีกลับให้แก้ไข', color: 'var(--amber)', narrative: true },
    // ── สี่เหตุการณ์ที่เคยไม่มีที่ลง (แก้ 2026-08-10) ────────────────────
    // ⚠️ ต้องเติม **ที่นี่และใน `askActionUpdate` พร้อมกัน** — ขาดที่ใดที่หนึ่ง
    // เหตุการณ์จะเงียบบนจอตามยามที่เตือนไว้ข้างล่าง
    // เลื่อนกำหนด = อำพัน (เรื่องที่ต้องรู้ ไม่ใช่เรื่องที่พัง) ชุดสีเดียวกับ `due`
    // ของงาน และกับ `bounce` ที่แปลว่า "ยังไม่จบ"
    reschedule: { label: 'เลื่อนวันกำหนดส่ง', color: 'var(--amber)', narrative: true },
    /* ⭐ **แจ้งกำหนดส่ง = ก้าวของตัวเอง** (มติผู้ใช้ 2026-08-19) — รับเรื่องคือการ
       ตัดรอบเข้าฝ่าย · วันที่รับปากมาทีหลังได้เมื่อฝ่ายรู้จริง ⇒ ต้องมีแถวของตัวเอง
       ในเธรด ไม่งั้นฝ่ายขายไม่มีวินาทีไหนเลยที่รู้ว่า "ได้วันแล้ว"
       ⚠️ narrative — เป็นคำสัญญาที่ให้ผู้ขอ ไม่ใช่ log ของระบบ */
    commitDue: { label: 'แจ้งกำหนดส่ง', color: 'var(--blue)', narrative: true },
    /* ⭐ **ยังไม่จบ** (มติผู้ใช้ 2026-08-20 · ปิดสองฝั่ง) — ถอนตราปิดของฝั่งที่กดไปแล้ว
       ⚠️ อำพันเหมือน `bounce`/`reschedule`: "ยังไม่จบ" ไม่ใช่ความผิดพลาด แต่เป็นเรื่อง
       ที่อีกฝั่งต้องรู้ทันที · narrative เพราะเป็นคำของคน ไม่ใช่ log ของระบบ */
    reopen: { label: 'ยังไม่จบ', color: 'var(--amber)', narrative: true },
    /* ⭐ **ตราปิดหลุดเพราะอีกฝั่งพิมพ์ในเธรด** (ม-145 · มติผู้ใช้ 2026-09-11 "คงกติกาเดิม
       แต่บันทึกให้เห็น") — ของเดิมหลุดเงียบ ⇒ บรรทัด "ปิดเคส" เดิมยังเป็นบรรทัดปิดล่าสุด
       ในเธรด ขณะที่ตารางบอกว่ายังไม่ปิด
       ⚠️ **quiet** — เขียนคู่กับข้อความของคนที่เพิ่งพิมพ์เสมอ ซึ่งเด้งแจ้งเตือนไปแล้ว
       (กติกาของ `isQuietUpdateKind`) · อำพันเหมือน `reopen` เพราะความหมายเดียวกัน */
    closure_cleared: { label: 'ถอนการปิด', color: 'var(--amber)', quiet: true },
    // ⚠️ **ป้ายของอดีต — ไม่มีใครสร้างแถวชนิดนี้แล้ว** ประตูหัวหน้าสายงานขาย
    // (mig 0216) ถูกถอดออกทั้งขั้น (มติผู้ใช้ 2026-08-16) · เก็บป้ายไว้เพราะเธรดของ
    // ใบที่เคยยืนยันไว้ยังมีแถวนี้อยู่จริง — ถอดป้ายแล้วแถวเก่าจะไปหยิบ meta ของ
    // ชนิดอื่นมาใช้เงียบ ๆ ตามที่เตือนไว้ข้างบน
    approve: { label: 'ยืนยันให้ดำเนินการ', color: 'var(--blue)' },
    /* มอบหมายให้คนในฝ่าย (mig 0230) — **สีกลาง ไม่ใช่น้ำเงิน** เพราะเป็นการจัดคน
       ภายในฝ่าย ไม่ใช่ก้าวของเรื่องที่ผู้ขอต้องตื่นเต้นด้วย · แต่ต้องลงเธรดเพราะ
       "ทำไมงานย้ายมือ" เป็นคำถามที่เกิดจริงตอนตามงาน */
    assign: { label: 'มอบหมายผู้รับผิดชอบ', color: 'var(--text-3)' },
    // สองอันนี้เป็น "เนื้อในเปลี่ยน" ไม่ใช่ก้าวของเรื่อง จึงเป็นสีกลาง
    update: { label: 'แก้ข้อมูลคำร้อง', color: 'var(--text-3)' },
    /* ⭐ ทะเบียนกลิ่น/สูตรที่ใบนี้อ้างถึงถูกแก้ (มติผู้ใช้ 2026-08-18) — ตารางโชว์ค่า
       **สด** จากทะเบียนอยู่แล้ว ⇒ สิ่งที่ประวัติต้องเก็บคือ *ค่าเดิม* และใครแก้
       ⚠️ เขียนจากฝั่งทะเบียน (`/api/master/scents|formulas/[id]`) ทางเดียว ⇒ แก้จาก
       หน้าทะเบียนหรือจากโมดัลในใบ ก็ลงประวัติของทุกใบที่อ้างถึงเหมือนกัน */
    registry: { label: 'แก้ทะเบียนกลิ่น/สูตร', color: 'var(--text-3)' },
    pdr: { label: 'แก้แบบฟอร์ม PDR', color: 'var(--text-3)' },
    quoted: { label: 'ตอบราคา', color: 'var(--green)', narrative: true },
    no_quote: { label: 'ตอบไม่ได้', color: 'var(--amber)', narrative: true },
    // ── ก้าวของแถว (mig 0202) ──────────────────────────────────────────
    // ⚠️ ต้องลงทะเบียนที่นี่ ไม่งั้น `updateKindMeta` ถอยไปใช้ meta ของชนิดอื่น
    // แล้วเหตุการณ์จะ **เงียบสนิทบนจอ** ทั้งที่แถวถูกบันทึกจริง (ดูยามใน appendUpdate
    // ซึ่งเตือนอย่างเดียว ไม่ตีกลับ — แถวประวัติหายไปเลยแย่กว่าป้ายเพี้ยน)
    // "ส่งมอบแล้ว" = คำกลางของสามสาย (กลิ่น·สูตร·เอกสาร — ม-89) · เนื้อเหตุการณ์
    // บอกคำเฉพาะสายอยู่แล้ว ("ส่งเอกสาร — IFRA …" มาจาก hopLabelFor)
    ready: { label: 'ส่งมอบแล้ว', color: 'var(--blue)' },
    pickup: { label: 'รับของแล้ว', color: 'var(--blue)' },
    sent: { label: 'ส่งให้ลูกค้าแล้ว', color: 'var(--blue)' },
    confirmed: { label: 'ลูกค้าคอนเฟิร์ม', color: 'var(--green)' },
    revise: { label: 'ลูกค้าขอให้แก้', color: 'var(--amber)', narrative: true },
    rejected: { label: 'ลูกค้าไม่เอา', color: 'var(--red)', narrative: true },
    // ── สองก้าวจบของสายเอกสาร (ม-85) ────────────────────────────────
    received: { label: 'ได้รับเอกสารแล้ว', color: 'var(--green)' },
    refused: { label: 'ปฏิเสธเอกสาร', color: 'var(--red)', narrative: true },
    // ⭐ ฝ่ายดึงงานที่ส่งไปแล้วกลับมาแก้ (มติผู้ใช้ 2026-08-20 · แนบไฟล์ผิด) —
    // `narrative` เพราะมันคือคำพูดถึงผู้ขอ ("ไฟล์ที่โหลดไปเมื่อกี้ใช้ไม่ได้")
    // ไม่ใช่ log ของระบบ · เหตุผลที่ฝ่ายพิมพ์อยู่ในเนื้อเหตุการณ์
    unready: { label: 'ดึงงานกลับมาแก้', color: 'var(--amber)', narrative: true },
    /* 🐞 **ป้ายนี้หายไปตั้งแต่วันที่เพิ่ม `answer` เข้า `askActionUpdate`** (เจอ 06/09/2026)
       ⇒ ทุกฝ่าย (RD · PC · FN · TS) ตอบคำร้องใบไหนก็ตาม กระดิ่งของผู้ขอขึ้นว่า
         **"ข้อความ"** เหมือนมีคนพิมพ์คอมเมนต์มา เพราะชนิดที่ไม่มีทะเบียนถอยไปหยิบ
         ป้ายของ `comment` ⇒ คนกวาดกระดิ่งเร็ว ๆ อ่านผ่าน ทั้งที่เป็นก้าวสำคัญของใบ
       ⚠️ **`narrative: true`** — มันคือคำพูดของฝ่ายถึงผู้ขอ ("ตอบแล้ว รอคุณปิดเรื่อง")
         ไม่ใช่ log ของระบบ · ไม่ประกาศธงนี้ = หายไปตอนผู้ใช้กดซ่อนเหตุการณ์ระบบ
       ⚠️ คำต้องกลางพอสำหรับ **ทุกหัวข้อคำร้อง** (สอบถาม · พัฒนากลิ่น · ขอเอกสาร ·
         ใบวางบิล · ประเมินพื้นที่) ไม่ใช่คำเฉพาะของสายใดสายหนึ่ง */
    answer: { label: 'ตอบเรื่องแล้ว', color: 'var(--green)', narrative: true },
    /* ⭐ ช่างไปแล้วเข้าพื้นที่ไม่ได้ ⇒ ใบถอยกลับขั้นลงคิว (§5E ② · มติข้อ 23)
       อำพันเหมือน `reschedule`/`bounce` — "ยังไม่จบ ต้องทำอะไรต่อ" ไม่ใช่ "พัง"
       `narrative` เพราะเป็นคำอธิบายถึงผู้ขอว่าทำไมยังไม่ได้คำตอบ ไม่ใช่ log ของระบบ */
    unable: { label: 'เข้าพื้นที่ไม่ได้', color: 'var(--amber)', narrative: true },
    /* ฝ่ายปิดใบโดยไม่ได้ผล (§5E ③) — เทาเหมือน `close` เพราะมันคือการจบใบ
       แต่แยกชนิดเพราะ **คนละความหมาย**: `close` = ได้ของแล้วผู้ขอรับ · ตัวนี้ = ไม่ได้ของ */
    close_unassessed: { label: 'ปิดโดยไม่ได้ผล', color: 'var(--text-3)', narrative: true },
    /* 🔴 TS ดึงผลที่ส่งไปแล้วกลับมาแก้ (§5E ④) — **แดง ไม่ใช่อำพัน**
       เพราะ SA อาจเอาตัวเลขนั้นไปเสนอราคาไปแล้ว ⇒ ต้องสะดุดตาที่สุดในเธรด */
    recall: { label: 'ดึงผลกลับมาแก้', color: 'var(--red)', narrative: true },
    /* หัวหน้า TS แจ้งช่างให้กลับไปเก็บงานหน้างาน (แผน §5.4)
       ⚠️ **ผู้ขอเห็นบรรทัดนี้ด้วย และควรเห็น** — ผลที่เขารออยู่ช้าลงเพราะอะไร
       ⚠️ แต่บรรทัดนี้ไม่ใช่ตัวที่แจ้งช่าง (ช่างไม่อยู่ในทะเบียนผู้รับของ dept_request
         และเปิดหน้าคำร้องไม่ได้) ⇒ route ยิง notifyUsers แยกอีกทาง */
    send_back: { label: 'แจ้งช่างให้กลับไป', color: 'var(--amber)', narrative: true },
    /* ช่างแจ้งว่าแก้ตามที่หัวหน้าส่งกลับแล้ว (มติผู้ใช้ 2026-09-22) — ปิดวงของ `send_back`
       ⚠️ **quiet** — ตัวที่แจ้งหัวหน้าคือ `notifyUsers` ของ route (href ไปแท็บสรุปส่งผลที่เคาะได้)
         ปล่อยให้เธรดเด้งด้วย = หัวหน้าคนที่ส่งกลับ (past author) ได้สองเด้ง อันหนึ่งพาไปหน้าคำร้อง
       ⚠️ ผู้เขียนไม่ผูก `authorId` (ช่างเปิดหน้าคำร้องไม่ได้ — ดู service-crew-role-gates) */
    send_back_done: { label: 'ช่างแจ้งว่าแก้แล้ว', color: 'var(--green)', narrative: true, quiet: true },
    close: { label: 'ปิดเคส', color: 'var(--text-3)' },
    cancel: { label: 'ยกเลิกเคส', color: 'var(--red)', narrative: true },
    /* 🔴 ใบเสนอราคาที่คำร้องนี้อ้างถูกผู้อนุมัติยกเลิก (มติเจ้าของ 24/09) — **แดง** เพราะใบวางบิล/ใบกำกับ/
       ใบเสร็จที่ FN ออกใน Express จากใบนี้อาจค้างอยู่ และระบบไม่ยกเลิกเอกสารการเงินให้ (เตือน + แจ้ง ไม่บล็อก)
       ⚠️ **quiet** — ตัวที่แจ้งผู้ขอ + ผู้รับผิดชอบคือ `notifyUsers` ของเส้นยกเลิกใบ (lib/sales/quotationCancelRepo)
         ปล่อยให้เธรดเด้งด้วย = ผู้ขอได้สองเด้งต่อหนึ่งการกด · แถวนี้มีไว้ให้ประวัติของใบเล่าครบ
       ⚠️ narrative — พกเหตุผลที่ผู้อนุมัติพิมพ์ ไม่ใช่ log ของระบบล้วน */
    quotation_cancelled: { label: 'ใบเสนอราคาที่อ้างถูกยกเลิก', color: 'var(--red)', narrative: true, quiet: true },
  },
  // ── โครงการ ──────────────────────────────────────────────────────────
  // ⭐ เธรด**ระดับโครงการ**: เรื่องที่ยังเป็นจริงแม้ดีลใบใดใบหนึ่งจะถูกยกเลิกไป
  // (ลูกค้าเลื่อนส่งมอบทั้งล็อต · โรงงานแจ้งของเสีย · ตัดสินใจข้ามดีล) — ส่วนเรื่อง
  // ที่ตายไปพร้อมดีลต้องอยู่ในเธรดของดีลใบนั้น ไม่ใช่ที่นี่
  //
  // ความเคลื่อนไหวของดีลลูกไหลเข้ามาแสดงรวมในเส้นเรื่องเดียวกันผ่าน `extraItems`
  // (อ่านอย่างเดียว + ลิงก์เข้าดีล) — กล่องพิมพ์บนหน้าโครงการลงที่เธรดนี้เสมอ
  //
  // ⚠️ ชุดนี้ **เลือกมาแล้วว่าอะไรไม่เอา** (ดูเหตุผลเต็มใน lib/pm/projectUpdates.js):
  // ไม่เอาการติ๊กรับของรายชิ้น · ไม่เอาทุกขั้นตอนที่เปลี่ยนสถานะ · เอาเฉพาะเหตุการณ์
  // ที่เปลี่ยนทิศทางของโครงการ ไม่งั้นเธรดกลายเป็น log ของระบบงานแล้วข้อความคนจม
  project: {
    comment: { label: 'ข้อความ', color: 'var(--accent)', authorable: true },
    deal_link: { label: 'ผูกดีล', color: 'var(--blue)' },
    deal_unlink: { label: 'ถอดดีล', color: 'var(--amber)' },
    delivery: { label: 'ของเข้า', color: 'var(--teal)' },
    milestone: { label: 'หมุดหมาย', color: 'var(--violet)' },
    close: { label: 'สถานะโครงการ', color: 'var(--green)' },
  },
  // ฟีดความเคลื่อนไหวของดีล (ย้ายมาจาก sales_deal_activities, mig 0063 → 0169)
  //
  // ⭐ เธรดเดียวในระบบที่คนเลือกชนิดเองได้หลายแบบ — ป้าย/สี/ลำดับยกมาจาก
  // ACTIVITY_META เดิมในหน้าดีลตรง ๆ (ผู้ใช้ต้องไม่รู้สึกว่าอะไรเปลี่ยน) และ
  // ชื่อ kind ตรงกับ CHECK ของตารางเก่าทุกตัว จึงไม่ต้องแปลงตอน backfill
  //
  // ไม่มีชนิด 'comment' ที่นี่โดยเจตนา — ของเดิมใช้ 'note' เป็นค่าตั้งต้น
  // (updateKindMeta/defaultAuthorableKind ถอยไปหาตัวแรกที่ประกาศ ไม่ได้ผูกกับ 'comment')
  deal: {
    // ── สีของป้ายชนิด: จัดเป็น "กลุ่มความหมาย" ไม่ใช่ไล่สีไปเรื่อย ────────
    // บันทึก = ชนิดตั้งต้น (ข้อความส่วนใหญ่) จึงเป็นสีกลาง ไม่แย่งสายตา
    // โทร/ประชุม/อีเมล = กลุ่ม "ติดต่อลูกค้า" โทนเย็นด้วยกัน
    // ประสานงานภายใน = คุยกันเองในบริษัท จึงต้องแยกจากกลุ่มข้างบนให้เห็นชัด
    // ขั้นถัดไป = ตัวเตือนที่มีกำหนดวัน เด่นสุด
    // ⚠️ สีนี้ใช้กับ **ป้ายชนิด** เท่านั้น — แถบซ้ายของข้อความคนใช้ accent เสมอ
    // (ดู UpdateThread) ไม่งั้นข้อความคนจะเปลี่ยนสีไปมาตามชนิดจนอ่านไม่เป็นชุดเดียว
    note: { label: 'บันทึก', color: 'var(--text-2)', authorable: true },
    call: { label: 'โทร', color: 'var(--blue)', authorable: true },
    meeting: { label: 'ประชุม', color: 'var(--violet)', authorable: true },
    email: { label: 'อีเมล', color: 'var(--teal)', authorable: true },
    internal: { label: 'ประสานงานภายใน', color: 'var(--green)', authorable: true },
    next_step: { label: 'ขั้นถัดไป', color: 'var(--amber)', authorable: true, due: true },

    // ── เหตุการณ์ระบบของดีล (ไม่มีธง authorable — ระบบเขียนให้เท่านั้น) ──
    // ⭐ **จุดประสงค์ของเธรดดีลคือสมุดบันทึกความเคลื่อนไหวของดีล** (มติผู้ใช้) แต่
    // ความเคลื่อนไหวที่มีค่าที่สุด — ราคาที่เสนอไป ลูกค้ารับหรือตีกลับ มูลค่าที่ขยับ —
    // เคยอยู่แต่ในเธรดของ *ใบ* กับตาราง forecast คนเปิดดีลย้อนหลังจึงไม่เห็นอะไรเลย
    //
    // ⚠️ ตั้งชื่อชนิดตาม **ความหมาย ไม่ใช่แหล่งที่มา** (ไม่ใช่ `quotation`/`sales_order`)
    // เพราะสีของชนิดคือสิ่งที่ช่วยให้กวาดตาแล้วรู้ว่า "อันไหนคือปัญหา" — ตีกลับกับ
    // อนุมัติต้องคนละสี แม้จะมาจากใบใบเดียวกัน · เลขที่ใบอยู่ในเนื้อความ ซึ่ง
    // `RichText` ทำเป็นลิงก์ `/go/<รหัส>` ให้เอง
    doc_submit: { label: 'ยื่นขออนุมัติ', color: 'var(--blue)' },
    doc_approve: { label: 'อนุมัติแล้ว', color: 'var(--green)' },
    doc_return: { label: 'ตีกลับ', color: 'var(--red)' },
    doc_accept: { label: 'ลูกค้ารับใบ', color: 'var(--green)' },
    doc_revise: { label: 'ออก Rev.', color: 'var(--amber)' },
    doc_cancel: { label: 'ยกเลิกเอกสาร', color: 'var(--red)' },
    // ผู้ยื่นเอาใบคืนเอง / กู้ร่างคืน — ไม่ใช่ปัญหาแบบเดียวกับ `doc_return` (ตีกลับ)
    // จึงเป็นสีกลาง ไม่ใช่สีแดง (คำศัพท์ล็อกตามมติ: ดึงกลับ ≠ ตีกลับ)
    doc_withdraw: { label: 'ดึงกลับ', color: 'var(--text-3)' },
    /* ใบพี่น้องเปิดกลับตอนย้อนการรับ (มติ 25/09 · mig 0388) — หนึ่งใบหนึ่งแถว · สีกลางแบบดึงกลับ (ใบกลับมาใช้ได้ ไม่ใช่ปัญหา)
       ⚠️ **quiet** — เขียนคู่กับแถว "ย้อนการรับ" ของใบหลักเสมอ ซึ่งเด้งแจ้งเตือนไปแล้ว · ไม่เงียบ = ใบปิด N ใบ
       ได้กระดิ่งเรื่องเดียวกัน N+1 ครั้ง (กติกาเดียวกับ `closure_cleared`) */
    doc_reopen: { label: 'เปิดใบกลับ', color: 'var(--text-3)', quiet: true },
    forecast: { label: 'ประมาณการ', color: 'var(--amber)' },
    task: { label: 'งานของดีล', color: 'var(--text-3)' },
    /* 🐞 ตกทะเบียนมาตั้งแต่ต้น: `dealRequestUpdate` (lib/sales/dealUpdates.js) เขียน
       `kind: 'request'` ลงเธรดดีลตั้งแต่มติ 2026-08-03 แต่ไม่มีใครเติมชนิดตรงนี้
       ⇒ `updateKindMeta` ตกไปใช้ชนิดตั้งต้น (`note`) ⇒ **ป้ายขึ้นว่า "บันทึก" สีเทา
       ทั้งในเธรดและบนหัวแจ้งเตือน** ทั้งที่มันคือเงาของคำร้องข้ามฝ่าย
       ของจริงตอนเจอ (2026-08-13): 39 แถวในเธรด · 55 ใบแจ้งเตือน · ไม่ถูกอ่าน 85%
       ซึ่งอ่านได้ว่า "มีคนเขียนบันทึกอะไรสักอย่าง" แทนที่จะเป็น "คำร้องของดีลนี้ถูกตอบแล้ว"
       ⚠️ การจัดเป็นเหตุการณ์ระบบไม่เคยพัง (`authorableKinds` ดูจากทะเบียน ไม่ใช่ fallback)
       เสียแค่ป้ายกับสี · สีเดียวกับ `task` โดยตั้งใจ: ทั้งคู่คือ "ของอื่นที่ผูกกับดีล"
       ไม่ใช่ความเคลื่อนไหวของตัวดีลเอง */
    request: { label: 'คำร้อง', color: 'var(--text-3)' },
  },
  // ลีด (mig 0091) — ชุดชนิด **เหมือนดีลทุกตัว** โดยเจตนา: คนกลุ่มเดียวกันทำงาน
  // แบบเดียวกัน และลีดที่ผ่านคัดกรองจะกลายเป็นดีล ถ้าคำศัพท์ต่างกันบันทึกการโทร
  // ก่อน/หลังแตกดีลจะอ่านเป็นคนละเรื่องทั้งที่เป็นการคุยกับคนเดียวกัน
  //
  // ⚠️ ไม่มีชนิดเหตุการณ์ระบบที่นี่ — เหตุการณ์ของลีด (คัดกรอง/มอบหมาย/ตีกลับ)
  // ยังอยู่ในตาราง `lead_events` ของตัวเอง (schema เฉพาะโดเมน + คิว/KPI query ตรง)
  // แล้วส่งเข้าเธรดผ่าน `extraItems` เป็นรายการอ่านอย่างเดียว — ตารางไม่ย้าย
  lead: {
    // ── สีของป้ายชนิด: จัดเป็น "กลุ่มความหมาย" ไม่ใช่ไล่สีไปเรื่อย ────────
    // บันทึก = ชนิดตั้งต้น (ข้อความส่วนใหญ่) จึงเป็นสีกลาง ไม่แย่งสายตา
    // โทร/ประชุม/อีเมล = กลุ่ม "ติดต่อลูกค้า" โทนเย็นด้วยกัน
    // ประสานงานภายใน = คุยกันเองในบริษัท จึงต้องแยกจากกลุ่มข้างบนให้เห็นชัด
    // ขั้นถัดไป = ตัวเตือนที่มีกำหนดวัน เด่นสุด
    // ⚠️ สีนี้ใช้กับ **ป้ายชนิด** เท่านั้น — แถบซ้ายของข้อความคนใช้ accent เสมอ
    // (ดู UpdateThread) ไม่งั้นข้อความคนจะเปลี่ยนสีไปมาตามชนิดจนอ่านไม่เป็นชุดเดียว
    note: { label: 'บันทึก', color: 'var(--text-2)', authorable: true },
    call: { label: 'โทร', color: 'var(--blue)', authorable: true },
    meeting: { label: 'ประชุม', color: 'var(--violet)', authorable: true },
    email: { label: 'อีเมล', color: 'var(--teal)', authorable: true },
    internal: { label: 'ประสานงานภายใน', color: 'var(--green)', authorable: true },
    next_step: { label: 'ขั้นถัดไป', color: 'var(--amber)', authorable: true, due: true },
  },
  // ── ใบเสนอราคา / ใบสั่งขาย: **ไม่มีเธรดของตัวเอง** (มติผู้ใช้ 2026-08-04) ──
  // เดิมมีชุด kind ของตัวเองและทุก action ลงสองที่ (เธรดของใบ + เงาบนดีล) แต่ตรวจ
  // ของจริงแล้ว **ไม่มีใครพิมพ์ในเธรดของใบเลยสักข้อความ** (QT 21 เหตุการณ์ · SO 1 ·
  // ข้อความคน 0) ขณะที่เงาบนดีลถูกอ่านจริงทั้งบนหน้าดีลและไหลต่อขึ้นหน้าโครงการ
  // ⇒ เหลือที่เดียวคือเธรดดีล (ชนิด `doc_*` ข้างบน) · ดูรายละเอียดใน
  // lib/sales/documentUpdates.js
  // ── master data: ลูกค้า / สินค้า ─────────────────────────────────────
  // ด่านอนุมัติชุดเดียวกัน (approvalStatus + rejectionReason + resetApprovalOnEdit)
  // จึงใช้ชุด kind เหมือนกันเป๊ะ — ป้ายต่างกันเมื่อไรคือสัญญาณว่ากฎเริ่มแตกเป็นสองชุด
  //
  // ⚠️ `override` (อนุมัติทั้งที่เอกสารบังคับไม่ครบ) เคยเขียนลงเธรดด้วยชื่อ `note`
  // ซึ่ง **ไม่เคยประกาศไว้ที่นี่** ⇒ `updateKindMeta` ถอยไปใช้ meta ของ `comment`
  // แล้วแถวนั้นขึ้นป้าย "ข้อความ" สี accent = หน้าตาเหมือนข้อความที่คนพิมพ์เอง
  // ทั้งที่เป็นการข้ามด่านเอกสาร (แถวที่ควรสะดุดตาที่สุดในเธรด) และในกล่องแจ้งเตือน
  // ก็ขึ้นว่า "ข้อความ · ลูกค้า X" เหมือนมีคนมาทัก — 25 แถวใน prod ก่อน mig 0200
  customer: {
    comment: { label: 'ข้อความ', color: 'var(--accent)', authorable: true },
    approve: { label: 'อนุมัติ', color: 'var(--green)' },
    reject: { label: 'ตีกลับให้แก้ไข', color: 'var(--red)' },
    reset: { label: 'กลับไปรออนุมัติ', color: 'var(--amber)' },
    override: { label: 'ยกเว้นเอกสาร', color: 'var(--red)', quiet: true },
    /* ⭐ รอบวางบิลถูกตั้ง/แก้/ล้าง (มติเจ้าของ 26/09 ข้อ 7 · mig 0389) — เนื้อความ "เดิม → ใหม่"
       มาจาก `billingRuleChangeUpdate` · เขียนผ่าน `logBillingRuleActivity` (ทั้งคู่ lib/master/customerBillingRuleUpdate.js)
       ซึ่งมีผู้เรียกทางเดียวคือ route `/api/customers/[id]/billing-rule`
       ⚠️ **ของลูกค้าเท่านั้น ไม่ใช่ของสินค้า** — ชุด kind ที่ต้องเหมือนกันเป๊ะคือ "ด่านอนุมัติ" (approve/reject/
         reset/override) · รอบวางบิลอยู่นอกด่านนั้นโดยมติ (แก้แล้วไม่ต้องอนุมัติใหม่) และสินค้าไม่มีรอบวางบิล
       ⚠️ **quiet โดยมติเจ้าของ** ทั้งที่ไม่ได้มาคู่กับเหตุการณ์ที่เด้งแทน (ข้อยกเว้นของกติกาใน
         `isQuietUpdateKind`): ผู้ติดตามเธรดลูกค้า = คนที่เคยคุย/ถูก @ ถึงเรื่องอื่น (ตีกลับเอกสาร ·
         ขอแก้ที่อยู่) ไม่ใช่คนที่ต้องลงมือเมื่อรอบเปลี่ยน · ผลของรอบไปถึงคนที่ต้องทำเองผ่านทางอื่นแล้ว
         (ชิปรอบบนใบสั่งขาย · กระดิ่งก่อนวันวางบิล) ⇒ แถวนี้มีไว้ให้ **ย้อนอ่าน** ว่ารอบเดิมเป็นอะไร ใครเปลี่ยน
       สีกลาง = "เนื้อในเปลี่ยน" ไม่ใช่ก้าวของด่าน (แบบเดียวกับ `update`/`registry` ของคำร้อง) */
    billing_rule: { label: 'รอบวางบิล', color: 'var(--text-3)', quiet: true },
  },
  product: {
    comment: { label: 'ข้อความ', color: 'var(--accent)', authorable: true },
    approve: { label: 'อนุมัติ', color: 'var(--green)' },
    reject: { label: 'ตีกลับให้แก้ไข', color: 'var(--red)' },
    reset: { label: 'กลับไปรออนุมัติ', color: 'var(--amber)' },
    override: { label: 'ยกเว้นเอกสาร', color: 'var(--red)', quiet: true },
  },
  // ── สายภาษีสรรพสามิต ────────────────────────────────────────────────
  excise_registration: {
    comment: { label: 'ข้อความ', color: 'var(--accent)', authorable: true },
    submit: { label: 'ยื่นให้ฝ่าย RA', color: 'var(--blue)' },
    approve: { label: 'ขึ้นทะเบียนแล้ว', color: 'var(--green)' },
    reject: { label: 'ตีกลับให้แก้ไข', color: 'var(--red)' },
    revoke: { label: 'ปลดอนุมัติ', color: 'var(--red)' },
  },
  excise_order: {
    comment: { label: 'ข้อความ', color: 'var(--accent)', authorable: true },
    status: { label: 'เปลี่ยนสถานะ', color: 'var(--blue)' },
    reject: { label: 'ตีกลับให้แก้ไข', color: 'var(--red)' },
  },
  // ── PO สหมิตร ────────────────────────────────────────────────────────
  // ไม่มีด่านอนุมัติของตัวเอง (ลูกค้าเป็นคนออก PO) — เหตุการณ์เดียวที่มีค่าคือ
  // จุดส่งมอบเข้าท่อขาย
  sahamit_po: {
    comment: { label: 'ข้อความ', color: 'var(--accent)', authorable: true },
    settle: { label: 'แปลงเป็นดีล', color: 'var(--green)' },
  },
  // ── นัดเข้าบริการ (mig 0188 · S-5) ───────────────────────────────────
  // ⭐ **เหตุผลการเลื่อนต้องอยู่ที่นี่ ไม่ใช่คอลัมน์** — คอลัมน์เดียวถูกเขียนทับทุกครั้ง
  // ที่เลื่อน แปลว่าประวัติการเลื่อน 5 ครั้งเหลือ 1 ซึ่งคือข้อมูลที่มีค่าที่สุดของโมดูลนี้
  // (ลูกค้าถามว่า "ทำไมเจ้าหน้าที่ไม่มาสักที" ต้องตอบได้ว่าเลื่อนเพราะอะไรบ้าง)
  service_visit: {
    comment: { label: 'บันทึกหน้างาน', color: 'var(--accent)', authorable: true },
    reschedule: { label: 'เลื่อนนัด', color: 'var(--amber)' },
    /* ปล่อยร่างเข้าคิว (ผ่านด่าน หรือหัวหน้าข้ามด่าน) — "ทำไมนัดนี้ขึ้นตารางทั้งที่
       ยังไม่จ่าย" เป็นคำถามที่ต้องตอบได้ทีหลัง (mig 0302) */
    queue: { label: 'เข้าคิว', color: 'var(--blue)' },
    done: { label: 'ปิดงาน', color: 'var(--green)' },
    cancel: { label: 'ยกเลิกนัด', color: 'var(--red)' },
  },
  // ทะเบียนกลิ่น (mig 0171) — เธรดสองฝ่าย: RD ส่งกลิ่น ↔ ฝ่ายขายนำผลจากลูกค้ามาลง
  //
  // ⚠️ **`feedback` ห้ามลบทิ้งแม้จะเลิกเขียนใหม่แล้ว** (ตาราง scent_revisions ถูก
  // ยกเลิกใน 0206 พร้อมมติที่ว่าลูกค้าให้แก้ = กลิ่นตัวใหม่) — `entity_updates` บน
  // prod มีแถวที่ใช้ kind นี้อยู่จริง ถอดทะเบียนเมื่อไร **เหตุการณ์เก่าในเธรดจะ
  // กลายเป็นช่องว่าง** · ทะเบียนนี้ถูกอ่านย้อนหลัง ไม่ใช่แค่ตอนเขียน
  scent: {
    comment: { label: 'ข้อความ', color: 'var(--accent)', authorable: true },
    accepted: { label: 'รับเข้าทะเบียน', color: 'var(--blue)' },
    sent: { label: 'ส่งกลิ่น', color: 'var(--violet)' },
    feedback: { label: 'ผลตอบรับลูกค้า', color: 'var(--green)' },
    status: { label: 'เปลี่ยนสถานะ', color: 'var(--amber)' },
  },
  // ใบขอราคาผลิต (mig 0143) — เธรดสองฝ่าย: เซลยื่น ↔ ผู้บริหารอนุมัติ/ตีกลับ
  // ใช้คำที่ล็อกไว้ของ workflow เอกสาร: "ตีกลับให้แก้ไข" / "ออก Rev."
  costing_request: {
    comment: { label: 'ข้อความ', color: 'var(--accent)', authorable: true },
    submit: { label: 'ยื่นขออนุมัติ', color: 'var(--blue)' },
    // ดึงกลับ = ผู้ยื่นเอาคืนเอง (ไม่ใช่ตีกลับ) จึงเป็นสีกลาง ไม่ใช่สีแดง
    withdraw: { label: 'ดึงกลับมาแก้ไข', color: 'var(--text-3)' },
    approve: { label: 'อนุมัติราคาผลิต', color: 'var(--green)' },
    returned: { label: 'ตีกลับให้แก้ไข', color: 'var(--red)' },
    revise: { label: 'ออก Rev.', color: 'var(--amber)' },
  },
  // ── เรื่องแจ้งปัญหาระบบ (mig 0223) — เธรดสองฝ่าย: ผู้แจ้ง ↔ ผู้ดูแลระบบ ──
  // ⚠️ ชื่อ kind **ตรงกับชื่อ action ใน `lib/issues/model.js` ทุกตัว** โดยเจตนา —
  // route handler ส่ง `kind: action` ตรง ๆ ถ้าชื่อสองฝั่งหลุดกัน เหตุการณ์จะขึ้น
  // ป้าย "ข้อความ" เหมือนคนพิมพ์เอง แล้วยังหายไปตอนกดซ่อนเหตุการณ์ระบบอีก
  system_issue: {
    comment: { label: 'ข้อความ', color: 'var(--accent)', authorable: true },
    acknowledge: { label: 'รับเรื่อง', color: 'var(--blue)' },
    assign: { label: 'มอบหมาย', color: 'var(--blue)' },
    impact: { label: 'ปรับผลกระทบ', color: 'var(--amber)' },
    resolve: { label: 'แจ้งว่าแก้แล้ว', color: 'var(--green)' },
    confirm: { label: 'ผู้แจ้งยืนยันว่าหาย', color: 'var(--green)' },
    // ดีดกลับ = เหลือง ไม่ใช่แดง — "ยังต้องแก้ต่อ" ไม่ใช่ "จบแล้ว"
    // (กติกาสีเดียวกับ bounce ของคำร้อง)
    reopen: { label: 'ยังไม่หาย', color: 'var(--amber)' },
    reject: { label: 'ไม่ใช่บั๊ก / ไม่ทำ', color: 'var(--red)' },
    auto_close: { label: 'ปิดอัตโนมัติ', color: 'var(--text-3)' },
  },
};

// ชนิดตั้งต้นของ entity ที่ไม่ได้ประกาศอะไรเลย — ยังคงชื่อเดิมไว้เพราะเป็นค่าที่
// เธรดส่วนใหญ่ใช้ตัวเดียว (ก่อนหน้านี้เป็น "ชนิดเดียวที่คนพิมพ์เองได้" ทั้งระบบ)
export const AUTHORABLE_KIND = 'comment';

// ชุดชนิดที่คนเลือกเองได้ของ entity นั้น (เรียงตามลำดับที่ประกาศ = ลำดับใน dropdown)
export function authorableKinds(entityType) {
  return Object.entries(UPDATE_KINDS[entityType] || {})
    .filter(([, meta]) => meta?.authorable)
    .map(([kind]) => kind);
}

// ชนิดที่ใช้เมื่อผู้โพสต์ไม่ได้เลือก (หรือ entity มีชนิดเดียว) — ตัวแรกที่ประกาศไว้
export function defaultAuthorableKind(entityType) {
  return authorableKinds(entityType)[0] || AUTHORABLE_KIND;
}

// ⚠️ ด่านกันปลอมไทม์ไลน์: API ต้องเรียกตัวนี้ก่อนรับ kind จาก client เสมอ
export function isAuthorableKind(entityType, kind) {
  return authorableKinds(entityType).includes(kind);
}

// ชนิดนี้กรอกกำหนดวันได้ไหม (เช่น "ขั้นถัดไป" ของฟีดดีล)
export function kindAcceptsDueDate(entityType, kind) {
  return !!(UPDATE_KINDS[entityType] || {})[kind]?.due;
}

export function updateKindMeta(entityType, kind) {
  const set = UPDATE_KINDS[entityType] || {};
  return set[kind]
    || set[defaultAuthorableKind(entityType)]
    || { label: 'อัปเดต', color: 'var(--accent)' };
}

/**
 * ชนิดที่ **ลงเธรดแต่ไม่ต้องเด้งกระดิ่ง**
 *
 * 🐞 ที่มา (ตรวจข้อมูลจริง 2026-08-13): อนุมัติสินค้าหนึ่งครั้งเขียนสองเหตุการณ์เสมอ —
 * `approve` แล้วตามด้วย `override` (เพราะสินค้าบังคับแนบ Artwork แต่บน prod ยังไม่มีใบ
 * ไหนแนบเลย ⇒ ต้องยกเว้นทุกครั้ง) · ทั้งคู่ fan-out ให้คนกลุ่มเดียวกันในวินาทีเดียวกัน
 * ⇒ ผู้ใช้ได้แจ้งเตือนสองใบที่แปลว่า "อนุมัติแล้ว" เหมือนกัน
 *
 * ตัวเลขตอนเจอ: สินค้ามี 259 แถว · **218 แถว (84%) มาจากคู่นี้** · ไม่ถูกอ่าน 84%
 * ซึ่งสูงสุดในระบบ — กล่องที่มีของซ้ำครึ่งหนึ่งคือกล่องที่คนเลิกอ่าน (กับดักที่มติ 14
 * พยายามกันไว้ตั้งแต่แรก)
 *
 * ⚠️ **ไม่ได้ลบเหตุการณ์ทิ้ง** — แถวยังอยู่ในเธรดครบ เปิดใบแล้วยังอ่านได้ว่าใครยกเว้น
 * เอกสารด้วยเหตุผลอะไร (ซึ่งเป็นเรื่องที่ต้องตรวจย้อนได้) · ที่ตัดคือ *การเด้ง* เท่านั้น
 *
 * ⚠️ ใส่ `quiet` ให้ชนิดใหม่ได้เฉพาะเมื่อมันมาคู่กับเหตุการณ์อื่นที่เด้งแทนอยู่แล้ว —
 * ไม่ใช่ใช้เป็นวิธี "ลดเสียง" ของเหตุการณ์ที่ไม่มีใครแจ้งซ้ำให้
 * ⚠️ ข้อยกเว้นที่มีมติกำกับ (ต้องเขียนเหตุผลไว้ที่ชนิดนั้นเสมอ): `customer.billing_rule`
 * (มติเจ้าของ 26/09 — แถวบันทึกย้อนหลังของการตั้งค่า ไม่ใช่งานของผู้ติดตามเธรด)
 */
export function isQuietUpdateKind(entityType, kind) {
  return !!(UPDATE_KINDS[entityType] || {})[kind]?.quiet;
}

export function isKnownUpdateKind(entityType, kind) {
  return !!(UPDATE_KINDS[entityType] || {})[kind];
}

// ── เหตุการณ์ระบบ vs ข้อความคน ───────────────────────────────────────────
// เหตุการณ์ระบบ = ของที่ไม่มีใครพิมพ์: แถวที่ kind ไม่ใช่ AUTHORABLE_KIND (ระบบเขียน
// ให้ตอนเกิดเหตุการณ์) + รายการอ่านอย่างเดียวจากแหล่งอื่น (`extraItems` — ประวัติ
// สถานะ/เหตุการณ์ลีด) ซึ่งไม่มีทางเป็นข้อความคนอยู่แล้ว
//
// ทำไมต้องแยก: พอเธรดขึ้นเอกสารที่มี action เยอะ (QT/SO มี 8 ตัวที่เขียนลงเธรด)
// เหตุการณ์ระบบจะถมจนข้อความคนจม — ต้องมีสวิตช์ให้เหลือเฉพาะที่คนคุยกัน
//
// ⚠️ ข้อความที่ถูกลบแล้วยังเป็น "ข้อความคน" (kind ยังเป็นชนิดที่คนเลือกได้) — ต้อง
// ไม่ถูกซ่อนไปกับเหตุการณ์ระบบ เพราะรอยที่ว่าเคยมีข้อความคือส่วนหนึ่งของบทสนทนา
//
// ⚠️ ต้องรู้ entityType ด้วย: ฟีดดีลมีชนิดที่คนเลือกเองได้ห้าตัว (โทร/ประชุม/อีเมล…)
// ถ้าเทียบกับ 'comment' ตัวเดียวเหมือนเดิม บันทึกการโทรของคนจะถูกจัดเป็น
// "เหตุการณ์ระบบ" แล้วหายไปตอนกดซ่อน
export function isSystemUpdateItem(entityType, item) {
  if (!item) return false;
  if (item.kind === 'extra') return true;
  const kind = item.row?.kind;
  if (!kind) return false;                    // ไม่รู้ = ถือว่าข้อความคน (ไม่ซ่อน)
  return !isAuthorableKind(entityType, kind);
}

/**
 * แถวนี้เป็นส่วนหนึ่งของ **บทสนทนา** หรือเป็น **log ของระบบ**
 *
 * ⭐ ต่างจาก `isSystemUpdateItem` ตรงที่นั่นถามว่า "คนเลือกชนิดนี้ตอนโพสต์ได้ไหม"
 * (ใช้กับสวิตช์ซ่อน) · ตัวนี้ถามว่า **"แถวนี้มีข้อความที่คนพิมพ์อยู่ในนั้นไหม"**
 * ⇒ เหตุการณ์ที่ระบบเขียนแต่พกเหตุผลของคน (ตีกลับ · ยกเลิก · เลื่อนวัน · ลูกค้าขอแก้)
 * นับเป็นบทสนทนา ทั้งที่ไม่ authorable
 *
 * ⚠️ ชนิดที่ยังไม่ประกาศธง `narrative` **ตกเป็น log** — เจตนา: ระบบเขียนแถวใหม่ได้
 * เรื่อย ๆ และของที่ไม่มีใครตัดสินใจว่าเป็นบทสนทนา ไม่ควรเด้งเข้าบทสนทนาเอง
 * ⚠️ ชนิดที่ไม่รู้จักเลย (ข้อมูลเก่า/ชนิดที่ถูกถอด) นับเป็นบทสนทนา — หายจากจอ
 * แย่กว่าอยู่ผิดกล่อง (กติกาเดียวกับ `isSystemUpdateItem`)
 */
export function isNarrativeUpdateItem(entityType, item) {
  if (!item) return false;
  if (item.kind === 'extra') return false;
  const kind = item.row?.kind;
  if (!kind) return true;
  if (!isKnownUpdateKind(entityType, kind)) return true;
  return !!UPDATE_KINDS[entityType][kind].narrative;
}

// ── ไฟล์แนบในข้อความ ────────────────────────────────────────────────────
// รับเฉพาะ ref ของไฟล์ที่อัปผ่าน /api/upload แล้ว (กัน payload แปลกปลอม) —
// แพตเทิร์นเดียวกับ sanitizeInquiryAttachments / sanitizeWonAttachments
export const MAX_UPDATE_ATTACHMENTS = 8;

export function sanitizeUpdateAttachments(input) {
  if (!Array.isArray(input)) return [];
  return input
    .filter((a) => a && typeof a === 'object' && typeof a.fileUrl === 'string' && a.fileUrl)
    .slice(0, MAX_UPDATE_ATTACHMENTS)
    .map((a) => ({
      fileUrl: String(a.fileUrl),
      driveFileId: a.driveFileId ? String(a.driveFileId) : null,
      fileName: a.fileName ? String(a.fileName).slice(0, 200) : null,
      mimeType: a.mimeType ? String(a.mimeType).slice(0, 100) : null,
      sizeBytes: Number.isFinite(a.sizeBytes) ? Number(a.sizeBytes) : null,
    }));
}

// ข้อความที่ลบแล้วยังอยู่ในเธรด (soft delete) — แสดงเป็นรอยว่าเคยมี ไม่ใช่หายเงียบ
export const DELETED_UPDATE_TEXT = 'ข้อความนี้ถูกลบแล้ว';

// ตัดของที่ผู้อ่านไม่ควรเห็นออกจากแถวที่ลบแล้ว (เนื้อความ/ไฟล์แนบหายไป เหลือแต่รอย)
export function redactDeleted(row) {
  if (!row?.deletedAt) return row;
  return { ...row, body: null, attachments: [], meta: {} };
}
