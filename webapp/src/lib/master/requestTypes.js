 ...
 *   ถึงสามจุด (ใบ 0 แถวที่ pending · ใบ deliversRows ที่ยังไม่ส่งงาน · ใบที่ลูกค้าไม่
 *   คอนเฟิร์มครบ) ⇒ รัดด่านกลาง = ใบ RD/PC ที่รับเรื่องแล้วแต่ล้ม **ค้างถาวรไม่มีประตูออก**
 * ⇒ เป็นธงรายหัวข้อ · ด่านยังเป็นตัวเดียว (`cancelRequestError`) ไม่แตกนิยามที่สอง
 *
 * ⚠️ หัวข้อที่เปิดธงนี้ **ต้องมีทางออกหลังรับเรื่องของตัวเอง** ไม่งั้นแค่ย้ายที่ค้าง
 *   (ประเมินพื้นที่ใช้ "ปิดใบโดยไม่ได้ประเมิน" ของฝ่าย TS) */
export function requestCancelBeforeAckOnly(kind) {
  return !!REQUEST_KINDS[kind]?.cancelBeforeAckOnly;
}

/* ⭐ **หัวข้อที่ปิดเรื่องได้เมื่อลูกค้าคอนเฟิร์มครบตามจำนวนใน SO** (ม-131 · ม-146)
 * วันนี้มีแค่พัฒนากลิ่น — แถวเกิดตอนฝ่ายส่ง จำนวนแถวจึงบอกไม่ได้ว่าได้ครบตามที่สั่ง
 * 🔴 **ห้ามถามว่า "ใบนี้ผูก SO ไหม" แทน** — ขอเอกสาร/เอกสารการเงินอ้าง SO ได้ (optionalRefs)
 *   แต่แถวของมันไม่มีขั้น "ลูกค้าคอนเฟิร์ม" ⇒ ด่านไม่มีวันผ่าน = ใบปิดไม่ได้ตลอดกาล
 *   (ของจริง 2026-09-11: RQ-DF-26090041 · RQ-DF-26090042 · DC-26080004) */
export function requestCloseNeedsSoConfirm(kind) {
  return !!REQUEST_KINDS[kind]?.closeNeedsSoConfirm;
}

/* ⭐ **หัวข้อที่ผู้ขอปิดเรื่องได้หลังฝ่ายตอบแล้วเท่านั้น** (มติเจ้าของ 24/09 ข้อ 3 · ใบประเมินพื้นที่)
 * 🐞 ใบประเมินที่ฝ่ายขายกดปิดก่อนมีผล ⇒ `closedAt` ล็อกจอประเมินทั้งใบ ช่างกรอกไม่ได้ ปุ่มส่งงาน/ส่งผลหาย
 * ⚠️ **ธงรายหัวข้อ ไม่ใช่กฎกลาง** — สอบถามข้อมูล/ขอเอกสารตั้งใจให้ผู้ขอเป็นคนตัดสินว่า "พอแล้ว" ได้เอง
 * ⚠️ หัวข้อที่เปิดธงนี้ต้องมีทางออกของใบที่ไม่ต้องการผลแล้ว (ประเมินพื้นที่: "ปิดใบโดยไม่ได้ประเมิน") */
export function requestCloseNeedsAnswer(kind) {
  return !!REQUEST_KINDS[kind]?.closeNeedsAnswer;
}

/* ⭐ **ทางตอบเฉพาะของหัวข้อ** (มติเจ้าของ 24/09 ข้อ 1) — null = ตอบด้วยปุ่ม "ตอบแล้ว" กลางได้ตามปกติ
 * ⚠️ ตัวแปลคีย์เป็นป้าย/ลิงก์/สิทธิ์อยู่ที่ `lib/requests/answerVia.js` — ไฟล์นี้บอกแค่ว่าหัวข้อประกาศอะไร */
export function requestAnswerViaKey(kind) {
  return REQUEST_KINDS[kind]?.answerVia || null;
}
...
export function requestShapeError(kind, body = {}) {
  ...
  /* ⭐ **รายละเอียดบังคับเฉพาะหัวข้อที่ทะเบียนสั่ง** (มติผู้ใช้ 2026-09-21) — หัวข้อที่
     ไม่มีตารางรายการและไม่มีแบบฟอร์ม PDR (ประเมินพื้นที่) ใช้ช่องนี้เป็นที่เดียวที่
     บริบทของงานอยู่ · ฝ่ายปลายทางต้องอ่านก่อนจัดคนและลำดับงาน
     ⚠️ ถามทะเบียน ไม่เทียบชื่อหัวข้อ (ม-34) · หัวข้อที่ใช้ PDR ไม่มีช่องนี้บนจอ */
  const kindForm = meta.form || {};
  if (kindForm.bodyRequired && !shape.hasPdr && !String(body.body ?? '').trim()) {
    return `ต้องกรอก${kindForm.bodyLabel || 'รายละเอียด'}`;
  }
