...
export function addressText(row) {
  ...
  if (r.addressOverride) return typed;
  /* 🐞 **แถวที่พิมพ์บ้านเลขที่ไว้แล้วแต่ยังไม่ได้เลือกจังหวัด เคยกลายเป็นแถวว่าง**
     (พบตอน UAT 2026-08-27) — `hasStructuredParts` เช็คแค่ `province` ⇒ ยังไม่เลือก
     จังหวัด ตัวนี้จะถอยไปคืนข้อความที่พิมพ์เองซึ่งว่าง ⇒ normalizeAddresses ตัดแถวทิ้ง
     ⇒ กดบันทึกลูกค้าใหม่แล้วโดนตีกลับว่า "ต้องมีที่อยู่สำหรับออกเอกสารอย่างน้อย
     1 รายการ" ทั้งที่บนจอมีข้อความอยู่ในช่องชัด ๆ และบ้านเลขที่ที่พิมพ์ไปหายเงียบ
     ⭐ ยังไม่เลือกจังหวัด **แต่มีข้อความที่พิมพ์เองอยู่แล้ว** = คืนของเดิมเหมือนเดิม
     (กันแถวยุคเก่าโดนเขียนทับ ตามกับดักที่อธิบายไว้ข้างล่าง) · ไม่มีของเดิมให้เสีย
     ค่อยประกอบจากฟิลด์ย่อยเท่าที่มี ซึ่งอย่างน้อยก็ได้ `line1` กลับมา */
  if (!hasStructuredParts(r)) return typed || composeThaiAddress(r);
...
export function pickDocumentAddresses(customer, { billingAddressId, shippingAddressId } = {}) {
  ...
  return {
    ...
    snapshot: {
      ...
      /* ── ข้อความอังกฤษของที่อยู่ **แถวเดียวกัน** (มติผู้ใช้ 2026-09-03) ──────
         ใบภาษาอังกฤษต้องพิมพ์ที่อยู่อังกฤษ — เดิมข้อความอังกฤษถูกยุบทิ้งตั้งแต่จุดนี้
         ใบจึงไม่มีอะไรให้พิมพ์นอกจากไทย
         ⚠️ **ไม่ถอยไปไทยที่นี่** — เก็บอังกฤษล้วน ไม่มีคือ null เพราะปลายทางต้องแยกออก
         ว่า "ว่างจริง" (เตือนบนหน้าต่างพิมพ์แล้วถอยไปไทย · กติกาเดียวกับชื่อสินค้า)
         ต่างจากคีย์ไทยข้างบนที่ตกไปอังกฤษได้ = "อังกฤษเพราะลูกค้ามีแค่นั้น"
         ⚠️ **ห้ามถอยไปที่อยู่ออกบิล** แบบคีย์ไทย — ที่นั่นถอยได้เพราะแถวจัดส่งที่ไม่มี
         ข้อความสักภาษาไม่มีอยู่จริง (โดน normalizeAddresses ตัดทิ้งไปแล้ว) แต่แถวจัดส่ง
         ที่มี "ไทยอย่างเดียว" มีจริง ⇒ ถอยไปบิลคือพิมพ์ที่อยู่ออกบิลเป็นที่อยู่จัดส่ง */
      billingAddressEn: billing ? (addressTextEn(billing) || null) : null,
      shippingAddressEn: shipping ? (addressTextEn(shipping) || null) : null,
