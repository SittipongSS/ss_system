...
export function quotationWonEffects({ quote, deal = null, project = null, linkingProject = false } = {}) {
  ...
  return [
    ...
    ...(linkingProject && projectLabel
      ? [`${dealName} เข้าโครงการ ${projectLabel} — ไทม์ไลน์ลอยของดีลถูกรับเข้าเป็นช่วงใหม่ ไม่สร้างทับ · งาน คำร้อง และใบที่เปิดไว้ก่อนหน้าย้ายตาม`]
      : []),
    `${dealName} เปลี่ยนเป็น Won · FC ขึ้น 100% · มูลค่าปิด ${fmtMoney(amount)} (ก่อน VAT)`,
    ...(amount === 0
      // ใบ 0 บาทปิด Won ได้ (มติ 2026-08-03) — ทวนให้เห็นก่อนกด เพราะยอดนี้ทับมูลค่าดีล
      ? ['ใบนี้ยอดเป็น 0 บาท ⇒ มูลค่าปิดของดีลจะเป็น 0']
      : []),
    'ใบเสนอราคาฉบับอื่นในดีลนี้ถูกปิด — แก้ไข / ออก Rev. ต่อไม่ได้',
    'เปิดใบสั่งขายได้ — เอกสารยืนยันคำสั่งซื้อและงวดชำระกรอกที่หน้าสร้างใบสั่งขาย',
    'ย้อนการรับใบได้เฉพาะก่อนออกใบสั่งขาย และต้องระบุเหตุผล',
  ];
}
