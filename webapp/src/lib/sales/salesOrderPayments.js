...
/* ⭐ `prepaid` **ไม่ใช่ค่าใน DB** — เป็นสถานะที่คำนวณจากงวดร่างที่มีวันจ่าย+หลักฐานแล้ว
   (มติผู้ใช้ 2026-08-19) ยอดของงวดร่างยังลอย จึงยังส่งให้บัญชีตรวจไม่ได้ แต่ต้องต่างจาก
   "รอชำระ" ให้เห็นด้วยตา ไม่งั้นคนดูจะไม่รู้ว่าเงินเข้าแล้ว · ดู `installmentDisplayStatus` */
...
/* ── เงินที่เข้ามาก่อนใบอนุมัติ (มติผู้ใช้ 2026-08-19 — ทางเลือก ก.) ─────────
   ...
   ⇒ เงินไม่เข้าทะเบียนของบัญชีก่อนยอดจะนิ่ง และไม่มีใครต้องถือสลิปไว้เอง */
...
/* ปลายทางของการ "แจ้งชำระ" ขึ้นกับว่าใครกด (มติผู้ใช้ 2026-08-18 — ทางเลือก ก.)
   ...
   ⭐ **งวดร่างจอดที่ `pending` เสมอ ไม่ว่าใครกด** (มติผู้ใช้ 2026-08-19) — ยอดยังลอย
   ตามแผนของ QT ⇒ ส่งให้บัญชีตรวจยอดที่ยังจะถูกเขียนทับคือให้เขาตรวจของปลอม
   แม้แต่บัญชีกดเองก็ยังไม่ `confirmed`: คำรับรองต้องอยู่บนยอดที่นิ่งแล้วเท่านั้น
...
export function installmentActionError(row, action, user, options = {}) {
  ...
  if (action === 'report') {
    ...
    /* ⭐ **งวดร่างบันทึกเงินได้แล้ว แต่ยังไม่ส่งให้บัญชี** (มติผู้ใช้ 2026-08-19)
       เดิมบล็อกทั้งก้าวเพราะยอดของงวดร่างยังเดินตามใบ ⇒ หลักฐานจะผูกกับตัวเลขที่กำลัง
       จะถูกเขียนทับ · เหตุผลนั้นยังจริง แต่มันห้ามผิดอย่าง — สิ่งที่ต้องรอยอดนิ่งคือ
       **การส่งให้บัญชีตรวจ** ไม่ใช่การบันทึกว่าเงินเข้า ⇒ ปลายทางไปจอดที่ `pending`
       แทน (ดู `installmentReportOutcome`) แล้วเลื่อนให้เองตอนอนุมัติ
