...
/* ── ช่องรหัสลูกค้าในเลขที่ (มติผู้ใช้ 2026-08-31) ──────────────────────────
   `CT-AAAA-BB-XXXXX-R` — AAAA คือ **เลข 4 หลักจากรหัส AR ของลูกค้า**

   🪤 **รหัส AR ของจริงไม่ได้มีทรงเดียว** (วัดสด 510 ราย): `AR-999` 488 · `AR-9999` 17 ·
   `A-999` 3 · `AR-K9999` 1 · `AR-M999` 1 ⇒ ถ้าเอาเฉพาะตัวเลข **มี 4 คู่ที่ชนกัน**
   (`A-002`↔`AR-002` · `A-003`↔`AR-003` · `A-009`↔`AR-009` · `AR-005`↔`AR-K0005`)
   ⇒ **มติผู้ใช้: แก้รหัสลูกค้ากลุ่มนั้นให้เข้ารูป `AR-9999` ก่อน** แล้วค่อยใช้เลข 4 หลัก
   ⚠️ ฟังก์ชันนี้ **ไม่รู้เรื่องการชนกัน** — มันแปลงรหัสเดี่ยว ๆ เท่านั้น · การกันชนอยู่ที่
   ทะเบียนลูกค้า (รหัสต้องไม่ซ้ำ) ไม่ใช่ที่นี่ */
export function customerNumberSlot(arCode) {
  const digits = String(arCode || '').match(/\d+/g);
  if (!digits) return null;
  const joined = digits.join('');
  // เอา 4 หลักท้าย — รหัสจริงสูงสุดคือ 1020 จึงไม่มีทางเกิน 4 หลักอยู่แล้ว
  return joined.slice(-4).padStart(4, '0');
}

/** รูปแบบเลขที่ของสัญญาใบนั้น — `CT-AAAA-BB-{RUNNING:5}`
 *
 * ⭐ **ไม่มี YYMM แล้ว** (มติผู้ใช้ 2026-08-31) — เลขรันเดินยาวไม่ตัดรอบเดือน
 *   ⇒ ดูจากเลขไม่รู้ปีที่ทำสัญญาอีกต่อไป (วันที่อยู่บนตัวเอกสารและในทะเบียน)
 * ⚠️ คืน `null` เมื่อชนิดไม่รู้จัก **หรือลูกค้ายังไม่มีรหัส AR** — ผู้เรียกต้องปฏิเสธ
 *   ไม่ใช่ปล่อยให้ออกเลขที่มีช่องมั่ว ๆ ซึ่งลบทิ้งไม่ได้แล้วเมื่อออกไป
 */
export function contractNumberPattern(kind, arCode) {
  const code = contractKindCode(kind);
  const slot = customerNumberSlot(arCode);
  return code && slot ? `CT-${slot}-${code}-{RUNNING:5}` : null;
}
...
/* ⭐ **เลขรันเดินยาว ไม่ตัดรอบเดือน** (มติผู้ใช้ 2026-08-31: "เลขรัน ไปเรื่อยๆ") —
   `entity_number_counters` คีย์ด้วย (scope, month) ⇒ ตัวนับที่ไม่ตัดรอบใช้ month = `'-'`
   ซึ่งเป็นแพตเทิร์นเดียวกับ AR/FG ที่มีอยู่แล้วบนฐาน (ไม่ใช่ท่าที่คิดขึ้นใหม่)
   🪤 ตัวนับของรูปแบบเก่าคือ (CT, '2608') ส่วนของใหม่คือ (CT, '-') — **คนละแถวกัน**
   ⇒ เลขใหม่เริ่มที่ 00001 และไม่ชนกับเลขเก่าเพราะสตริงคนละรูปโดยสิ้นเชิง
   ⚠️ ใบที่ออกเลขไปแล้วยังเป็นรูปแบบเก่า **ห้ามแปลงย้อนหลัง** — เลขที่ออกไปแล้วถูกอ้าง
   บนกระดาษของลูกค้า · ระบบจะมีสามรูปแบบอยู่ร่วมกัน (`CT-26080001-0` ยุคแรก ·
   `CT-SD-2608xxxx-0` ยุคกลางถ้าออกทัน · `CT-0121-SR-00001-0` ของใหม่) */
