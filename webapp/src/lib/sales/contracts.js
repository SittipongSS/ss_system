...
// ── เลขที่สัญญา ────────────────────────────────────────────────────────────
//
// CT-AA-YYMMXXXX-R (AA = อักษรย่อชนิดสัญญา · รีเซ็ตต่อเดือน scope 'CT')
//
...
/** รูปแบบเลขที่ของสัญญาชนิดนั้น — คืน null เมื่อชนิดไม่รู้จัก (ผู้เรียกต้องปฏิเสธ
 *  ไม่ใช่ปล่อยให้ออกเลขที่มีอักษรย่อมั่ว ๆ ซึ่งลบทิ้งไม่ได้แล้วเมื่อออกไป) */
export function contractNumberPattern(kind) {
  const code = contractKindCode(kind);
  return code ? `CT-${code}-{YY}{MM}{RUNNING:4}` : null;
}

/* ⚠️ **เคาน์เตอร์ยังเป็นบ่อเดียวทั้งสามชนิด** (มติผู้ใช้ 2026-08-31 เลือกเอง) —
   อักษรย่อเปลี่ยนแค่ *หน้าตาเลข* ไม่ได้แยกสายเลขรัน ⇒ เลขในซีรีส์ของแต่ละชนิด
   จะกระโดด (CT-SD-26080001 · CT-MF-26080002) ซึ่งเป็นเรื่องปกติของบ่อร่วม
   ⇒ **ห้ามเปลี่ยน scope เป็นรายชนิดโดยไม่ถามก่อน** — RPC สองตัวใน SQL
   ...
   ⚠️ ใบที่ออกเลขไปแล้วก่อน 31/08 ยังเป็นรูปแบบเก่า (`CT-26080001-0`) และ
   **ห้ามแปลงย้อนหลัง** — เลขที่ออกไปแล้วถูกอ้างบนกระดาษของลูกค้า */
