...
export const DOCUMENT_SOURCES = {
  ...
  // ⭐ แหล่งที่ 8 (2026-08-14): เอกสารมีชีวิตที่หลายคนแก้พร้อมกันได้ (Google Doc/Sheet)
  // อยู่**เหนือ**ไฟล์แนบเพราะเป็นของที่ยังขยับอยู่ — ของที่ตรึงแล้วรอด้านล่างได้
  sharedDoc: { label: 'เอกสารร่วม', order: 1.5 },
...
export function buildEntityDocuments({
  ...
  for (const a of attachments) {
    // ⭐ เอกสารมีชีวิตแยกเป็นแหล่งของตัวเอง — ไม่ใช่ "ไฟล์แนบ" ชนิดหนึ่ง
    // ของสองอย่างนี้ทำคนละหน้าที่: ไฟล์แนบคือ*หลักฐาน*ที่ตรึงไว้แล้ว ส่วนเอกสารร่วม
    // คือ*พื้นที่ทำงาน*ที่ยังขยับอยู่ · ปนกันแล้วคนอ่านลิสต์แยกไม่ออกว่าอันไหน
    // เชื่อถือได้เป็นหลักฐาน
    if (isGoogleDoc(a)) {
      rows.push({
        id: `gdoc:${a.id}`,
        source: 'sharedDoc',
        title: a.fileName || 'เอกสารร่วม',
        note: googleDocKindLabel(a),
        // เปิดผ่าน webViewLink ตรง ไม่ผ่าน proxy — proxy สตรีมไฟล์ไบนารี ส่วนนี่คือ
        // หน้าเว็บของ Google ที่ต้องใช้ session ของผู้ใช้เอง
        href: a.fileUrl || null,
        // ⚠️ เอกสารร่วมโชว์ "แก้ล่าสุด" ไม่ใช่ "สร้างเมื่อ" — ของที่ยังขยับอยู่
        // สิ่งที่คนอยากรู้คือมันเคลื่อนไหวล่าสุดเมื่อไหร่
        // ⚠️ วันที่ที่มีคือ "วันที่ผูกเข้าดีล" ไม่ใช่ "แก้ล่าสุด" — เวลาแก้ล่าสุดอยู่ที่
        // Drive (modifiedTime) ซึ่งระบบไม่ได้เก็บไว้ · ห้ามติดป้ายว่าแก้ล่าสุด
        // ถ้าจะเอาจริงต้องยิง Drive ถามทุกครั้งที่เปิดแท็บ ซึ่งแพงเกินกว่าเหตุ
        at: a.createdAt || null,
        // ลิงก์ฝังในหน้า — null เมื่อแถวเก่าไม่มี googleFileId ⇒ จอไม่ต้องโชว์ปุ่ม "ดู"
        previewUrl: googleDocPreviewUrl(a),
      });
      continue;
    }
    rows.push({
