...
//   ย้อนการอนุมัติ     = ผู้รีวิวถอยการอนุมัติของตัวเอง (มติ 2026-08-18 — เดิม
//                       "ยกเลิกอนุมัติ" ซึ่งอ่านชนกับ "ยกเลิก SO" ที่แปลว่าใบตายถาวร)
//
...
test('retired workflow wording never comes back', () => {
  for (const file of FILES) {
    ...
    assert.doesNotMatch(src, /ถอดอนุมัติ/, `${file}: ใช้ "ย้อนการอนุมัติ" แทน "ถอดอนุมัติ"`);
    /* มติผู้ใช้ 2026-08-18: "ยกเลิกอนุมัติ" → "ย้อนการอนุมัติ" — คำเดิมอ่านชนกับ
       "ยกเลิก SO" ซึ่งเป็นคนละเรื่อง (ใบตายถาวร vs ถอยเพื่อออกฉบับใหม่)
       "ย้อน" เข้าชุดกับ "ย้อนการรับ" ของใบเสนอราคาที่เป็นการถอยเหมือนกัน */
    assert.doesNotMatch(src, /ยกเลิกอนุมัติ/, `${file}: ใช้ "ย้อนการอนุมัติ" แทน "ยกเลิกอนุมัติ"`);
