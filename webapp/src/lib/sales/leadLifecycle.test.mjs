...
test("คัดกรอง: ทีมที่ส่งกลับมาแล้วครบโควตาถูกล็อก ไม่ใช่ซ่อน", () => {
  ...
  /* ⚠️ ชื่อทีมมาจาก **ทะเบียนจริง** ตอนรัน (สแนปช็อตที่ `AppLayout` โหลด) — ในเทสต์ไม่มี
     ทะเบียน จึงได้รหัสทีม ซึ่งเป็นพฤติกรรมที่ตั้งใจ: ไม่รู้จัก = รหัสดิบ ไม่ใช่ชื่อเก่าจากโค้ด
     ที่ต้องยืนยันคือ **ตัวเลือกยังบอกได้ว่าเป็นทีมไหน** ไม่ใช่ป้ายเปล่า */
  assert.match(locked.label, new RegExp(TEAMS[0]), "ต้องยังบอกได้ว่าเป็นทีมไหน");
