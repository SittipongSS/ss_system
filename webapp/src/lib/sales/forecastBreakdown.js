...
/* ── ใครโหลดไฟล์นี้ได้ (มติผู้ใช้ 2026-09-02: "AE sup ขึ้นไป") ─────────────────
 *
 * ⚠️ **แคบกว่า `salesplan:view` มาก** — ไฟล์นี้มียอด FC ของ **ทุกทีมทุกคน** เป็นแถว ๆ
 *    พร้อมชื่อลูกค้าและราคาต่อหน่วย · AE ที่เห็นแค่ดีลของตัวเองบนจอ ไม่ควรได้ไฟล์ที่
 *    เห็นทั้งบริษัท (บทเรียนเดียวกับ `canExportLeadReport`: "ดูตัวเลขรวมได้" กับ
 *    "โหลดรายการออกไปได้" เป็นคนละสิทธิ์)
 * ⭐ `isSuperuser` = `admin || ae_supervisor` ซึ่งตรงกับ "AE sup ขึ้นไป" พอดี —
 *    เขียนผ่านตัวนี้ ไม่ใช่เทียบ role ตรง ๆ จะได้ขยับตามทะเบียนสิทธิ์กลางเสมอ
 */
export const canExportForecastReport = (role) => isSuperuser(role);
