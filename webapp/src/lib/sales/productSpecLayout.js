...
export const PRODUCT_SPEC_COST_MM = Object.freeze({
  /* ⭐ หัวข้อหน้าตาแบบ "งวดชำระเงิน / PAYMENT SCHEDULE" ของใบเสนอราคา (มติผู้ใช้ 2026-09-22 รอบสี่ — ดู extraCss ของ
     productSpecDocument): h3 8.7pt บรรทัดเดียว 5.06 (Chrome วาด 19.14px · offsetHeight 19 = 5.03) + margin 3.5 + 1.5
     ⇒ 10.06 · ระยะจริงจากขอบล่างตารางก่อนหน้าถึงขอบบนตารางถัดไปวัดได้ 10.05–10.32 (ปัดพิกเซลของสองก้อน)
     (รุ่นก่อน h3 10.5pt + margin 5 + 1.6 = 12.69 · ค่าคงที่ 12.8)
     🪤 ค่าคงที่ 10.2 ต่ำกว่าปลายบนของช่วง 10.32 **โดยตั้งใจ** — ส่วนเกิน 10.06 → 10.32 คือเศษพิกเซลที่ offsetHeight (ปัดเป็นพิกเซลเต็ม)
        ของตารางก่อนหน้าตัดทิ้ง = ความสูงจริงของตารางนั้น ไม่ใช่ของหัวข้อ · ตารางจ่ายเองด้วย `tableEdge` (2px) ·
        ตัวรับประกันจริงคือสวีปรายแผ่น (ที่แผนคิด ≥ ที่วาดทุกแผ่น · เผื่อต่ำสุด 0.54 · หัว productSpecDocument.js) —
        ปัดค่านี้ขึ้นไปครอบ 10.32 = นับเศษเดียวกันซ้ำสองที่ */
  heading: 10.2,
  /* หัวข้อช่องลงนาม ("การตรวจสอบและอนุมัติ / FINAL REVIEW & APPROVAL") ห่างก้อนบน 2.5 แทน 3.5 (`h3.signHeading`) ⇒ 9.06
     (วัด: ทั้งก้อน 43.16 = หัวข้อ 9.03 + กล่องบรรทัดเดียว 34.13 — ผลรวมรายชิ้นแบบที่สวีปวัด · offsetHeight ของ .signTail
     ทั้งก้อนได้ 43.13 เพราะปัดพิกเซลครั้งเดียว · รุ่นก่อน 10.19 / 44.32)
     🪤 หัวข้อนี้อยู่ใน .signTail (ไม่ใช่ลูกตัวแรกของ .sheetContent) ⇒ margin บนไม่หายแม้ขึ้นต้นแผ่น ขณะที่งบแผ่นต่อคืน margin
        ของหัวข้อแรกไว้ 3.5 ⇒ แผ่นที่ขึ้นต้นด้วยลายเซ็นคืนเกิน 3.5 — ปลอดภัยเพราะแผ่นนั้นมีแต่ลายเซ็น (สูง ~44 จากงบ ~270) */
  signatureHeading: 9.2,
  // h3 ตัวแรกของแผ่นต่อไม่มี margin บน (`.sheetContent > h3:first-child`) — ดู productSpecPageBudgets
  headingTopMargin: 3.5,
  tableEdge: 0.53,      // แถวแรก + เส้นขอบบนแบบ collapse สูงกว่า pitch ได้ถึง 0.41 (วัดรอบสอง) ⇒ 2px
  /* หัวตาราง checklist บรรทัดเดียวเท่า cert (วัด 7.67 จอ = พิมพ์) — หัวคอลัมน์ "ลำดับ" ลด padding ข้าง + nowrap
     (extraCss ของ productSpecDocument · ผลตรวจรอบสี่) · 🐞 เดิม "ลำดับ" ตก "ลำ / ดับ" ในช่อง 9mm ⇒ หัวสองบรรทัด 12.44 (ค่าคงที่ 12.7) */
  checklistHead: 7.9,
...
/* ── ช่องลงนาม (`signatureSection` ของเปลือก — กล่องชุดเดียวกับ QT/SO · มติ 2026-09-22) ─────────────────
   ...
   กริดยืดทุกกล่องเท่ากล่องที่สูงที่สุด ⇒ ทั้งก้อน = หัวข้อ h3 (8.7pt · margin บน 2.5 · `signatureHeading`) + กล่องที่สูงที่สุด
   ...
   รอบสี่ (หัวข้อ 8.7pt แบบใบเสนอราคา · กล่องไม่เปลี่ยน): ทั้งก้อน 43.16 / 47.66 / 52.15 · 242 แบบ (จอ = พิมพ์) เผื่อต่ำสุดทั้งก้อน 0.71 */
...
/**
 * ลายเซ็นทั้งก้อน = หัวข้อ "การตรวจสอบและอนุมัติ / FINAL REVIEW & APPROVAL" + แถวกล่อง (สูงเท่ากล่องที่สูงที่สุด)
...
export function productSpecPageBudgets({ headerMm, partyMm }) {
  ...
  /* ⭐ แผ่นต่อทุกแผ่นเปิดด้วยหัวข้อเสมอ (หัวข้อใหม่ · หัวข้อ "(ต่อ)" · หัวลายเซ็น — `paginateProductSpecSections`
     รับประกัน) และ h3 ตัวแรกของ `.sheetContent` ไม่มี margin บน ⇒ ต้นทุนหัวข้อที่คิดเต็ม (`heading`) เกินจริงเท่า margin บน
     (3.5 · รุ่นก่อน 5) ต่อแผ่น คืนที่นี่ทีเดียว · แผ่นแรกไม่ได้คืน (กล่องผู้ซื้อมาก่อน h3 ⇒ margin บนยังอยู่) */
