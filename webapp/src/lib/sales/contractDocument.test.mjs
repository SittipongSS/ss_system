...
test('เอกสารที่เรนเดอร์มีเลขที่ · คู่สัญญา · และข้อสัญญาครบทั้งสามหมวด', () => {
  ...
  /* ⭐ เอกสารสัญญาเป็นทศนิยม 2 ตำแหน่งทั้งหมด (มติผู้ใช้ 2026-09-03) —
     ต่างจากกติกาของจอ ("เงินเต็มหลัก") โดยตั้งใจ · เดิมเทสต์นี้ล็อก `3,000 บาท` ไว้
     ซึ่งเป็นรูปแบบเก่า ไม่ใช่เจตนาของเทสต์ (เจตนาคือ "ค่าที่กรอกถูกเติมลงข้อสัญญาจริง") */
  assert.match(html, /3,000\.00 บาท/);
...
test('ข้อมูลคู่สัญญาและวันที่พิมพ์เป็นตัวหนา · ตัวเลขเงื่อนไขไม่หนา', () => {
  ...
  assert.doesNotMatch(html, /<strong class="fill">3,000\.00<\/strong>/);
});
