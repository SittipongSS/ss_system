 ...
 *
 * ⭐ สัปดาห์เริ่ม **วันอาทิตย์ (อา–ส)** เหมือนปฏิทินทุกตัวของระบบ — มติเจ้าของ 2026-09-26
 * (เดิมถังนี้นับ จ.–อา. ตามที่ Marketing นับ ขณะที่ปฏิทินขึ้นต้นวันอาทิตย์ ⇒ ชิป "สัปดาห์นี้"
 * กับกริดปฏิทินในแผงเดียวกันเป็นคนละสัปดาห์) · ต้นสัปดาห์มาจาก `weekStartOf` ที่เดียว
 * 🔴 หาวันในสัปดาห์จาก **สตริงวัน** เท่านั้น ห้ามผ่าน `new Date(timestamp).getUTCDay()`
 * เพราะ timestamp มี offset +07 แล้ววันต้นสัปดาห์จะตกไปอยู่สัปดาห์ก่อนทั้งก้อน
 * (เจอจริงตอนสำรวจข้อมูลก่อนทำใบนี้)
 */
...
export function leadDailyBuckets({ byDay = {}, days = null, unit = 'day' } = {}) {
  ...
  return [...weeks.values()]
    ...
    .map((bucket) => ({
      ...bucket,
      /* 🐞 ช่วงในป้ายต้องเป็น **วันที่อยู่ในงวดจริง** ไม่ใช่อาทิตย์–เสาร์เต็มสัปดาห์:
         โหมดรายเดือน สัปดาห์หัวท้ายคาบเกี่ยวเดือนอื่นเสมอ · ป้าย "26/07 – 01/08" ที่ขึ้น
