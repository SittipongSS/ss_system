...
/* ⭐ **ทะเบียนสรรพสามิตเข้ากระดิ่ง แต่เข้าทาง `kinds`** (2026-08-28) — เหตุผลเดียว
 * กับลีดและการมอบหมายงานเป๊ะ ๆ
 *
 * ทำไมต้องเข้า: การเปลี่ยนสถานะทะเบียนเคยมีโครงแจ้งข้ามเลนอยู่ใน route จริง แต่ตัวส่ง
 * ถูกถอดตอนเลิกใช้ Chat เหลือ `if` ว่างเปล่าทั้งสองกิ่ง ⇒ SA ไม่รู้ผล RA ไม่รู้ว่ามีคิว
 * ของจริงตอนตรวจระบบ 2026-08-28: **ทะเบียน 17 ใบค้าง `pending_legal` ทั้งหมด**
 * เก้าใบค้าง 28–34 วัน ทั้งที่เอกสารครบทุกใบ (ดู lib/tax/registrationNotify.js)
 *
 * 🪤 **ทำไมไม่ใส่ `'excise_registration'` ลง `entityTypes`**: เธรดกลางเปิดอยู่กับ
 * ทะเบียนทุกใบ (`UpdateThread` บนหน้ารายละเอียด) ⇒ ทุกคอมเมนต์จะไหลเข้ากระดิ่ง
 * แล้วคำร้องถูกดันตกขอบกล่อง 30 แถวอีกรอบ = ย้อนกลับไปเป็นปัญหาที่มติ 2026-08-20
 * เพิ่งแก้ · `kinds` เลือกเฉพาะ "เหตุการณ์ที่ต้องลงมือ"
 *
 * ⚠️ ชุดนี้ต้องตรงกับ kind ที่ยิงจริงเสมอ — `notifyRegistration` ยิง `excise_${action}`
 * เพิ่ม action ใหม่แล้วลืมเติมที่นี่ = แจ้งเตือนเงียบหายจากกระดิ่งโดยไม่มีอะไรฟ้อง
 * `notifications.test.mjs` อ่านซอร์สจริงมาเทียบกับลิสต์นี้ ดริฟต์แล้วแดง */
export const EXCISE_BELL_KINDS = Object.freeze([
  'excise_submit',   // SA ยื่นขึ้นทะเบียน → ฝ่าย RA (คิวตรวจ)
  'excise_approve',  // อนุมัติแล้ว → เจ้าของใบ (เอาไปออกใบยื่นได้)
  'excise_reject',   // ตีกลับ → เจ้าของใบ (ต้องแก้ตามเหตุผล)
 ...
 *
 * ทำไมต้องเข้า: รอบบริการไม่ได้ถูกกวาดจาก cron (กวาดตอนเปิดทะเบียนต่อสัญญา) ⇒
 * ถ้าไม่เตือน ไซต์ที่รอบหมดจะรู้ตอนลูกค้าโทรมาถาม
...
export const SERVICE_BELL_KINDS = Object.freeze([
  ...
  /* 🔴 หัวหน้า TS แจ้งช่างให้กลับไปเก็บงานหน้างาน → คนที่ถูกมอบหมายบนนัดของใบนั้น
     ⚠️ ยิงด้วย `entityType: 'dept_request'` ซึ่งอยู่ในกล่องอยู่แล้ว — ประกาศไว้ที่นี่
       เพราะทะเบียนนี้คือที่เดียวที่ตอบได้ว่า "กระดิ่งของโมดูลบริการมีเรื่องอะไรบ้าง"
     ⚠️ **href ต้องชี้ไปจอของฝ่ายบริการ ไม่ใช่หน้าคำร้อง** — ช่างเปิดหน้าคำร้องไม่ได้
       (role `ts` ไม่มี requests:answer/costing:view ⇒ 403) กระดิ่งจะพาไปชนกำแพง */
  'survey_send_back',
  /* ⭐ ช่างกด "ส่งงาน" หน้างานแล้ว → หัวหน้า TS ที่ส่งผลได้ (มติ 2026-09-21) — ถึงคิวเคาะ
 ...
 * สรรพสามิต และรอบบริการ: เอา **เหตุการณ์ที่ต้องลงมือ** ไม่เอาทั้ง entity
 *
 * ทำไมต้องเข้า: ตัวไล่ปิดร่างตามใบเสนอราคา (`contractQuotationSync`) ยิงแจ้งเตือน
 * มาตั้งแต่ 2026-08-22 แล้ว แต่ยิงด้วย `entityType: 'sales_contract'` ซึ่ง**ไม่อยู่ใน
 * ทะเบียนนี้** ⇒ ไปโผล่ที่หน้าเต็ม `/notifications` อย่างเดียว · ข้อความที่บอกว่า
 * "ร่างสัญญาถูกยกเลิกตามใบเสนอราคา" จึงไม่เคยถึงตาเจ้าของใบเลย ทั้งที่เป็นเรื่องที่
 * เขาต้องรู้ทันที (ร่างที่พิมพ์ค้างไว้ใช้ไม่ได้แล้ว)
 *
 * 🪤 **ทำไมไม่ใส่ `'sales_contract'` ลง `entityTypes`** ทั้งที่สั้นกว่า: นอกจากเหตุผล
 * เดิม (ลากทุกความเคลื่อนไหวเข้ากล่องจนคำร้องตกขอบ 30 แถว) ยังมีด่านแข็งอีกชั้น —
 * เทสต์ `notifications.test.mjs` บังคับว่าทุก `entityType` ในกล่องต้องมีเธรดใน
 * `UPDATE_ENTITIES` และสัญญา**ไม่มีเธรด** ⇒ ใส่แล้วเทสต์แดงทันที
 *
 * ⚠️ ชุดนี้ต้องตรงกับ kind ที่ยิงจริง · notifications.test.mjs กวาดทั้ง src หาไฟล์ที่
 *    ยิง `entityType: 'sales_contract'` มาเทียบ ดริฟต์แล้วแดง */
export const CONTRACT_BELL_KINDS = Object.freeze([
  // ใบเสนอราคาที่สัญญาอ้างถึงถูกปิด → เจ้าของใบ + คนที่สร้าง (ร่างถูกยกเลิกตาม/ใบที่ออกเลขแล้วแค่เตือน)
  'contract_quotation_closed',
  // ใบที่ค้างขั้น "รอลงนาม" เกินเกณฑ์ → เจ้าของใบ (หนึ่งคนหนึ่งเด้งต่อวัน)
  'contract_signature_overdue',
  /* ใบที่รอ AE Supervisor (เอกสารแทนสัญญาที่แนบแล้ว + ใบรอรับรองการลงนาม) → AE Sup
     หนึ่งคนหนึ่งเด้งต่อวัน · ของจริงที่ทำให้ต้องมี: เอกสารแทนสัญญาแนบแล้วค้าง 12 วัน
     โดยไม่มีใครกด เพราะช่องทางเดียวคือป้ายตัวเลขบนเมนู */
  'contract_approval_pending',
  // AE Sup กดอนุมัติ/รับรองแล้ว → เจ้าของใบ + คนสร้าง (ไปผูกเข้าใบสั่งขายต่อได้)
  'contract_approved',
]);
