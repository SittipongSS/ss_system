...
/* เพดานจำนวนจุดที่ยัง `.from(<ตารางในทะเบียน>).select(...).eq('id', …)` เอง
   ...
   **สัญญาแม่** ของบันทึกที่ผ่าน `loadScoped` มาแล้ว (`addenda/[id]` · `issue` · `document`)
   ...
   2026-09-16 — **59 → 60** (TS แจ้ง "ไม่พบจุดนี้หน้างาน" · มติข้อ 23 · mig 0362) ·
   `service/intake/site-not-found/route.js` (sales_orders) อ่านใบเพื่อตรวจสามอย่างก่อนเขียนธง
   ลงบรรทัด: ใบยังอนุมัติและไม่ถูก Rev. ทับ · ใบเป็นใบย้อนหลัง · ชื่อใบไว้ใส่ audit
