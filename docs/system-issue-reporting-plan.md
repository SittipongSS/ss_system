...
> สถานะ: **รอตรวจ** · ตรวจกับโค้ดเมื่อ 2026-08-07 · ครบทั้ง 3 ก้อน · mig 0223 รันบน Supabase แล้ว · รอ smoke test ด้วยบัญชีจริง (ดู "เหลือทำ" ท้ายไฟล์)
...
## 6. Schema — migration 0223 (Q22, Q23)

> ⚠️ **เลขเลื่อนจาก 0219 → 0223** — สาย RD merge เข้า main ก่อนและกินเลข 0219–0222 ไป
> ตามกติกาโปรเจกต์ ไฟล์ที่มาทีหลังเลื่อนเลข · **เนื้อ SQL ไม่เปลี่ยนแม้แต่บรรทัดเดียว**
> และฐานจริงรันไปแล้วในชื่อ 0219 เมื่อ 2026-08-07 ⇒ **ไม่ต้องรันซ้ำ**
...
```sql
-- ============================================================
--  Migration 0223: ระบบแจ้งปัญหาระบบ
...
|---|---|---|
| 1 · หลังบ้าน ✅ | migration 0223 · `lib/issues/*` · `/api/issues` · ลงทะเบียน `UPDATE_ENTITIES` · `driveEntityMap` · เทสต์ 4 ไฟล์ (36 เคส) | ยังไม่มีหน้าจอ |
...
## รอบแก้หลังใช้งานจริง (2026-08-11) — หน้ารายการ

ระบบถูกใช้จริงแล้ว (19 เรื่องจากผู้ใช้ 11 คนใน 4 วัน) · สามอาการที่เจอตอนตรวจหน้ารายการกับของจริง

**1. แท็บ "ทั้งหมด"/"ที่ฉันรับผิดชอบ" ว่างเปล่าหลังกดรับเรื่อง**
`load()` ล้าง `extra` เป็น `null` แต่ effect ที่ดึง `extra` ผูกกับ `[admin, tab]` เท่านั้น ⇒ กด "รับเรื่อง"
หรือแจ้งเรื่องใหม่ขณะอยู่แท็บนั้น แล้วรายการกลายเป็นว่างจนกว่าจะสลับแท็บไปกลับ
⇒ รวมการโหลดเป็นรอบเดียวที่ผูกกับแท็บด้วย (`load` ขึ้นกับ `[admin, tab]`)

**2. API ล่มแล้วหน้าบอกว่า "ไม่มีเรื่องในถังนี้"**
ของเดิม `r.ok ? r.json() : { items: [] }` + `.catch(() => setExtra([]))` ⇒ กลืน error เป็นลิสต์ว่าง
⚠️ แอดมินอ่านว่า "ไม่มีงานค้าง" ซึ่งตรงข้ามกับความจริง · ตอนนี้โยนผ่าน `describeResponseError` ขึ้นแถบ error

**3. สลับแท็บแล้วเห็น "ไม่มีเรื่องในถังนี้" แวบหนึ่ง**
การดึงของแท็บไม่มี `loading` ของตัวเอง ⇒ EmptyState โผล่ก่อนข้อมูลมา · ตอนนี้อยู่ในรอบ `load` เดียวกัน
โครง `Workspace loading` จึงคลุมทั้งหมด

**4. เรื่องที่ปิดแล้วลอยเหนือเรื่องที่ยังไม่มีใครรับ**
`sortIssueQueue` เรียง **ผลกระทบก่อนเวลา** ซึ่งถูกสำหรับคิวที่ยังเดินอยู่ แต่ถูกใช้กับทุก query รวมแท็บ
"ทั้งหมด" ⇒ `IS-26080002` ที่ถูกปฏิเสธไปแล้ว (ตั้งไว้ `blocked`) ลอยอยู่บนสุด
⇒ เพิ่มชั้นแรก: **เรื่องที่จบแล้วไปท้ายเสมอ** · ลำดับผลกระทบยังเป็นตัวตัดสินของกลุ่มที่ยังเดินอยู่เหมือนเดิม
...
2. ~~ตั้ง webhook ห้องผู้ดูแลระบบ~~ — **ผู้ใช้ตัดสินว่าข้ามไปก่อน (2026-08-07)**

   โค้ดฝั่ง webhook พร้อมแล้วและไม่พังเมื่อไม่มี URL (`sendChat` ข้ามเงียบตามกติกาเดิม)
   ตั้งทีหลังได้ทุกเมื่อที่หน้าตั้งค่า Chat webhooks (space "ผู้ดูแลระบบ") หรือ env `CHAT_WEBHOOK_ADMIN`

   ⚠️ **ผลที่ตามมา — เรื่องใหม่จะไม่มีสัญญาณถึงใครเลย** แอดมินต้องเปิด `/support` ดูคิวเอง
   กระดิ่งแจ้งเตือนช่วยไม่ได้ในขั้นนี้ เพราะ `recipients` ของเธรดมีแค่ผู้แจ้งกับผู้รับผิดชอบ
   และตอนเรื่องเข้ามายังไม่มีผู้รับผิดชอบ (มติ Q11 + มติ 14 ที่ห้าม fan-out ให้ทุกคนในฝ่าย)

   **ถ้าพบว่าเรื่องค้างเพราะไม่มีใครรู้** ให้เลือกทางใดทางหนึ่ง ไม่ใช่ทั้งสอง:
   ตั้ง webhook ตามเดิม · หรือเปิดข้อยกเว้นให้ `recipients` ส่งถึงแอดมินทุกคนเฉพาะเรื่องใหม่
   (ทางหลังขัดมติ 14 — ต้องคุยก่อน ไม่ใช่แก้เงียบ ๆ)
