...
- หัวข้อในเนื้อมีเลขข้อ: 1. Product Overview · 2. Market Positioning · 3. Functional Information · 4. Checklist Project · 5. Certification & Documents · 6. ภาพประกอบ — หัวข้อที่ไม่มีแถวไม่พิมพ์และเลขถัดไปเลื่อนขึ้น · ลายเซ็นไม่มีเลข · **เนื้อสเปค (ป้ายในตาราง) ยังชุดเดียวทั้งสองภาษา**
...
- **ตัดหน้าตามหัวข้อ**: หัวข้อหนึ่งอยู่แผ่นเดียวกันเสมอ (ลงที่เหลือไม่พอ = ย้ายทั้งหัวข้อ) · เฉพาะหัวข้อที่สูงกว่าแผ่นเปล่าจึงตัดทีละแถว แผ่นต่อเปิดด้วย "เลขข้อ. หัวข้อ (ต่อ)" + หัวตารางซ้ำ · ลายเซ็นไม่แบ่ง
  ...
  - ผลที่ตามมา: ใบมาตรฐาน (17 แถว · 4 เอกสาร) = **2 แผ่น** ลายเซ็นอยู่แผ่นสอง (เหลือจริง 3.5mm) · ภาพ 5–6 รูป = หัวข้อภาพเต็มแผ่นของตัวเอง แล้ว**ลายเซ็นอยู่แผ่นถัดไปคนเดียว** (ตามกติกาไม่ตัดหัวข้อ — รอเจ้าของตัดสินถ้าไม่ต้องการ) · ที่อยู่ยาวหลายบรรทัด + ช่องสเปคเต็มเพดาน = แผ่นแรกมีแต่หัวกับกล่องผู้ซื้อได้
...
- ลายเซ็น (มติผู้ใช้ 22/09 รอบสาม "final review ต้องปรับให้เหมือน QT และ SO" + "ชื่อ ตำแหน่ง ขอเป็นชื่อเต็ม") — **กล่องชุดเดียวกับใบเสนอราคา/ใบสั่งขาย** (`signatureSection` ของ `documentShell` · สี่คอลัมน์ผ่าน `--sig-cols: 4` · FM-SA-04 บังคับช่องกว้างเท่ากัน `minmax(0, 1fr)` + ชื่อตัดในช่อง ตามที่ตัวจองหน้าคิด — `1fr` ของเปลือกให้ชื่อที่ไม่มีจุดตัด (อีเมลสำรอง) ถ่างช่องจนก้อนลายเซ็นสูงเกินที่จอง 30mm · QT/SO คง `1fr` เพราะแบ่งหน้า v4 จองแถวตามแบบถ่างช่อง) ใต้หัวข้อ "Final Review & Approval" (ไม่มีเลขข้อ):
  ...
  - หัวข้อลายเซ็นห่างก้อนบน 2.5mm (หัวข้อเนื้อ 5mm) ⇒ ก้อนลายเซ็นทั้งก้อนสูง 44.32 เท่ากล่องเดิม ⇒ ใบมาตรฐานยังสองแผ่น
