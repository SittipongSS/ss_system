...
| ไม่แปลอะไร | ข้อความที่คนกรอก (ชื่อลูกค้า ที่อยู่ เงื่อนไขชำระ หมายเหตุ) — ลูกค้าต่างชาติกรอกเป็นอังกฤษมาแต่ต้นอยู่แล้ว การเพิ่มคอลัมน์ En ที่ไม่มีใครกรอกคือหนี้ที่จ่ายเปล่า |
| **ชื่อสินค้า FG — เลือกตามภาษาของใบ** (เพิ่ม 2026-08-20) | เดิมอยู่ฝั่ง "ไม่แปล" ⇒ ใบอังกฤษได้ชื่อไทยกลางตารางที่เหลือเป็นอังกฤษ · **แต่ชื่ออังกฤษมีอยู่ในฐานข้อมูลสินค้าแล้ว** (`products.productDescriptionEn`) จึงไม่ใช่คอลัมน์ที่ต้องเพิ่ม — ใบภาษาไหนใช้ชื่อภาษานั้นก่อน **ไม่มีก็ตกไปอีกภาษา ไม่แปลให้เอง** · ชื่อสองภาษาถูกตรึงลง `quotation_lines.metadata` (`descriptionTh`/`descriptionEn`) ตอน sync จาก master ⇒ ใบที่อนุมัติแล้วพิมพ์ซ้ำได้เหมือนเดิมแม้ชื่อใน master ถูกแก้ทีหลัง (หลักการเดียวกับราคา/หน่วยที่ freeze อยู่แล้ว) · บรรทัดที่พิมพ์เอง (ไม่ผูก FG) และใบเก่าที่ยังไม่มีคู่ภาษา พิมพ์ข้อความเดิม · **แบรนด์ยังเป็นอังกฤษก่อนเสมอทุกภาษา** ตามกติกาเดิมของ `productBrandName` |
...
| ⛔ ห้าม | เอา `docLanguage` **หรือคู่ภาษาของบรรทัด** ไปใส่ `quotationApprovalContent` — `approvalFingerprint` ของใบที่อนุมัติแล้วทุกใบบน prod ถูกเก็บไว้และเทียบกับค่าที่คำนวณสดตอนกด "ส่งลูกค้า" เพิ่มคีย์ = ส่งใบไม่ได้ทั้งระบบ |
