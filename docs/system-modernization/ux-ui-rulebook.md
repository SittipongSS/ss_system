...
โครงสร้างส่วนหัวของหน้า (นอก Card) จากซ้ายไปขวา:

1. ลูกศรย้อนกลับอยู่มุมซ้ายบนเสมอ สำหรับหน้าที่เข้ามาจากหน้าอื่น
   เช่นหน้ารายละเอียดหรือหน้าย่อย; หน้า landing ของระบบไม่ต้องมี
2. ชื่อหน้า (Page title) อยู่แถวเดียวกับลูกศรย้อนกลับ
3. Action ระดับหน้า/ระดับ entity เช่น แก้ไข ลบ พัก ปิด อยู่ขวาบน
   นอก Card แถวเดียวกับลูกศรย้อนกลับ

กฎ:

- Action ขวาบนนอก Card สงวนไว้สำหรับสิ่งที่กระทำต่อ entity ทั้งตัว
  ของหน้านั้น ไม่ใช่ action ของข้อมูลย่อยภายใน Card
- Action ของเนื้อหาภายใน Card เช่น ปุ่มเพิ่มรายการ อยู่ขวาสุดของ
  Card header ของการ์ดนั้น ไม่ยกขึ้นไปปนกับ action ระดับหน้า
- กฎนี้รวมหน้า list ด้วย: ปุ่มเพิ่ม/สร้างรายการอยู่ที่ Card header ของ
  การ์ดทะเบียน ไม่อยู่บน Page header แม้หน้านั้นมีการ์ดตารางเดียว
  (มติ 20 กรกฎาคม 2026)
- ปุ่มแก้ไข/ลบ ระดับหน้าใช้ icon button (`.btn-icon`) พร้อม `aria-label`
  และ `title`; ปุ่มลบใช้โทน danger
- ลูกศรย้อนกลับพาไปหน้าแม่ตามโครงสร้างข้อมูล ไม่ใช่ browser history ล้วน
  เพื่อให้พฤติกรรมคงที่เมื่อเข้าหน้าผ่านลิงก์ตรง
- ตัวอย่างอ้างอิง: หน้ารายละเอียดดีล (`sales-planning/deals/[id]`)
