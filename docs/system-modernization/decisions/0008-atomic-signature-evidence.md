...
## Amendment — Signature Evidence hard-delete boundary

- Signature Evidence เป็น immutable child record ที่อ้างกลับไปยังเอกสารต้นทาง จึงห้าม hard delete Quotation/Sale Order ที่มี evidence แม้ active pointer บนเอกสารจะถูกล้างหลังยกเลิกหรือคืนเป็น Draft
- Sale Order hard delete เป็น cleanup action สำหรับ Draft ที่ยังไม่เคยเข้าสู่ signed workflow เท่านั้น; เอกสารสถานะอื่นต้องใช้ cancel/void flow เพื่อเก็บ audit trail และคำนวณ Actual ใหม่
- UI ต้องซ่อน destructive action เมื่อเอกสารไม่ผ่าน policy แต่ authority หลักยังอยู่ที่ API ซึ่งตรวจทั้งสถานะและ historical evidence ก่อน delete
- Foreign Key `document_signature_evidence_salesOrderId_fkey` เป็น safety boundary ไม่ใช่ข้อมูลเสียหาย; application ต้องแปลงกรณีนี้เป็นข้อความที่แนะนำ `ยกเลิก SO` แทน raw database error

## Addendum 2026-07-21 — การประทับลายเซ็นลงเอกสารที่ออกจริง

- รูปลายเซ็นที่ฝังบนเอกสารที่ออกแล้ว (issued snapshot) มองเห็นโดยทุกผู้ใช้ที่มีสิทธิ์ดูเอกสารนั้น (`salesplan:view`) — กว้างกว่า Signature Vault ที่เป็น owner-only โดยเจตนา เพราะลายเซ็นบนเอกสารธุรกิจย่อมถูกเห็นโดยผู้อ่านเอกสารเป็นปกติ (PR #626)
- ลายเซ็นผู้เสนอราคาเป็น visual stamp เท่านั้น: ใช้ลายเซ็น active ของเจ้าของใบ ณ เวลาตรึง snapshot ไม่มี evidence/consent รายใบกำกับ ดังนั้นหาก snapshot ถูก regenerate ภายหลัง อาจได้ลายเซ็นเวอร์ชันใหม่กว่าตอนออกครั้งแรก (PR #628)
- ลายเซ็นผู้อนุมัติไม่มีปัญหาข้อข้างต้น เพราะถูก pin ผ่าน Signature Evidence (signature version FK) ตั้งแต่ตอนอนุมัติ
