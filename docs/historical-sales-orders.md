...
> สถานะ: **รอตรวจ** · ตรวจกับโค้ดเมื่อ 2026-09-24 (ยกเลิกหลังอนุมัติ มติ 24/09 · mig 0387 — §8D · §9.2) · **โมเดล 22/09 (mig 0374) แทนโมเดล "เกิดเป็นอนุมัติแล้ว" ของ 0360** ·
...
> ราคาของแผนไหลกลับลงบรรทัด · เหตุของจำนวนไม่เต็ม · ปุ่มลบ/placeholder/กล่องสรุปแบบใบเสนอราคา — **ข้อที่ยังต่าง 9 ข้อรอเจ้าของรับรอง** (§8A)
...
| `cancel` | ได้ทุกสถานะ (อนุมัติแล้ว = ผู้จัดการฝ่ายขายที่อนุมัติได้ · มติ 24/09) · **trigger ยกเลิกเอกสารแทนสัญญาให้ในทรานแซกชันเดียว** · **งวดยกมาเป็นโมฆะตามใบ** (รอตรวจ → ฐานตีกลับให้ · รับรองแล้ว → คงแถว + หมายเหตุบังคับ ≥ 10 ตัวอักษร · mig 0387) — response พก `contractVoided` · `contractVoidedLabel` · `openingVoided` ให้ toast · ติดด่านเฉพาะ**งวดปกติ**ที่รับรองแล้ว/รอบัญชีตรวจ (`historicalCancelBlock` · trigger 0387 กันซ้ำ) |
...
**งวดปกติ** = "ผู้จัดการฝ่ายขายยกเลิกใบนี้ไม่ได้จนกว่าบัญชีถอนคำรับรอง" (ปิดทางแก้ "ยกเลิกแล้วคีย์ใหม่" ชั่วคราว) · `HISTORICAL_STATUS_NOTE`
...
|---|---|---|
| ① | ลูกค้าและสัญญา | ลูกค้า · AE (`ae`/`senior_ae` ล็อกเป็นตัวเอง) · ทีม (เฉพาะตอน AE อยู่ ≥2 ทีม) · ชนิดเอกสารแทนสัญญาเป็นไทล์จาก `EXTERNAL_DOC_KINDS` ทั้งชุด (5 ตัวรวม "อื่น ๆ") **ไม่มีค่าตั้งต้น** · เลขอ้างอิง · ช่วงวัน · **ไฟล์สัญญา** · เลขเอกสารเดิม 3 ช่อง · **ภาษีมูลค่าเพิ่ม 2 ไทล์ = ตัวเลือกของใบเสนอราคา** (`QUOTE_VAT_OPTIONS`: "รวม VAT แล้ว" · "+ VAT 7% ท้ายใบ" — ไม่มีค่าตั้งต้น · ไทล์ "ถอด VAT" ถอดแล้ว 23/09) · หมายเหตุ |
| ② | ไซต์ โซน และรายการ | เลือกจาก**ทะเบียนไซต์ของลูกค้ารายนั้น** (`includeInactive=0`) · โซนที่ปิดใช้งานขึ้นแต่กดไม่ได้ · **ช่องค้นหนึ่งช่อง** (รหัสไซต์/ชื่อไซต์/ชื่อโซน/รหัสโซน) · **การ์ดไซต์พับได้** (กางเองเมื่อมีโซนที่เลือกไว้ · ตรงคำค้น · โหลดไม่สำเร็จ · หรือลูกค้ามีไซต์ ≤4) — **การ์ดไซต์มีแต่ตัวติ๊กโซน** (ติ๊ก = เพิ่มบรรทัด) · ⭐ **ตารางรายการตารางเดียวใต้การ์ด = ตารางใบเสนอราคา** (มติ 23/09 · `components/salesPlanning/QuoteLineCells.js` ตัวเดียวกับ `QuotationLineItems`): # · รายการ (แพ็คเกจ 02-001 ผ่าน `quoteLineFromProduct` · ข้อมูล FG · **ไซต์ · โซน** · **รอบบริการที่ขายไว้** ใต้คำอธิบาย) · จำนวน (+ หน่วยจากสินค้า · ล็อกตั้งแต่ยังไม่เลือกแพ็คเกจ) · ราคา/หน่วย (ทะเบียน · ปิดตั้งแต่ยังไม่เลือกแพ็คเกจ) · ส่วนลดรายการ (ไม่ลด/%/บาท) · จำนวนเงิน (`quoteLineNet` · ขีดจนกว่าจะมีจำนวนเต็ม > 0 และแพ็คเกจที่ตั้งราคาแล้ว — `historicalZoneLineAmount` · จำนวนไม่เต็ม/ศูนย์ขึ้นเหตุใต้ช่องจำนวน) · ปุ่มลบ (= ถอนติ๊ก · ปิดเมื่อทะเบียนอ่านไม่ครบ) · **กล่องสรุปท้ายตารางของใบเสนอราคา** (`QuoteLineTotals` + `historicalTotalsView` ตัวเดียวกับขั้น ④) · เตือนเมื่อโซนนั้นมีรอบขายของใบอื่นที่ยังเดินอยู่ · ขั้นนี้**ไม่มีแถบสรุปข้างขวา** · 🚫 ถอด: แพ็ค · ยอดที่พิมพ์เอง · ปุ่ม "ราคาแพ็คเกจ × แพ็ค × เดือน" · ช่องระยะสัญญา · คอลัมน์ "โซน" |
...
| ④ | ตรวจและส่งอนุมัติ | (ไม่มีแถบสรุปข้างขวา) สรุป · **ตารางรายการฝั่งอ่านตัวเดียวกับหน้าใบสั่งขาย** (`QuotationReadOnlyLineItems` + `showServiceRounds` + `showInstallationPoint` — ไซต์ · โซน และรอบใต้คำอธิบาย · ท้ายตารางใช้ป้ายของใบเสนอราคา) · **"หลังบันทึก จะเกิดอะไร"** (AE Sup → FN → TS → SA · บอกด้วยว่าผู้คีย์ที่เป็นผู้ตรวจเองอนุมัติใบตัวเองไม่ได้) · ใบที่อาจซ้ำ · คำเตือน · ปุ่ม **"บันทึกและส่งอนุมัติ"** |

⚠️ **ข้อที่ยังต่างจากตารางใบเสนอราคา — `รอตรวจ` (รอเจ้าของรับรองทีละข้อ)** เหตุผลด้านข้อมูลอยู่หลังขีด:
1. จำนวนเริ่มที่**ว่าง** (ใบเสนอราคาเริ่มที่ 1.00) — ว่าง = ตีกลับ ไม่นับเป็น 1 (เดาจำนวนแทนผู้คีย์คือบั๊กที่มตินี้แก้)
2. จำนวนต้องเป็น**จำนวนเต็ม** (ใบเสนอราคารับ 1.5) — RPC 0379 + `service_zone_terms.packageQty` เป็นจำนวนเต็ม · ช่องยังเป็น `MoneyInput` ตัวเดียวกับใบเสนอราคา (โชว์ "12.00")
3. ไม่มี "+ แทรกหมายเหตุ" รายบรรทัด — หมายเหตุพิมพ์ลงใบเสนอราคาเท่านั้น ใบย้อนหลังไม่พิมพ์ และ body/แผนไม่มีช่องนี้
4. ไม่มีปุ่ม "เพิ่มสินค้า"/"เพิ่มรายการเอง" — เพิ่มบรรทัดด้วยการติ๊กโซน (หนึ่งโซนหนึ่งบรรทัด · บรรทัดที่ไม่ผูกโซนบันทึกไม่ได้)
...
6. กล่องสรุปไม่มี "หัก ส่วนลด" ท้ายใบ — แผน/RPC ของใบย้อนหลังคิดส่วนลดรายบรรทัดอย่างเดียว (เพิ่ม = migration ใหม่)
7. VAT ไม่ใช่ดรอปดาวน์ท้ายตาราง และ**ไม่มีค่าตั้งต้น** (ใบเสนอราคาเปิดที่ "+ VAT 7% ท้ายใบ") — เลือกที่ขั้น ① (form-design-rules §2) · ท้ายตารางบอกตัวเลือกในวงเล็บ
8. มีช่อง "ใช้แพ็คเกจเดียวกันทุกโซน" เหนือการ์ดไซต์ — ลูกค้าจริงมี 43 โซน (เลือกทีละบรรทัด = 43 ครั้ง)
...
1. ถ้ามี**งวดปกติ**ที่รับเงินในระบบหลังอนุมัติ (รับรองแล้ว/รอบัญชีตรวจ) — **ให้บัญชีถอนคำรับรอง/ตีกลับก่อน** (`historicalCancelBlock`
...
**แนะนำรัน 0387 ก่อน deploy** (DDL · SQL Editor · รันซ้ำได้) — รันก่อนไม่พังอะไร: โค้ดเดิมตอบ 400 ก่อนถึง UPDATE ทุกใบที่งวดมีเงิน/รอตรวจ
⚠️ สลับลำดับ (deploy ก่อน) = ผู้จัดการยกเลิกใบที่งวดยกมารอตรวจได้ แต่งวดค้าง `reported` บนใบที่ยกเลิก (คิว/ป้ายเมนูบัญชี) —
route เตือนดังในคำตอบ + audit (`HISTORICAL_CANCEL_SETTLE_SCHEMA_MISSING`) ไม่เงียบ · หัวไฟล์มีคำสั่งตรวจ ①②③ และบล็อกลองจริง (ROLLBACK)
