...
| [role-per-department.md](role-per-department.md) | **ยกเลิก role `staff`** (2026-08-28) — PC/PD/WH/QC/TS ได้ role ของตัวเอง · cap ตรงกับงานจริงตั้งแต่ชั้น role แทน "ถือกว้างแล้วแคบด้วยฝ่าย" · ย้ายบัญชีครบ + ถอดตัวแปลงช่วงเปลี่ยนผ่านแล้ว 2026-08-29 · **§7 ผังตำแหน่งฝ่ายขาย** (2026-09-24) Commercial Director · CM · AC Supervisor · Senior AC + กลุ่มตำแหน่ง + mig 0382/0383 | รอตรวจ |
...
|---|---|---|
| [fm-sa-04-document-model.md](fm-sa-04-document-model.md) | **ใบสเปคสินค้า FM-SA-04 — โมเดลเอกสารชุดใหม่** (มติ 2026-09-21) — สเปคในฐานข้อมูลไม่มีเลข ไม่มี Rev · เลขที่ FM-SA-04-DDMMYY-XXX + Rev + ด่าน AC → AE เจ้าของดีล → AE Sup อยู่ที่เอกสารที่ออกจากบรรทัด SO · แทนโมเดลสามชั้นของ 0364/0369 · mig 0370 รันแล้ว · ขึ้น prod #1764 · มติ 22/09 กระดาษ "รายละเอียดผลิตภัณฑ์": หัวแบบใบเสนอราคา · ภาษาตาม SO · เลขที่ DDMMYY-XXX-RR · ตัดหน้าตามหัวข้อ + เลขข้อ · ช่องลงนามแบบ QT/SO (รูปลายเซ็นจริง · ตำแหน่งเต็ม ใช้ร่วมกับ QT/SO) · ปริมาตรบรรจุ/จำนวนผลิตใน Product Overview (ขึ้น prod #1782 · สีแบบใบเสนอราคา + ภาพขึ้นแผ่นใหม่ #1785) · รอบสี่ (มติ 22/09): accent ที่ชื่อเอกสารที่เดียว + หัวข้อตามภาษา "ไทย / ENGLISH" แบบใบเสนอราคา (แบรนช์ `claude/spec-accent-headings`) · รอบห้า (มติ 23/09): หน้า "ออกเอกสาร" แบบใบเสนอราคา `/sales-planning/spec-documents/new` — เลขที่ออกตอนกดบันทึก ยกเลิกไม่เหลืออะไร · รอบหก (มติ 23/09): **ลบร่างที่ยังไม่เคยยื่นได้** เหมือนร่างใบเสนอราคา — AC/admin · ยื่นแล้ว/ดึงกลับ/ถูกตีกลับ ลบไม่ได้ (ใช้ยกเลิกเอกสาร) · **เลขที่ไม่นำกลับมาใช้** ตัวนับไม่ถอย · mig 0375 (คอลัมน์ `firstSubmittedAt` + RPC `delete_product_spec_document_draft`) รันแล้ว (ตรวจ 23/09) · รอบห้า–หก merge แล้ว (#1792) · รอบเจ็ด (มติ 23/09): **ซ่อนปุ่มยกเลิกช่วงร่าง** — ร่างที่ไม่เคยยื่นมีแค่ "ลบร่างเอกสารถาวร" · เคยยื่นแล้วยกเลิกกลับมา · ใบ active มีปุ่มปลายทางหนึ่งตัวพอดีทุกจอ รวมแถว "บรรทัดถูกถอด" บนการ์ดหน้า SO (แบรนช์ `claude/spec-void-hide-error-detail`) | รอตรวจ |
| [historical-sales-orders.md](historical-sales-orders.md) | **ใบสั่งขายย้อนหลัง (SO ย้อนหลัง)** — คีย์งานบริการที่ขายไปแล้วนอกระบบ · **มติ 2026-09-22 (mig 0374) ทับโมเดล "เกิดเป็นอนุมัติแล้ว" ของ 0360**: ฝ่ายขายทุกตำแหน่ง+admin คีย์ในฟอร์มหน้าเต็ม 4 ขั้น (ฟอร์มแก้ = ฟอร์มสร้าง) · **เลือกไซต์/โซนจากทะเบียนตอนคีย์** (`sales_order_lines."serviceZoneId"`) · ใบเกิดเป็นร่าง → **AE Sup อนุมัติ** ซึ่งออกเลขเอกสารแทนสัญญา ตรึงงวด และเปิดรอบขายของโซนให้ TS ในทรานแซกชันเดียว · **"งวดยกมา"** บัญชีรับรองครั้งเดียว (ไม่เข้ากองใบกำกับ) · TS เหลือแค่ตั้งรอบ · **ถอดสวิตช์ยกเว้นด่านเงิน** (ใบยอด 0 ผ่านด่าน② เองทุก origin) และถอด "TS ไม่พบจุด"/"ถอดจุด" (mig 0362/0366 ไปไม่ถึงแล้ว) · แก้ข้อมูลหลังอนุมัติ = ยกเลิกใบแล้วคีย์ใหม่ (trigger ยกเลิกสัญญาให้) · ธง `origin` + **เก็บยอดจริง กรองที่ตัวคำนวณ** (ไม่นับ Actual/FC/เป้า ทุกสถานะ) และดีลภาชนะ 1 ใบต่อลูกค้า × AE ยังเป็นฐานเดิมจาก 0360 (P1 ขึ้น prod 16/09 · #1723 · mig 0360 รันแล้ว) · **mig 0374 รันแล้ว 23/09** · เฟส 3 (คีย์ของจริง) เริ่มได้หลัง merge + UAT · **มติ 23/09 "3500 x 1 ชุด x 12 เดือน — ต้องไม่ต่างจากฟอร์มใบเสนอราคา"**: บรรทัดโซน = บรรทัดใบเสนอราคา (จำนวน × ราคา/หน่วยจากทะเบียน − ส่วนลดรายการ · VAT สองตัวเลือกของใบเสนอราคา · ถอดแพ็ค/ยอดพิมพ์เอง/ปุ่มลัด) แบรนช์ `claude/legacy-so-qt-lines` — ขั้น A (แผน/ตัวเขียน/ฟอร์มฝั่ง lib) + ขั้น B (จอ: แถวโซนวาดด้วย `QuoteLineCells` ตัวเดียวกับตารางใบเสนอราคา · VAT สองไทล์ · ขั้น ④ ใช้ตารางรายการฝั่งอ่าน · การ์ดโซนของหน้าใบ) ทำแล้ว · **mig 0379 `รอดำเนินการ` — รันบน prod ก่อน merge แล้ว deploy ทันที** · **มติ 25/09 (A14)**: ขั้น ② = ตารางใบเสนอราคา + ช่อง "ไซต์ · โซน" ในบรรทัด · ปุ่ม "เพิ่มรายการ"/"เพิ่มหลายโซน" · ส่วนลดท้ายใบ (ไม่ต้อง migration) · VAT ในกล่องสรุป · ถอด "ใช้แพ็คเกจเดียวกันทุกโซน" (แบรนช์ `claude/legacy-so-step2-qt`) · **A15** รื้อขั้น ① (หัวเอกสาร · การ์ด · ก้อนแดงหลังกดไปต่อทุกขั้น) · **A16** รื้อขั้น ③ งวดชำระ (คำถามสามทาง · ห่วงโซ่งวด · งวดสุดท้ายคิดให้ · แบ่งตามเดือนปฏิทิน · สัญญาครบรอบแบ่งได้ · ไม่มีแถบสรุปขวาทุกขั้น) · **A17** รื้อขั้น ④ "ตรวจแบบผู้อนุมัติ" (ใบที่อาจซ้ำบนสุด · แถวตรวจประโยคเดียวกับหน้าต่างอนุมัติ + "แก้ในขั้น …" · แผงบันทึกเหนือแถบท้าย · ถอดปุ่มบันทึกตัวที่สองที่ข้ามด่าน · ผู้อนุมัติ = "ผู้จัดการฝ่ายขาย") · **A18** บันทึกใบซ้ำที่ผู้คีย์ยืนยัน (ใบไหน·ใคร·เมื่อไร·เหตุผล ลง metadata โดยไม่แก้ฐาน · ยืนยันรายใบ · ผู้อนุมัติเห็นในหน้าต่างอนุมัติ + การ์ดหน้าใบ · ใบที่พบเพิ่มตอนเปิดใบ = เตือน) | รอตรวจ |
...
| [sales-contract-plan.md](sales-contract-plan.md) | ระบบสัญญาในบริหารงานขาย · **§5.0 = ทะเบียนแม่แบบที่เดียว** (ออกแบบกลิ่น `20260813` · บริการ `20260903` · บันทึกเพิ่มเติม `20260821c` · **จ้างผลิตยังไม่มีต้นฉบับ**) · Rev. · บันทึกเพิ่มเติม · เอกสารภายนอกใช้แทนสัญญา · ขั้นรับรอง AE Sup · **§5.6 ผู้อนุมัติยกเลิกสัญญาที่ลงนามแล้ว (มติ 24/09 · งานบริการหยุดตั้งแต่วันยกเลิก)** · mig 0278/0280/0282/0322/0323/0324/0327 รันครบ · ยังไม่ UAT ครบวงสาย generated | รอตรวจ |
...
| [so-documents-tab.md](so-documents-tab.md) | **แท็บ "เอกสาร" ของใบสั่งขาย** (มติ 2026-09-25) — รวมไฟล์สี่บ้านของใบ (แนบเพิ่ม · ยืนยันคำสั่งซื้อ · การชำระ · สัญญา) ให้เห็นที่เดียว · แนบเพิ่มหลังออกใบได้ทุกสถานะยกเว้นใบยกเลิก (entity `sales_order` · สิทธิ์ผ่านดีลของใบ) · ไฟล์ยืนยันคงตรึงที่เดิม · ลบได้เฉพาะคนแนบ+แอดมิน · ไฟล์ย้ายตามใบ Rev. · ไม่มี migration | รอตรวจ |
| [billing-cycle.md](billing-cycle.md) | **กำหนดวางบิล** (มติ 2026-09-25/26) — รอบวางบิลที่ทะเบียนลูกค้า → วันวางบิล/กำหนดชำระรายงวด (สองช่องแยก · "เลยกำหนด" ยังอ่าน dueDate) · แตะเลือกรอบ · SA+FN แก้รอบไม่ต้องอนุมัติ · ทะเบียน FN + กระดิ่งเจ้าของดีล/FN · mig 0389 รันแล้ว | รอตรวจ |
...
| [service-business-system-plan.md](service-business-system-plan.md) · [service-field-operations.md](service-field-operations.md) | มติเคาะแล้ว · **G-1 เสร็จไปแล้วโดยไม่มีใครอัปเดต** (FN + mig 0212 + mig 0192) · C-3 ลงเป็นงวดชำระ SO (mig 0245) · **F-1 เสร็จ 2026-08-27** (เมนู งานวันนี้/จัดคิวช่าง + `/service/today` + rename `zone`→`routeZone` mig 0304 + ปลดการ์ดระบบ) · มติใหม่ 2026-08-27: สัญญาบริการต่อยอด `sales_contracts` ไม่แยกตาราง · Zone เป็น entity ถาวรเกิดโยงจาก SO line · ส่วนสัญญา/งวด/ด่านเงิน **พักรอต้นฉบับสัญญาจ้างบริการ** — ตรวจกับ `billing-request-flow-plan.md` §1 ก่อนหยิบ | กำลังดำเนินการ · **เฟส 4 (งานเข้าใหม่ /service/intake) เสร็จ 2026-08-28 → [service-intake-phase4.md](service-intake-phase4.md)** · **F-6c คำร้องประเมินพื้นที่เป็นการ์ดบนหน้าจัดคิว + โมดัลลงคิวตัวเดียวของสองหน้า + เจ้าของด่าน "โซนยังไม่จัดสรร" เป็น TS (มติเจ้าของ 23/09) — รอตรวจ** · **F-6d ปิดปุ่ม "+ งานนอกรอบ" (พักรอประชุม) + ปุ่ม "ลบนัด" ของงานนอกรอบ · กติกาลบเดียวของจอกับ API `lib/service/visitDelete.js` (มติเจ้าของ 24/09) — รอตรวจ** · **F-6e โมดัลนัด + โมดัลลงคิวเป็นเปลือกเดียว แบบ A "สองคอลัมน์" (`ScheduleModalShell` · UI อย่างเดียว ก้อนที่ส่งเท่าเดิม · มติเจ้าของ 24/09) — รอตรวจ** |
| [site-survey-request-plan.md](site-survey-request-plan.md) · [site-survey-mockup.html](site-survey-mockup.html) · [site-survey-prototype.html](site-survey-prototype.html) | **คำร้องประเมินพื้นที่ SA → TS (2026-08-29)** — หัวข้อคำร้องตัวที่ 6 ของฝ่าย TS: SA ขอ → TS ลงคิววันเวลา+ช่าง → ไปวัด ก×ย×ส (เมตร) + รูป → สรุปจำนวนแพ็คเกจต่อโซน → ปิดสองฝั่ง · **ไม่มีตารางใหม่** เพิ่มคอลัมน์ + ขยาย CHECK (`dept_requests.dept` รับ TS · `service_visits.kind` รับ survey · ขนาดที่ `service_zones`) · มติเคาะครบ 21 ข้อ (§8) · ม็อกออกแบบ 14 จอ + **ม็อกเดินเรื่องเสมือนจริง 9 ขั้น + 4 ทางแยก** (คำร้อง → ผลถึง SA → ปิดเรื่อง) · 🅿️ สายใบเสนอราคาพักไว้ · **เฟส 1 (เปิดใบ → คิว TS → รับเรื่อง) ขึ้นแล้ว 29/08 · mig 0313/0314 รันแล้ว** · **แยก "วันเข้าพื้นที่" ออกจาก "วันส่งผล" ทั้งฝั่ง SA และ TS (mig 0368 · 21/09 — §4A)** · **ลงคิวได้ทั้งจากหน้าใบและการ์ดบนหน้าจัดคิว ด้วยโมดัลตัวเดียว · "ลงคิวใหม่" ไม่ขึ้นหลังเข้าพื้นที่แล้ว (มติเจ้าของ 23/09 — รอตรวจ)** · **โมดัลลงคิวหน้าตาแบบ A สองคอลัมน์ เปลือกเดียวกับโมดัลนัด + แผงด่าน + บรรทัดผลลัพธ์ + ชิปผู้ขอ (มติเจ้าของ 24/09 — รอตรวจ)** · **หน้าคำร้องแบบ A ไทม์ไลน์งาน 6 ขั้น + แถบตอนนี้ + ผลรายพื้นที่ · รางขั้นเดินตามนัด (มติเจ้าของ 25/09 — รอตรวจ · §10.4)** · **จอหน้างานแบบ A: รายการพื้นที่ → หน้าวัดทีละพื้นที่ (≤999 สองหน้า · ≥1000 สองบาน · ≥1200 รางของหัวหน้า) · หัวงานใหม่ · แผ่นส่งงานไม่มีค่าตั้งต้น · ส่งกลับทีละข้อ + ช่างติ๊กรายข้อ · ภาพผังย้ายไปแท็บสรุปส่งผล · ตัดพื้นที่ที่ยังไม่วัดได้ · ไม่เก็บร่างในเครื่อง · ไม่มี migration (มติเจ้าของ 25/09 — รอตรวจ · §10.5)** | กำลังดำเนินการ |
