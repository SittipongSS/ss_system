...
| [deal-forecast-by-category.md](deal-forecast-by-category.md) | **ก่อนแตะมูลค่าคาดการณ์ของดีล** (mig 0264) — มูลค่า = แถวรายหมวด (หมวด · จำนวน · หน่วย · ราคา/หน่วย) · ยอดรวม **ล็อก** คิดจากแถวเท่านั้นแล้วเขียนลง `projectValue` เหมือนเดิม · หมวดของดีล = หมวดแถวแรก (ตัวกรองขั้นตอนไทม์ไลน์ — template มาจาก **ประเภทดีล**) · ดีลเก่าไม่ backfill · สูตรอยู่ที่ `lib/sales/dealValueItems.js` ที่เดียว |
| [empty-value-rule.md](empty-value-rule.md) | **ก่อนเขียนค่าที่อาจว่าง** — ค่าว่างทั้งระบบขึ้นว่า `N/A` (`naText()` ใน lib/format) · ใช้กับที่แสดงผลเท่านั้น ห้ามใส่ลง `value` ของช่องที่พิมพ์ได้ · `0`/`false` ไม่ใช่ค่าว่าง · คำที่มีความหมายเฉพาะ ("ไม่มีกำหนด" · "ระบบ") เก็บไว้ |
