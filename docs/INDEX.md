...
| [customer-delete-guard.md](customer-delete-guard.md) | **ก่อนแตะเส้นลบลูกค้า หรือเพิ่มตารางที่ถือ `customerId`** — FK เป็น SET NULL ⇒ ฐานไม่ได้กันให้ ด่านจริงอยู่ที่โค้ด · ทะเบียน 25 ตารางประกาศที่เดียว (`CUSTOMER_REFERENCE_TABLES`) · `npm run check:refs` เทียบกับฐานจริงไม่ให้ตกหล่น |
