...
| [so-pending-approval-amount.md](so-pending-approval-amount.md) | **ก่อนแตะยอด Actual / ยอดใบสั่งขายบนจอใดก็ตาม** (mig 0353 · มติ 2026-09-11) — SO ที่ **รออนุมัติ** โชว์ยอดเป็น **ตัวเลขแยก** "รออนุมัติ" ข้าง Actual ทุกจอ ห้ามบวกเข้า Actual/เป้า/% (สองจุดคัดลอกลง `sales_history` ถาวร) · นับเฉพาะ `pending_approval` · ยอดก่อน VAT · **เดือน = เดือนปัจจุบันเวลาไทยเสมอ** · cache `metadata.soPendingAmount/Count` เขียนโดย trigger คู่เดิม (แก้คู่กันเสมอ) · ตัวช่วยกลาง + `PendingApprovalAmount` ชิ้นเดียวทุกจอ · กอง **Won รอยื่น SO** ตัดดีลเก่าที่สร้างเป็น Won (`isLegacyWonAtCreate` · #1722) · ออกจากกองเมื่อ **Actual > 0** เท่านั้น (ใบอนุมัติ/ใบยื่นยอด 0 บาทยังนับ · มูลค่าดีล 0 ไม่นับ · #1725 + รอบห้า) · รอบห้าแก้ลิ้นชัก/งวดไตรมาส/คำเตือนเป้าทีม/แถบเตือนโหลดไม่ครบ (#1733) · **รอบหก: ยอดดีล = ใบที่ลูกค้ารับ** (mig 0361 ยังไม่รัน · backfill 41 ดีล) · ใบ SO อนุมัติ 0 บาท = ส่วนลด 100% ถือว่าจบ · รายงาน Excel อ่านใบที่ลูกค้ารับ | รอตรวจ |
