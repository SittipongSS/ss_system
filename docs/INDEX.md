...
| [ui-visibility-rule.md](ui-visibility-rule.md) | **ก่อนตัดสินใจว่าปุ่ม/ช่องไหนโผล่บนจอ** — ไม่มีสิทธิ์ = **ไม่โชว์** · ติดเงื่อนไขของใบ = **โชว์เสมอแล้วบอกเหตุ** (`visible` vs `allow` ของ `recordLifecycle` · `GatedAction` สำหรับปุ่มเดี่ยว) · `disabled` เปล่า ๆ ใช้ได้เฉพาะสถานะชั่วคราวกับเงื่อนไขที่เห็นอยู่ในจอเดียวกัน · หน้าไม่มีสิทธิ์ = `AccessDenied` ตัวเดียวทั้งระบบ (ห้าม `return null`) · สามอย่างที่ไม่ใช่ด่านจึงซ่อนได้ (ทำไปแล้ว · ไม่มีของให้ทำ · เครื่องมือของจอ) — บังคับด้วยกฎ 8/9 ใน `systemRules.test.mjs` |
...
| [system-rules-guarded.md](system-rules-guarded.md) | **กฎที่มีเทสต์บังคับแล้ว (9 ข้อ)** — ทุก API route ต้องมีด่านสิทธิ์ · `ADMIN_LOCKDOWN` ต้องเป็น true · ห้ามเขียน `frozenAt: null` · จออนุมัติต้องบอกผลลัพธ์ · ช่องที่พิมพ์บนเอกสารต้องล้างการอนุมัติ · อ่านก่อนเพิ่ม route/จออนุมัติ/ช่องบนใบเสนอราคา |
