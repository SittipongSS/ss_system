...
| [supabase-anon-default-privileges.md](supabase-anon-default-privileges.md) | **ก่อนแตะสิทธิ์ฐานข้อมูล / เมื่อ Security Advisor ขึ้นเตือน** (2026-09-02) — `mig 0336` ถอนสิทธิ์ `anon`/`authenticated` ออกหมดแล้ว แต่ `ALTER DEFAULT PRIVILEGES` ผูกกับ **role ที่สร้างอ็อบเจกต์** จึงครอบได้แค่ `postgres` · ชุดของ `supabase_admin` ยังแจก anon อยู่และ **ปิดเองไม่ได้** (`42501 permission denied to change default privileges`) · ยังไม่อันตรายเพราะอ็อบเจกต์ใน `public` เป็นของ postgres ทั้ง 113 ตัว · มีคำสั่งตรวจว่าหลุดหรือยังในไฟล์ | ระงับ |
...
| [deal-forecast-source.md](deal-forecast-source.md) | **ก่อนแตะยอด FC ของดีล** (mig 0337 · มติ 2026-09-02) — FC มี **ที่มา** เป็นข้อมูล: `manual` (ยอดที่ AE กรอก เก็บถาวรใน `forecastManualValue`) หรือ `quotation` (ยอดก่อน VAT ของใบที่ **อนุมัติแล้ว**) · ร่างไม่ขยับ FC · Rev. เดินตามเอง · **หลายเลขที่ = ไม่เดา ไม่รวม** ให้คนเลือก · ดีลเก่า **ไม่ backfill** ขึ้นคิว `/sa/forecast-review` · 🔴 mig ต้องรันก่อน deploy ไม่งั้นสร้างดีลไม่ได้ · **รายงาน FC รายหมวด (Excel)** แกนเดือน = วันที่ลูกค้ารับของ (`endDate` → `demandMonth` สหมิตร) · บังคับกรอกวันเริ่ม/วันสิ้นสุด |
| [so-pending-approval-amount.md](so-pending-approval-amount.md) | **ก่อนแตะยอด Actual / ยอดใบสั่งขายบนจอใดก็ตาม** (mig 0353 · มติ 2026-09-11) — SO ที่ **รออนุมัติ** โชว์ยอดเป็น **ตัวเลขแยก** "รออนุมัติ" ข้าง Actual ทุกจอ ห้ามบวกเข้า Actual/เป้า/% (สองจุดคัดลอกลง `sales_history` ถาวร) · นับเฉพาะ `pending_approval` · ยอดก่อน VAT · **เดือน = เดือนปัจจุบันเวลาไทยเสมอ** · cache `metadata.soPendingAmount/Count` เขียนโดย trigger คู่เดิม (แก้คู่กันเสมอ) · ตัวช่วยกลาง + `PendingApprovalAmount` ชิ้นเดียวทุกจอ · กอง **Won รอยื่น SO** ตัดดีลเก่าที่สร้างเป็น Won (`isLegacyWonAtCreate` · #1722) · ออกจากกองเมื่อ **มี SO ที่อนุมัติ** (แม้ยอด 0 บาท = ส่วนลด 100% · มติรอบบ่าย 16/09 กลับกติกา "Actual > 0" ของ #1725) · ใบที่ยื่นแล้วยอด 0 ยังอยู่ในกอง · มูลค่าดีล 0 ไม่นับ · รอบห้าแก้ลิ้นชัก/งวดไตรมาส/คำเตือนเป้าทีม/แถบเตือนโหลดไม่ครบ (#1733) · **รอบหก: ยอดดีล = ใบที่ลูกค้ารับ** (`quotationWonAmount` · **mig 0361 รันแล้ว** · backfill 38/38 ใบ กู้คืนได้จากไฟล์สำรอง + audit) · ย้อนรับใบคิดยอดใหม่ · รายงาน Excel อ่านใบที่ลูกค้ารับ และไม่ตัดดีลยอด 0 ที่มีใบ · 🪤 backfill **ไม่ขยับ** FC คงเหลือ/กองรอยื่น SO — ที่ขยับคือส่วนต่างของดีล Won · คิว "Won รอ SO" ตรวจแล้วไม่ต้องแก้ (คนละคำถามกับกองบนแท็บ) | เสร็จสมบูรณ์ |
| [sales-report.md](sales-report.md) | **ก่อนแตะรายงานยอดขาย `/sa/targets/report` หรือไฟล์ Excel ของมัน** (มติ 2026-09-22) — งวด "รายเดือน \| ช่วงวัน" แบบหน้าลีด · ช่วงวันปันเป้าตามวัน · เดือนเดียวทบยอดตั้งแต่ ม.ค. · จอกับไฟล์ผ่านตัวโหลด/ตัวคิดชุดเดียว (`salesReportData` · `reportSummary`) ห้ามคิดเลขซ้ำ · กดยอดในตารางกรองรายการใบพร้อมเทียบยอด | รอตรวจ |
...
| [master-code-scheme.md](master-code-scheme.md) | **ก่อนแตะรหัสที่ระบบรันเลขให้ทุกชนิด** (AR · FG · DL · PJ · PB · SV · SS · IS) — สวิตช์ "ระบบใหม่" ในโมดัลเพิ่ม · กติกา 4 หลัก/เลขรัน 10001 · เคาน์เตอร์ atomic (mig 0230) · **รหัสออกพร้อม insert ในทรานแซกชันเดียว ห้ามจองไว้ก่อน** (mig 0237/0240/0242) · **ลบร่างที่ไม่เคยอนุมัติ = เลขกลับมาใช้ได้ · เคยอนุมัติแล้ว = เลขตายถาวร** (mig 0248 · ตัดสินจาก `firstApprovedAt` ไม่ใช่ `approvalStatus`) · รหัสที่ระบบออกให้แก้ไม่ได้ · เช็คซ้ำ: ลูกค้า = เลขผู้เสียภาษี + สาขา (บล็อก) · สินค้า = ลูกค้า + ชื่อ + ขนาด (เตือน) · **§9 รหัสไซต์ `ST-XXXX-AA-BBB-CCCC` + รหัสโซน `ZN-CCCC-FF-DDDDD` (2026-08-29 · mig 0315)** — ท่อนหน้าเป็นข้อมูลของแถว เลขรันอยู่ท้าย จึงไม่ต้องแก้ SQL · จังหวัด/รหัส AR/ชั้น กลายเป็นของบังคับ · **§11 รอบตัดเลขรัน (2026-09-01 · mig 0328)** — ใบเสนอราคา/ใบสั่งขาย **ตัดทุกปี** (ET/PT ยังรายเดือน) · `YYMM` ในเลข ≠ ตัวตัดรอบ · สลับรอบต้อง seed แถวรอบใหม่ก่อนสลับฟังก์ชัน |
...
| [requests-rd-decision-log.md](requests-rd-decision-log.md) | มติสายคำร้อง RD — ม-1…ม-24 รอบแรก · ม-25…ม-62 รอบรื้อที่สอง · ม-63…ม-75 รอบแบบทั้งระบบ + ฟอร์ม PDR · ม-76…ม-94 รอบรื้อ 4 อาการ + Control Panel (2026-08-08) · ม-95…ม-96 เอกสาร PDR เหมือนกระดาษ FM-RD-01 + แบ่งหน้าเอง + พรีวิวเต็มจอครบทุกชนิด (2026-08-09) · PDR 2.2/2.3 เป็นต้นทุน/ราคาขายรายสินค้า (mig 0229) + หมวดใบสั่งขายที่เปิดคำร้องพัฒนากลิ่นได้ (2026-08-10) · รื้อโมดูล RD: ภาพรวมตอบ "วันนี้ทำอะไรก่อน" + ลบแดชบอร์ด RD (2026-08-11) · **รับเรื่อง = ตัดรอบ · "แจ้งกำหนดส่ง" เป็นก้าวที่สอง + สถานะ "รอกำหนดส่ง" + คำใหม่ "วันที่ต้องการรับงาน" (2026-08-19)** · **ม-137 ฝ่ายดึงงานที่ส่งแล้วกลับมาแก้ได้ (สายเอกสาร RD/FN — เผื่อแนบผิด · 2026-08-20)** · **ม-141 พัฒนาสูตร standard ↔ NPD (mig 0351) · ม-143 PDR ข้อ 2.x รายสินค้า + กลิ่นจากทะเบียน (NPD) + ที่อยู่ 1.7 จากทะเบียน + 1.15 Archetype (mig 0352 · 2026-09-11) · ม-144 ทางส่งของ NPD: แถวงานแตกจากแบบฟอร์ม PDR ตอนรับเรื่อง** · **ม-148 พัฒนากลิ่น "ส่งเป็นอะไร": หัวน้ำหอม = F · สินค้า = กลิ่น + สูตร · ใส่ราคาสูตรได้ F·B·FB (RM_B · mig 0372 · 2026-09-22)** · **ม-149 Project Coordinator ลบกลิ่น/สูตรได้ทุกสถานะเมื่อไม่มีใครอ้าง (cap `registry:delete` · 2026-09-22)** · **ม-150 แชร์กลิ่น/สูตรให้ลูกค้ารายอื่น — ใช้ได้เหมือนของตัวเอง · RD แชร์ (mig 0373 · 2026-09-22)** · **ม-151 PDR 1.8 ไทย/ต่างชาติ จากทะเบียนลูกค้า (2026-09-23)** |
...
|---|---|---|
| [fm-sa-04-document-model.md](fm-sa-04-document-model.md) | **ใบสเปคสินค้า FM-SA-04 — โมเดลเอกสารชุดใหม่** (มติ 2026-09-21) — สเปคในฐานข้อมูลไม่มีเลข ไม่มี Rev · เลขที่ FM-SA-04-DDMMYY-XXX + Rev + ด่าน AC → AE เจ้าของดีล → AE Sup อยู่ที่เอกสารที่ออกจากบรรทัด SO · แทนโมเดลสามชั้นของ 0364/0369 · mig 0370 รันแล้ว · ขึ้น prod #1764 · มติ 22/09 กระดาษ "รายละเอียดผลิตภัณฑ์": หัวแบบใบเสนอราคา · ภาษาตาม SO · เลขที่ DDMMYY-XXX-RR · ตัดหน้าตามหัวข้อ + เลขข้อ · ช่องลงนามแบบ QT/SO (รูปลายเซ็นจริง · ตำแหน่งเต็ม ใช้ร่วมกับ QT/SO) · ปริมาตรบรรจุ/จำนวนผลิตใน Product Overview (ขึ้น prod #1782 · สีแบบใบเสนอราคา + ภาพขึ้นแผ่นใหม่ #1785) · รอบสี่ (มติ 22/09): accent ที่ชื่อเอกสารที่เดียว + หัวข้อตามภาษา "ไทย / ENGLISH" แบบใบเสนอราคา (แบรนช์ `claude/spec-accent-headings`) · รอบห้า (มติ 23/09): หน้า "ออกเอกสาร" แบบใบเสนอราคา `/sales-planning/spec-documents/new` — เลขที่ออกตอนกดบันทึก ยกเลิกไม่เหลืออะไร · รอบหก (มติ 23/09): **ลบร่างที่ยังไม่เคยยื่นได้** เหมือนร่างใบเสนอราคา — AC/admin · ยื่นแล้ว/ดึงกลับ/ถูกตีกลับ ลบไม่ได้ (ใช้ยกเลิกเอกสาร) · **เลขที่ไม่นำกลับมาใช้** ตัวนับไม่ถอย · mig 0375 (คอลัมน์ `firstSubmittedAt` + RPC `delete_product_spec_document_draft`) **ยังไม่รัน — ต้องรันก่อน deploy** (โค้ดอ่านคอลัมน์นั้นแล้ว · PostgREST ล้มทั้งคำสั่งถ้าไม่มีคอลัมน์) (ทั้งสองรอบอยู่แบรนช์ `claude/spec-doc-new-page`) | รอตรวจ |
| [historical-sales-orders.md](historical-sales-orders.md) | **ใบสั่งขายย้อนหลัง (SO ย้อนหลัง)** — คีย์งานบริการที่ขายไปแล้วนอกระบบ (มติ 2026-09-14 ข้อ 1–21 + คำตอบคำถามเปิด 15/09 + ข้อ 22–23 เคาะ 16/09) · ตารางเดียว + ธง `origin` (`pipeline`/`historical`) บน `sales_orders`/`sales_deals` · **เก็บยอดจริง กรองที่ตัวคำนวณ** (ไม่นับ Actual/FC/เป้า) · ดีลภาชนะ 1 ใบต่อลูกค้า × AE (AE บังคับ · ย้ายเจ้าของทีละใบ) · TS ผูกโซนในคิวงานเข้าใหม่ · ยกเว้นด่านเงินรายใบ (ข้อ② เท่านั้น) · **P1 ขึ้น prod 16/09 (#1723 · mig 0360 รันแล้ว)** · P2a โมดัลคีย์ 4 ขั้น + ชิป/ตัวกรอง/รางบนทะเบียน SO โค้ดครบ รอตรวจ (§8A) · P2b ข1 "TS ไม่พบจุดนี้หน้างาน" ขึ้น prod 16/09 (#1737 · mig 0362 รันแล้ว · §8B) · ข2 ถอดบรรทัด + คิดเงินหัวใบใหม่ + กระดิ่งถึงผู้คีย์ โค้ดครบ รอตรวจ (§8C · 🛑 **mig 0366 ต้องรันก่อน merge**) · เฟส 3 (คีย์ของจริง) เริ่มได้ | กำลังดำเนินการ |
| [deal-lead-link.md](deal-lead-link.md) | **ผูกดีลกับลีดต้นทางย้อนหลัง (สองทาง)** (มติ 2026-09-22) — SA ลืมกดเปิดดีลจากลีด ⇒ เลือกลีดในฟอร์มเพิ่มดีล · ปุ่มผูก/ถอดที่หน้าดีล · "ผูกดีลที่มีอยู่" ที่หน้าลีด · ลีดทุกสถานะรวมไม่ไปต่อ (รอคัดกรอง/รอกระจายเฉพาะ Admin/AE Sup) · ถอด/ลบดีล = ลีดที่ไม่เหลือดีลกลับสถานะเดิมไม่มีวันติดตาม · mig 0371 รันแล้ว 22/09 | รอตรวจ |
| [legacy-deal-switch.md](legacy-deal-switch.md) | **สวิตช์ "ดีลเก่าจากระบบเดิม" ไม่มียอด** (มติ 2026-09-14) — Actual มาจาก SO อนุมัติเท่านั้น (trigger ไม่แก้) · ดีลเก่า Won = บันทึกงาน ไม่มีมูลค่า ไม่เข้า FC · server ตีกลับถ้าส่งยอด · ยอดที่เคยพิมพ์ 15 ใบ (1,956,850) ย้ายเป็นบันทึกบนหน้าดีล (mig 0359 รันแล้ว 2026-09-15) · มติ 2026-09-15: ไม่นับใน Won รอยื่น SO · คำใต้การ์ด "ดีลเก่าจากระบบเดิม · ไม่มีใบสั่งขายในระบบนี้" · client เขียน `wonSource`/`acceptedQuotationId` ไม่ได้ (ห้ามถอด `legacy`) · ค้างเจ้าของยืนยันดีลที่ยังผลิต (บล็อก B 11 ใบ) / 5 ใบ ส.ค.–ก.ย. / ข้อมูลผิด 4 ใบ | รอตรวจ |
...
| [sales-contract-plan.md](sales-contract-plan.md) | ระบบสัญญาในบริหารงานขาย · **§5.0 = ทะเบียนแม่แบบที่เดียว** (ออกแบบกลิ่น `20260813` · บริการ `20260903` · บันทึกเพิ่มเติม `20260821c` · **จ้างผลิตยังไม่มีต้นฉบับ**) · Rev. · บันทึกเพิ่มเติม · เอกสารภายนอกใช้แทนสัญญา · ขั้นรับรอง AE Sup · mig 0278/0280/0282/0322/0323/0324/0327 รันครบ · ยังไม่ UAT ครบวงสาย generated | รอตรวจ |
...
| [site-survey-request-plan.md](site-survey-request-plan.md) · [site-survey-mockup.html](site-survey-mockup.html) · [site-survey-prototype.html](site-survey-prototype.html) | **คำร้องประเมินพื้นที่ SA → TS (2026-08-29)** — หัวข้อคำร้องตัวที่ 6 ของฝ่าย TS: SA ขอ → TS ลงคิววันเวลา+ช่าง → ไปวัด ก×ย×ส (เมตร) + รูป → สรุปจำนวนแพ็คเกจต่อโซน → ปิดสองฝั่ง · **ไม่มีตารางใหม่** เพิ่มคอลัมน์ + ขยาย CHECK (`dept_requests.dept` รับ TS · `service_visits.kind` รับ survey · ขนาดที่ `service_zones`) · มติเคาะครบ 21 ข้อ (§8) · ม็อกออกแบบ 14 จอ + **ม็อกเดินเรื่องเสมือนจริง 9 ขั้น + 4 ทางแยก** (คำร้อง → ผลถึง SA → ปิดเรื่อง) · 🅿️ สายใบเสนอราคาพักไว้ · **เฟส 1 (เปิดใบ → คิว TS → รับเรื่อง) ขึ้นแล้ว 29/08 · mig 0313/0314 รันแล้ว** · **แยก "วันเข้าพื้นที่" ออกจาก "วันส่งผล" ทั้งฝั่ง SA และ TS (mig 0368 · 21/09 — §4A)** | กำลังดำเนินการ |
...
| [so-centric-workflow-model.md](so-centric-workflow-model.md) | **ต้นแบบเส้นงานที่มี SO เป็นศูนย์กลาง (มติ 2026-08-30)** — หลัก 8 ข้อ (ใบเดียวเป็นศูนย์กลาง·เกณฑ์ระดับใบตัวตัดสินเดียว·toggle สาย·การ์ดข้ามโมดูล·เอกสารผูกพัน+ทางสำรอง·เงินนำงาน paidThrough·ด่านคำนวณสด·วงจรต่ออายุ) + เช็กลิสต์ 7 คำถามตอนขยายเส้นใหม่ · reference implementation = เส้นบริการ 02-001 | เสร็จสมบูรณ์ |
| [service-contract-phase-plan.md](service-contract-phase-plan.md) · [service-contract-phase-mockup.html](service-contract-phase-mockup.html) · [service-contract-phase-prototype.html](service-contract-phase-prototype.html) | **เฟสสัญญา + ด่านเงิน สายบริการ — มติ 2026-08-30 ปลด P-A** (ม็อก 12 จอทุกโมดัล SA/AE Sup/FN/TS + ผังว่าแต่ละชิ้นวางตรงไหน + **ม็อกเดินเรื่องกดได้ 12 ขั้น 3 ทางแยก แยกจอตามฝ่าย**) · จ่ายก่อนบริการเสมอ (paidThrough จาก `coversFrom/To` บนงวด SO — **ไม่สร้าง `service_contract_installments` แล้ว**) · สัญญาเอกสารภายนอก + อนุมัติ AE Sup (ปลดล็อกโดยไม่รอต้นฉบับ) · จำนวนรอบขายบน SO line · ทะเบียนต่อสัญญา 90 วัน + followup · ด่านราย (นัด×โซน) เปิดเฉพาะส่วนที่ SO ครอบ · หน้า SO รื้อเป็น 5 แท็บ+แถบสถานะเส้น · **แผนละเอียดระดับไฟล์ M1–M5 / PR-A→F พร้อมลงมือ — เขียนไว้ให้เซสชันใหม่อ่านไฟล์เดียวเริ่มงานได้** · **PR-A (ช่วงครอบงวด + จ่ายถึง) โค้ดเสร็จ รอ merge + รัน mig 0320** | กำลังดำเนินการ |
| [service-uat-2026-08-30.md](service-uat-2026-08-30.md) | รอตรวจ | สคริปต์ UAT ใบประเมินพื้นที่เฟส 1–2 (12 ข้อ) — เข้าระบบด้วยเบอร์ · เปิดใบ · ลงคิว · เลื่อนสองทาง · ลงคิวใหม่ · สิทธิ์รายตำแหน่ง + วิธีเก็บกวาด |
...
**มติ (ADR) 16 ใบ** อยู่ที่ [system-modernization/decisions/](system-modernization/decisions/) —
