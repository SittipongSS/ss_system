...
⇒ ยุบเป็นตารางเดียวไม่ได้ (สัญญาใบเดียวครอบหลายไซต์ · ไซต์เดียวมีหลายโซนที่ทยอยหมด
แต่การโทรหาลูกค้าเป็นเรื่องเดียว) ·
...
**เนื้อ:** แถว = ไซต์ที่ active term จบใน ≤90 วัน หรือจบแล้วยังไม่ปิดเรื่อง (**คำนวณสด — ห้ามเก็บสถานะ**)
🛠 **แก้จากแผนเดิม (2026-09-06)** — แผนสั่งให้คำนวณจาก `term.endDate` แต่ **mig 0324 ย้ายแหล่ง
ความจริงของช่วงสัญญาไปที่ใบสั่งขายแล้ว** (`sales_orders."serviceContractId"` →
`sales_contracts."expiryDate"`) และปิดป้ายบนคอลัมน์เก่าว่าห้ามเขียน · PR-E เดินตามแผนฉบับก่อน 0324
⇒ อ่านคอลัมน์ที่ไม่มีใครเขียน ⇒ **ทะเบียนว่างถาวร กระดิ่งไม่ยิงสักใบ** (แก้แล้ว #PR ต่อสัญญา 06/09)
...
🛠 **แถบสถานะเส้น 4 ช่อง — ตัดออก (มติผู้ใช้ 2026-08-31 หลังดูม็อกเทียบสามทรง)**
ผู้ใช้เลือก "ทาง ก": ป้ายบนหัวแท็บบอกสถานะครบอยู่แล้ว (`สัญญา · ยังไม่ผูก` · `การชำระ 1/3`)
และกดครั้งเดียวเจอเหตุเต็มในแท็บนั้น ⇒ ไม่ต้องมีที่ที่สองมาเขียนเรื่องเดียวกันแล้วคอยดูแลให้ตรงกัน
· **เส้นเดินงานเดิมอยู่ครบ** (บรีฟกลิ่น/ขึ้นทะเบียน/ของเข้า/ผลิต/ยื่นภาษี) — ใบส่วนใหญ่เป็นสายสินค้า
ถอดเส้นนั้นคือเสียมากกว่าได้ · ม็อกที่ใช้ตัดสิน: artifact "หัวใบสั่งขาย สามทาง"
**Deep link:** `?tab=contract|payment|service|history` — คิว FN/TS/กระดิ่ง ลิงก์ตรงแท็บ
🪤 **`#payment` ของเดิมต้องยังทำงาน** — ทะเบียนการชำระของฝ่ายบัญชีลิงก์ `/sa/sales-orders/{id}#payment`
(ลิงก์ที่ส่งไปแล้วแก้ย้อนหลังไม่ได้) ⇒ แปลง hash เป็นแท็บตอนโหลด · และต้องผูก `order?.id`
ไว้ในรายการพึ่งพาของเอฟเฟกต์ ไม่งั้นอ่าน hash ไม่ทันตอนใบยังโหลดไม่เสร็จ (วัดเองบนพรีวิว)
**กติกา:** ย้ายของเดิม **ห้ามเขียนการ์ดซ้ำสองชุด** — component เดิมย้ายที่อยู่เฉย ๆ · แท็บใช้ primitive `Tabs` ของระบบ
**ตรวจรับ (แก้ตามมติจริง):** ใบสายสินค้า = **3 แท็บ** (ภาพรวม · การชำระ · ประวัติ) ·
ใบบริการ = **5 แท็บ** (+ สัญญา + งานบริการ) · ไม่มีราง 4 ช่อง · เส้นเดินงานเดิมอยู่ครบทั้งสองแบบ
🛠 แท็บ "สัญญา" ผูกกับเกณฑ์ `orderHasServiceRounds` เป๊ะ ๆ ตามการ์ดเดิม — ใบสายสินค้ายังไม่มี
ที่เก็บสัญญาบนใบ (`serviceContractId` เกิดมาเพื่อสายบริการ · mig 0324) ⇒ ใส่แท็บเปล่าให้ไม่ได้
> ⚠️ **ถูกแทนแล้ว 25/09/2569** — แท็บ "สัญญา" ขึ้นทุกใบ (มติเจ้าของ "เพิ่มสัญญาในหน้า SO" → "ทุกใบ") เพราะแท็บ
> มีการ์ดสัญญาทุกฉบับของดีลแล้ว ไม่ใช่แท็บเปล่า · การ์ดผูกสัญญาบริการยังเฉพาะเส้นบริการ · ใบย้อนหลังนอกเส้นบริการไม่มีแท็บ
> ดู [sales-contract-plan.md §5.7](sales-contract-plan.md) (และแท็บ "เอกสาร" ขึ้นทุกใบตั้งแต่ #1834 ⇒ นับแท็บข้างบนไม่ตรงแล้ว)
