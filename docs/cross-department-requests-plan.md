...
- สถานะ: **เสร็จสมบูรณ์** · ตรวจกับโค้ดเมื่อ 2026-08-12

> ⚠️ **บรรทัดสถานะเดิมเขียนว่า "มติครบแล้ว รอเริ่ม PR-1" ค้างไว้จนถึง 2026-08-12**
> ทั้งที่ของถูกสร้างครบไปแล้วตั้งแต่ mig 0171–0174 · นี่คืออาการที่ `AGENTS.md` เตือน
> ไว้เอง (เนื้อในเดินหน้า แต่ไม่มีใครแก้บรรทัดสถานะ) ⇒ **ตรวจกับโค้ดจริงแล้วปิดเล่ม**
>
> หลักฐานที่ตรวจ (ไฟล์จริงในรีโป ไม่ใช่ความจำ):
> - **PR-1/PR-2** ทะเบียนกลิ่น+สูตร `0171_scent_formula_registry.sql` · รวมคำร้องเป็น
>   `dept_requests` `0173_dept_requests.sql` (เปลี่ยนชื่อจาก `material_price_asks`)
> - **PR-6** ปลดระวางเมนูสอบถาม `0174_retire_inquiries.sql` (DROP `inquiries` ·
>   `inquiry_messages`) · เธรดรวมอยู่ที่ `entity_updates` แล้ว
> - **PR-7** แจ้งเตือนรายคน + ตัวนับยังไม่ได้อ่าน — `lib/notifications.js:unreadCount`
>   และ `/api/notifications`
> - **PR-0** role `executive` แยกจาก `admin` แล้ว (`lib/permissions.js`)
> - **ชนิดคำร้องขยายรายฝ่ายได้เอง** — `lib/requests/kinds/{rd,pc,fn,shared}/`
>   (เกินกว่าที่แผนนี้วางไว้ · โจทย์ "แต่ละฝ่ายขยายเองได้" ถูกทำในรอบหลัง)
>
> **สิ่งที่ยังค้างจริง ๆ ย้ายไปอยู่ `request-hub-rebuild-plan.md` (R-5)** — FK
> `scentId`/`formulaId` ยังเป็น `ON DELETE SET NULL` ทั้ง `dept_requests` และ
> `material_prices` ⇒ ลบทะเบียนแล้วลิงก์หายเงียบ · ที่เหลือในเล่มนี้ไม่มีอะไรค้าง
...
> 🔄 **รูปแบบเปลี่ยนแล้ว 2026-08-31 — ตารางข้างล่างเป็นของเดิม เก็บไว้อ่านเลขเก่า**
> ของใหม่คือ **`RQ-AA-YYMMXXXX`** (ดูหัวข้อ "รูปแบบเลขที่คำร้อง" ท้ายไฟล์)

| ชนิด | scope | รูปแบบ (ของเดิม ก่อน 31/08) |
|---|---|---|
| `price_f` / `price_fb` | `RM` | `RM-YYMMXXXX` (เลิกใช้แล้ว) |
| `price_pm` | `PM` | `PM-YYMMXXXX` (เลิกใช้แล้ว) |
| `scent_brief` บรีฟออกแบบกลิ่น | `SB` | `SB-YYMMXXXX` |
| `mockup` ขอ Mock-up | `MU` | `MU-YYMMXXXX` (เลิกใช้แล้ว) |
...
**นำกลับมาใช้กับหัวข้อ "สอบถามข้อมูล" แล้ว 2026-08-31** · ตรวจฐานก่อนใช้ซ้ำ:
ไม่มีแถว `IQ` ใน `entity_number_counters` เลย ⇒ เริ่มนับ 0001 ใหม่ ไม่ทับของใคร
...
**ก้อนสอง — ผลลัพธ์ของคำร้อง (ทำแล้ว):**

🔴 **เจอก่อนอื่นเลย: หน้ารายละเอียดคำร้อง `/sa/requests/[id]` เปิดไม่ได้ทั้งหน้าบน prod**
— `import { … askProgress }` (ชื่อก่อน rename ใน PR-2) แต่โค้ดเรียก `requestProgress`
→ `Runtime ReferenceError` ทุกครั้งที่ render · build/เทสต์ผ่านหมดเพราะ Turbopack ไม่
error ตอน import ชื่อที่ไม่มี และตัวแปรที่ไม่ได้ import พังตอน render เท่านั้น
→ ✅ **เปิดกฎ `no-undef` ใน eslint** (ปิดอยู่โดยปริยายใน `eslint-config-next` เพราะ
ฝั่ง TS มี type checker ทำแทน — โปรเจกต์นี้เป็น JS ล้วนจึงไม่มีใครตรวจเลย)
เปิดแล้วเจออีก 3 จุดที่พังจริงบน prod แบบเดียวกัน:
  - `components/ui/Toast.js` ใช้ `normalizeToast` โดยไม่ import — `ToastProvider` อยู่ใน
    root layout → toast ทุกตัวที่ยิงผ่าน provider พัง
  - `sa/projects/[id]` + `sahamit/po/[id]` ใช้ `notifyToast` โดยไม่ import → ทางที่ error
    จะพังซ้อน error เดิม ผู้ใช้ไม่เห็นข้อความอะไรเลย

🐞 **หน้ารายละเอียดยังส่ง entityType ชุดก่อน mig 0173** (`material_ask` / `material_ask_item`)
เธรดจึงอ่านคนละคีย์กับที่ server เขียนเหตุการณ์ลง (`dept_request`) = **ไม่เห็นทั้ง
เหตุการณ์ของระบบและข้อความเก่า** และข้อความใหม่ตกไปอยู่คีย์ที่ไม่มีใครอ่าน · ไฟล์แนบ
พลาดคู่กัน (prod มีไฟล์ 1 แถวใต้ `dept_request_item` ที่หน้าจอมองไม่เห็น)
→ ✅ เพิ่ม `lib/master/entityTypeUsage.test.mjs` ผูก JSX เข้ากับทะเบียนกลาง

- ✅ **ชนิดที่ไม่มีบรรทัดจบงานได้แล้ว** — 5 ใน 8 ชนิด (สอบถาม/บรีฟกลิ่น/ขอ mockup/
  ขอเอกสาร/ติดตามของเข้า) เคยเปิดแล้ว**ค้างถาวร**: `canClose` อ่านจาก `progress.complete`
  ซึ่งเป็น false เสมอเมื่อ `total = 0` และไม่มีปุ่ม "ตอบแล้ว" เลย
  → ใช้ `closeRequestError` / `answerRequestError` ของ lib เป็นตัวตัดสิน (ตัวเดียวกับ
  ที่ server ใช้) + ปุ่ม "ตอบแล้ว" ให้ผู้ตอบ + หัวเรื่อง/สรุป/ขั้นตอนพูดภาษาของชนิดนั้น
  (เดิมเขียนว่า "เคสขอราคาวัสดุ" ทุกจุด บรีฟกลิ่นขึ้นว่า "รายการ 0 · ตอบแล้ว 0/0")
  + แสดง `body` ของคำร้อง ซึ่งหน้านี้ไม่เคยแสดงเลย (ผู้ตอบต้องเดาว่าเขาขออะไร)
- ✅ **ปิดบรีฟกลิ่นต้องระบุผลลัพธ์** — ผูกกลิ่นเดิม / เพิ่มกลิ่นใหม่ / ไม่ได้กลิ่น
  - ⚠️ **ไม่เดาชื่อกลิ่นจากหัวเรื่องคำร้อง** — หัวเรื่องเป็นข้อความบรีฟ ไม่ใช่ชื่อกลิ่น
    (บทเรียนตรงจาก prod: สินค้า 10 แถวเอาชื่อกลิ่นไปกรอกช่องชื่อสูตร)
  - สร้างใหม่เรียก `createScent` ตัวเดิมของหน้าทะเบียน — RD + ใส่รหัส = เข้าทะเบียนเลย
    (`developing`) · คนอื่น = ร่างรอ RD รับ · ผูกข้ามลูกค้าถูกบล็อก (มติ 9)
  - "ไม่ได้กลิ่น" เป็นคำตอบที่ถูกต้อง — บรีฟที่ลูกค้าไม่เอาต้องปิดได้
- ✅ **ตอบราคา F/FB ประทับ `scentId`/`formulaId` ลงแถววัสดุ** — คอลัมน์ของ mig 0171
  ที่ยังไม่มีโค้ดไหนเขียนลงเลย · **ไม่ทับของเดิม** (วัสดุตัวหนึ่งถูกถามซ้ำได้หลายใบ
  ถ้าใบหลังทับได้ ประวัติราคาจะชี้กลิ่นผิดตัวย้อนหลังทั้งชุด)
...
## รูปแบบเลขที่คำร้อง — `RQ-AA-YYMMXXXX` (มติผู้ใช้ 2026-08-31)

> *"รหัสคำร้อง ... เป็น RQ-AA-YYMMXXXX · AA คือตัวย่อ หัวข้อคำร้อง แต่ฝ่ายแต่ละหัวข้อ"*

**ของเดิมตัวย่อหัวข้อ *แทนที่* `RQ`** ⇒ เลขคำร้องขึ้นต้นไม่เหมือนกันสักใบ
(`SB-` `FD-` `DC-` `DF-` `AS-` `RQ-`) มองผ่าน ๆ ไม่รู้ว่าเป็นคำร้อง
**ของใหม่: ทุกใบขึ้นต้น `RQ` เหมือนกันหมด แล้วตัวย่อหัวข้ออยู่กลาง**

| หัวข้อ | ฝ่าย | ตัวย่อ | ตัวอย่าง |
|---|---|---|---|
| สอบถามข้อมูล | ทุกฝ่าย | **`IQ`** (เดิม `RQ`) | `RQ-IQ-26080001` |
| พัฒนากลิ่น | RD | `SB` | `RQ-SB-26080018` |
| พัฒนาสูตร | RD | `FD` | `RQ-FD-26080010` |
| ขอเอกสาร | RD | `DC` | `RQ-DC-26080005` |
| ขอเอกสารการเงิน | FN | `DF` | `RQ-DF-26080014` |
| ประเมินพื้นที่ | TS | `AS` | `RQ-AS-26080011` |
| ~~ติดตามของเข้า~~ (ปลดระวาง) | PC | `RQ` | ต้องตั้งตัวย่อเองก่อนเปิดคืน |

### เลขรันรวมก้อนเดียว ตัดรอบทุกปี (มติผู้ใช้ 2026-08-31 รอบสอง)

> *"RQ ก็ XXXX เปลี่ยนเป็นเลขรันรวมตัดทุกปีแทน"*

ของเดิมแยกตัวนับ **รายหัวข้อ** และตัดรอบ **ทุกเดือน** ⇒ `RQ-SB-26080001` กับ
`RQ-DC-26080001` เป็นคนละใบที่ลงท้ายเลขเดียวกัน · ของใหม่เลขไม่ซ้ำกันทั้งปีไม่ว่าหัวข้อไหน
...
| ค่า | คือ | ตัวอย่าง |
|---|---|---|
| `scope` ของตัวนับ | คงที่ `RQ` ทุกหัวข้อ | `RQ` |
| `month` ของตัวนับ | **ปี** 2 หลัก (นาฬิกาไทย) | `26` |
| `prefix` | `RQ-` + ตัวย่อหัวข้อ + เดือนที่กดส่ง | `RQ-SB-2608` |

⚠️ **ตัวย่อหัวข้อไม่ใช่คีย์ตัวนับอีกแล้ว** — เหลือหน้าที่เดียวคืออยู่ในเลขให้คนอ่าน
(ทะเบียนหัวข้อยังเก็บไว้ในช่องชื่อ `scope` ตามเดิม เพราะเปลี่ยนชื่อฟิลด์กระทบหลายไฟล์)

⇒ **ไม่ต้องมี migration** — RPC 0243 รับ `p_scope`/`p_month`/`p_prefix` เป็นพารามิเตอร์
· ตัวนับเดิม (`SB/2608`, `RQ/2608` …) ไม่ถูกแตะ · ตัวนับใหม่คือแถว `RQ/26` ซึ่งยังไม่มี
จึงเริ่มที่ 0001 และไม่ชนกับเลขเก่าเพราะสตริงคนละรูป

🪤 กิ่ง seed ของ RPC ใช้ `LIKE p_prefix || '%'` ซึ่งผูกกับหัวข้อ+เดือน ⇒ ถ้าแถวตัวนับของ
ปีนั้นหายไป มันจะ seed จากเลขของหัวข้อเดียวแล้วนับซ้ำ · กิ่งนั้นทำงานเฉพาะตอนแถวหาย
ซึ่ง trigger ของ mig 0241 กันไว้ไม่ให้ลบอยู่แล้ว

⚠️ **ใบเก่าแปลงไม่ได้** — trigger `dept_request_doc_no_immutable` ห้าม UPDATE `docNo`
⇒ สองรูปแบบอยู่ร่วมกันถาวร (กติกาเดียวกับเลขที่สัญญา)
🪤 การเรียงคิวด้วย "เลขที่" เป็นการเทียบสตริงดิบ — เลขสองรูปแบบปนกันจะเรียงแยกกอง

✅ **ลิงก์ `/go/<รหัส>` กดได้แล้ว** (2026-09-01) — เดิม `docRefs.js` รับ "คำนำหน้า 2 ตัว +
ขีด + ตัวเลข" (`CODE_BODY = [0-9][0-9-]*[0-9]`) ⇒ `RQ-SB-…` มีตัวอักษรกลาง จึงไม่เข้า
แพตเทิร์น และไม่มี scope ไหนลงทะเบียนใน `DOC_REF_TYPES` เลย (`DR` ที่ลงไว้เป็นคำนำหน้า
ที่ **ไม่เคยมีเอกสารใบไหนใช้จริง** — ถอดออกแล้ว)

ตอนนี้ทะเบียนถือ **รูปแบบรายคำนำหน้า** (`body`) แทน regex ร่วมตัวเดียว ⇒ ลงทะเบียนครบ
ทั้งสามยุคของเลขคำร้อง (`RQ-26080064` · `SB-26080014` · `RQ-SB-26080008`) และเลขชุดอื่น
ที่คนอ้างในเธรด (สัญญา · แจ้งปัญหา · นัดบริการ · ไซต์ · ลูกค้า · สินค้า)
🪤 `PB-` ใบผลิต กับ `ZN-` โซน **ยังไม่ลงทะเบียน** เพราะไม่มีหน้ารายละเอียดของตัวเองให้
redirect ไป — เทสต์ในไฟล์ `docRefs.test.mjs` เช็คว่า path ของทุกคำนำหน้ามีหน้าจริงรองรับ
