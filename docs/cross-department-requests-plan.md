...
⭐ **`scope` กับ `prefix` เป็นคนละเรื่อง** — `scope` คือคีย์ของตัวนับ
(`entity_number_counters`) ส่วน `prefix` คือสตริงที่ต่อหน้าเลขรัน
...
🪤 ห้ามเอา `RQ-` ไปใส่ใน `scope` — SQL เตะด้วย `^[A-Z]{2,4}$` และตัวนับจะเริ่มใหม่ทุกหัวข้อ
