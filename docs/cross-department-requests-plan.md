...
⚠️ **ลิงก์ `/go/<รหัส>` ยังกดไม่ได้** — `docRefs.js` รับ "คำนำหน้า 2 ตัว + ขีด + ตัวเลข"
(`CODE_BODY = [0-9][0-9-]*[0-9]`) ⇒ `RQ-SB-…` มีตัวอักษรกลาง จึงไม่เข้าแพตเทิร์น
· เดิมก็กดไม่ได้อยู่แล้ว (ไม่มี scope ไหนลงทะเบียนใน `DOC_REF_TYPES` เลย — `DR` ที่
ลงไว้คือคำนำหน้าของ **id แถว** ไม่ใช่เลขที่) ⇒ **ไม่ใช่ของที่เพิ่งพัง** แต่ถ้าจะทำให้กดได้
ต้องขยาย `CODE_BODY` ซึ่งเป็น regex ร่วมของ QT/SO/PJ/DL/CR/DR — แยกเป็นงานของตัวเอง
